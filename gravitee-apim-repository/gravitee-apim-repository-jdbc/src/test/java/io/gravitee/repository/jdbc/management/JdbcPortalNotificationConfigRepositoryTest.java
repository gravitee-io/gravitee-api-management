...
public class JdbcPortalNotificationConfigRepositoryTest {
    ...
    public void generateQuery_withHookAndOrgId() {
        PortalNotificationCriteria criteria = PortalNotificationCriteria.builder().hook("hook1").orgId("org1").build();
        String query = repository.generateQuery(criteria);
        String expectedQuery =
            "select pnc.`user`, pnc.reference_type, pnc.reference_id, pnc.created_at, pnc.updated_at  from " +
            ...
            "where 1=1 and pnch.hook = ? and pnc.org_id = ?";
