...
public class JdbcAlertRepository extends JdbcAbstractCrudRepository<AlertTrigger, String> implements AlertTriggerRepository {
    ...
    private static final JdbcObjectMapper ORM = JdbcObjectMapper
        .builder(AlertTrigger.class, "alert_triggers", "id")
    ...
    public List<AlertTrigger> findByReference(final String referenceType, final String referenceId) throws TechnicalException {
        ...
        try {
            List<AlertTrigger> rows = jdbcTemplate.query(
                "select * from alert_triggers where reference_type = ? and reference_id = ?",
                ORM.getRowMapper(),
                referenceType,
                referenceId
            );
    ...
    public Optional<AlertTrigger> findById(String id) throws TechnicalException {
        ...
        try {
            Optional<AlertTrigger> alert = jdbcTemplate
                .query(SELECT_ESCAPED_ALERT_TRIGGERS_TABLE_NAME + " a where id = ?", ORM.getRowMapper(), id)
                .stream()
                .findFirst();
            if (alert.isPresent()) {
                addEvents(alert.get());
