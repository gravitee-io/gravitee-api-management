...
public class JdbcPortalNotificationConfigRepository extends TransactionalRepository implements PortalNotificationConfigRepository {
    ...
    public void deleteByUser(String user) throws TechnicalException {
        ...
        try {
            jdbcTemplate.update("delete from portal_notification_configs" + " where " + escapeReservedWord("user") + " = ?", user);
    ...
    public void deleteReference(NotificationReferenceType referenceType, String referenceId) throws TechnicalException {
        ...
        try {
            jdbcTemplate.update(
                "delete from portal_notification_configs" + " where reference_type = ?" + " and reference_id = ? ",
                referenceType.name(),
                referenceId
            );
    ...
    private void storeHooks(PortalNotificationConfig parent, boolean deleteFirst) {
        ...
        if (parent.getHooks() != null && !parent.getHooks().isEmpty()) {
            jdbcTemplate.batchUpdate(
                "insert into portal_notification_config_hooks ( " +
                escapeReservedWord("user") +
