...
public class JdbcApplicationRepository extends JdbcAbstractCrudRepository<Application, String> implements ApplicationRepository {
    ...
    private static final JdbcObjectMapper ORM = JdbcObjectMapper
        .builder(Application.class, "applications", "id")
    ...
    public Optional<Application> findById(String id) throws TechnicalException {
        ...
        try {
            ...
            jdbcTemplate.query(
                "select a.*, am.k as am_k, am.v as am_v from applications a left join application_metadata am on a.id = am.application_id where a.id = ?",
                rowMapper,
                id
            );
    ...
    public Set<Application> findAll(ApplicationStatus... ass) throws TechnicalException {
        ...
        try {
            ...
            JdbcHelper.CollatingRowMapper<Application> rowMapper = new JdbcHelper.CollatingRowMapper<>(mapper, CHILD_ADDER, "id");
            jdbcTemplate.query(query.toString(), (PreparedStatement ps) -> ORM.setArguments(ps, statuses, 1), rowMapper);
            for (Application application : rowMapper.getRows()) {
                addGroups(application);
            }
    ...
    public Page<Application> search(ApplicationCriteria applicationCriteria, Pageable pageable) {
        ...
        final StringBuilder sbQuery = new StringBuilder("select ")
            ...
            .append(" left join application_metadata am on a.id = am.application_id ");

        if (applicationCriteria != null) {
            sbQuery.append("where 1 = 1 ");
            if (!isEmpty(applicationCriteria.getIds())) {
                sbQuery.append("and a.id in (").append(ORM.buildInClause(applicationCriteria.getIds())).append(") ");
            }
        ...
        jdbcTemplate.query(
            ...
            (PreparedStatement ps) -> {
                ...
                if (applicationCriteria != null) {
                    if (!isEmpty(applicationCriteria.getIds())) {
                        lastIndex = ORM.setArguments(ps, applicationCriteria.getIds(), lastIndex);
                    }
                    if (!StringUtils.isEmpty(applicationCriteria.getName())) {
