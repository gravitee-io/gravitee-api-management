...
/**
 * JDBC implementation of ApiProductRepository.
 *
...
public class JdbcApiProductRepository extends JdbcAbstractCrudRepository<ApiProduct, String> implements ApiProductsRepository {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JdbcApiProductRepository.class);
    ...
    public ApiProduct create(ApiProduct apiProduct) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.create({})", apiProduct.getName());
        try {
            ...
            LOGGER.error("Failed to create api product:", ex);
    ...
    public ApiProduct update(final ApiProduct apiProduct) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.update({})", apiProduct.getName());
        if (apiProduct == null) {
        ...
        try {
            ...
            LOGGER.error("Failed to update api product:", ex);
    ...
    public Optional<ApiProduct> findById(String id) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.findById({})", id);
        try {
            List<ApiProduct> apiProducts = jdbcTemplate.query(
                "SELECT ap.*, apa.api_id " +
                    "FROM " +
                    this.tableName +
                    " ap " +
                    "LEFT JOIN " +
            ...
            return Optional.ofNullable(aggregateApiIds(apiProducts));
        } catch (final Exception ex) {
            LOGGER.error("Failed to find api product by id:", ex);
    ...
    public Set<ApiProduct> findAll() throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.findAll()");
        try {
            List<ApiProduct> apiProducts = jdbcTemplate.query(
                "SELECT ap.*, apa.api_id " +
                    "FROM " +
            ...
            LOGGER.error("Failed to find all api products:", ex);
    ...
    public Optional<ApiProduct> findByEnvironmentIdAndName(String environmentId, String name) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.findByEnvironmentIdAndName({}, {})", environmentId, name);
        try {
            List<ApiProduct> apiProducts = jdbcTemplate.query(
                "SELECT ap.*, apa.api_id " +
                    "FROM " +
            ...
            LOGGER.error("Failed to find api product by environment and name:", ex);
    ...
    public Set<ApiProduct> findByEnvironmentId(String environmentId) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.findByEnvironmentId({})", environmentId);
        try {
            List<ApiProduct> apiProducts = jdbcTemplate.query(
                "SELECT ap.*, apa.api_id " +
                    "FROM " +
            ...
            LOGGER.error("Failed to find api products by environment:", ex);
    ...
    public void delete(String id) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.delete({})", id);
        try {
    ...
    public Set<ApiProduct> findByApiId(String apiId) throws TechnicalException {
        LOGGER.debug("JdbcApiProductRepository.findByApiId({})", apiId);
        try {
            List<ApiProduct> apiProducts = jdbcTemplate.query(
                "SELECT ap.*, apa.api_id " +
                    "FROM " +
    ...
    private ApiProduct aggregateApiIds(List<ApiProduct> apiProducts) {
        if (apiProducts == null || apiProducts.isEmpty()) {
            return null;
        }

        ApiProduct result = apiProducts.get(0);
        List<String> apiIds = new ArrayList<>();

        for (ApiProduct apiProduct : apiProducts) {
            if (apiProduct.getApiIds() != null) {
                apiIds.addAll(apiProduct.getApiIds());
            }
        }

        if (!apiIds.isEmpty()) {
            result.setApiIds(apiIds);
        }
    ...
    private List<ApiProduct> aggregateApiProducts(List<ApiProduct> apiProducts) {
        ...
        Map<String, ApiProduct> aggregatedMap = new LinkedHashMap<>();

        for (ApiProduct apiProduct : apiProducts) {
            String id = apiProduct.getId();
            ApiProduct existing = aggregatedMap.get(id);

            if (existing == null) {
                aggregatedMap.put(id, apiProduct);
            } else {
                if (apiProduct.getApiIds() != null) {
                    if (existing.getApiIds() == null) {
                        existing.setApiIds(new ArrayList<>());
                    }
                    existing.getApiIds().addAll(apiProduct.getApiIds());
                }
            }
        }
