...
public class MongoAuditRepository implements AuditRepository {
    ...
    public List<String> deleteByReferenceIdAndReferenceType(Audit.AuditReferenceType referenceType, String referenceId)
        ...
        final List<AuditMongo> audits = internalAuditRepo.deleteByReferenceIdAndReferenceType(referenceId, referenceType.name());

        LOGGER.debug("Delete audit by reference [{},{}] = {}", referenceId, referenceType, audits);
        return audits.stream().map(AuditMongo::getId).collect(Collectors.toList());
    }
