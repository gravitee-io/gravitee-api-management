...
public class HazelcastRepositoryProviderTest {

    private final HazelcastRepositoryProvider provider = new HazelcastRepositoryProvider();

    @Test
    public void shouldReturnHazelcastType() {
        assertEquals("hazelcast", provider.type());
    }
    ...
    @Test
    public void shouldReturnPluginConfigurationClass() {
        Class<?> configClass = provider.configuration(Scope.RATE_LIMIT);
        assertEquals(PluginConfiguration.class, configClass);
    }
