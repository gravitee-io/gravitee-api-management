...
public class SearchMessageMetricsQueryAdapter {
    ...
    private static void addAdditionalFieldQuery(ArrayList<JsonObject> terms, String fieldName, JsonArray trimmedValues) {
        ...
        if (fieldName.startsWith("string_")) {
            if (trimmedValues.size() == 1) {
                query = JsonObject.of("match_phrase", JsonObject.of(fieldPath, trimmedValues.getString(0)));
            } else {
                var shouldClauses = new JsonArray();
                for (int i = 0; i < trimmedValues.size(); i++) {
                    shouldClauses.add(JsonObject.of("match_phrase", JsonObject.of(fieldPath, trimmedValues.getString(i))));
                }
                query = JsonObject.of("bool", JsonObject.of("should", shouldClauses, "minimum_should_match", 1));
            }
        } else {
            query = trimmedValues.size() == 1
                ? JsonObject.of("term", JsonObject.of(fieldPath, trimmedValues.getString(0)))
                : JsonObject.of("terms", JsonObject.of(fieldPath, trimmedValues));
        }
