...
public interface AuthzMeasuresAdapter {
    FieldResolver fieldResolver();

    JsonObject adaptQuery(Query query);

    JsonObject adaptMetrics(List<MetricMeasuresQuery> metrics, List<Filter> filters);

    JsonObject buildMeasureAggs(MetricMeasuresQuery metric);

    JsonObject scopeFilter(Metric metric, List<Filter> filters);
}
