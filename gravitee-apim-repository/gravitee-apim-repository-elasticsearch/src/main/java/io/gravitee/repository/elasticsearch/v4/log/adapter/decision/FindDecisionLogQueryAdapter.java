...
/**
 * Reads one decision record by its event id. The api id is part of the predicate rather than a
 * post-filter: it scopes the lookup the same way the caller's permission does, so a known event id from
 * another api cannot be read through this path.
 *
...
public final class FindDecisionLogQueryAdapter {
    ...
    public static String adapt(String apiId, String eventId) {
        ...
        filters.add(MAPPER.createObjectNode().set(TERM, MAPPER.createObjectNode().put(DecisionLogFields.API_ID, apiId)));
        filters.add(MAPPER.createObjectNode().set(TERM, MAPPER.createObjectNode().put(DecisionLogFields.EVENT_ID, eventId)));
