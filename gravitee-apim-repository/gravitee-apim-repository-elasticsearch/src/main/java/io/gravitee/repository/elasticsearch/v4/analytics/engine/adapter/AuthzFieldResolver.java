...
public class AuthzFieldResolver implements FieldResolver {
    ...
    // Only an evaluation writes `decision` and only a search writes `search-type`, so a value_count
    // on those fields counts each population exactly.
    @Override
    public String fromMetric(Metric metric) {
        return switch (metric) {
            ...
            case AUTHZ_SEARCHES -> "search-type";
            case AUTHZ_FAILURES -> STATUS;
            case AUTHZ_EVAL_DURATION -> "duration-nanos";
    ...
    public String decisionValue(Metric metric) {
        return switch (metric) {
            case AUTHZ_PERMITS -> "PERMIT";
            case AUTHZ_FORBIDS -> "FORBID";
            case AUTHZ_NOT_APPLICABLE -> "NOT_APPLICABLE";
