...
public class AnalyticsElasticsearchRepository extends AbstractElasticsearchRepository implements AnalyticsRepository {
    ...
    @Override
    public Optional<GroupByAggregate> searchGroupBy(QueryContext queryContext, GroupByQueryCriteria query) {
        var index = this.indexNameGenerator.getWildcardIndexName(queryContext.placeholder(), Type.V4_METRICS, clusters);
        return this.client.search(index, null, SearchGroupByAdapter.adaptQuery(query))
            .map(SearchGroupByAdapter::adaptResponse)
            .blockingGet();
    }
