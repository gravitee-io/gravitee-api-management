...
public class SearchRequestsCountByEventQueryAdapter {
    ...
    private static JsonObject buildElasticQuery(RequestsCountByEventQuery query) {
        ...
        addTermFilter(filters, query.apiId());
    ...
    private static void addTermFilter(List<JsonObject> filters, Optional<String> value) {
        value.ifPresent(v -> filters.add(JsonObject.of("term", JsonObject.of("api-id", v))));
    }
    ...
    public static Optional<CountByEventAggregate> adaptResponse(SearchResponse searchResponse) {
        if (searchResponse == null) {
            return Optional.empty();
        }
        long totalHits = searchResponse.getSearchHits().getTotal().getValue();
        return Optional.of(CountByEventAggregate.builder().total(totalHits).build());
    }
}
