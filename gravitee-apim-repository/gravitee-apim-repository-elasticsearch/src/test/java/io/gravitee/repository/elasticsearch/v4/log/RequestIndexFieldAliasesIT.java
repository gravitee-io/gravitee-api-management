...
public class RequestIndexFieldAliasesIT extends AbstractElasticsearchRepositoryTest {
    ...
    @Nested
    class ClientIdentifierAlias {

        @Test
        void should_resolve_client_identifier_alias_to_subscription_field() {
            var query = """
                {
                  "query": {
                    "exists": { "field": "client-identifier" }
                  }
                }
                """;
            ...
            assertThat(response.getSearchHits().getTotal().getValue())
                .as("client-identifier alias resolves to 'subscription'; no V2 fixture docs have this field populated")
                .isZero();
        }
    }
