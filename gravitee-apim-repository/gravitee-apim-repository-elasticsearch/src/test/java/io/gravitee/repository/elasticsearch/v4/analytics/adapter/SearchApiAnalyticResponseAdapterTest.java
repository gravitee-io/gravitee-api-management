...
class SearchApiAnalyticResponseAdapterTest extends AbstractAdapterTest {
    ...
    class AdaptFirst {
        ...
        void should_return_empty_when_no_hit() {
            var result = SearchApiAnalyticResponseAdapter.adaptFirst(new SearchResponse());
            assertThat(result).isEmpty();
        }

        @Test
        void should_build_api_analytics_() {
            final SearchResponse searchResponse = buildSearchHit("api-proxy-v4-metric.json");
            final Optional<ApiAnalytic> apiAnalytic = SearchApiAnalyticResponseAdapter.adaptFirst(searchResponse);
