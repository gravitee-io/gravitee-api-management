...
class SearchTopHitsAdapterTest {
    ...
    class Query {
        ...
        @Test
        void should_build_top_hits_query_searching_for_ids() {
            var queryParams = TopHitsQueryCriteria.builder().apiIds(API_IDS).build();

            var result = SearchTopHitsAdapter.adaptQuery(queryParams);

            assertThatJson(result).isEqualTo(TOP_HIT_QUERY_WITH_API_IDS);
        }

        @Test
        void should_build_top_hits_query_searching_for_top_ids_and_time_ranges() {
            var queryParams = TopHitsQueryCriteria.builder().apiIds(API_IDS).from(FROM).to(TO).build();
        ...
        void should_build_top_hits_query_with_empty_id_filter_array_for_null_query_params() {
            ...
            assertThatJson(result).isEqualTo(TOP_HIT_QUERY_WITH_EMPTY_OR_NULL_API_IDS);
        }

        @Test
        void should_build_top_hits_query_with_empty_id_filter_array_for_empty_api_ids_query_params() {
            var queryParams = TopHitsQueryCriteria.builder().apiIds(List.of()).build();
            var result = SearchTopHitsAdapter.adaptQuery(queryParams);
        ...
        private static final String TOP_HIT_QUERY_WITH_API_IDS =
            """
                 {
                     "size": 0,
                     "query": {
        ...
        private static final String TOP_HIT_QUERY_WITH_EMPTY_OR_NULL_API_IDS =
            """
                 {
                     "size": 0,
                     "query": {
