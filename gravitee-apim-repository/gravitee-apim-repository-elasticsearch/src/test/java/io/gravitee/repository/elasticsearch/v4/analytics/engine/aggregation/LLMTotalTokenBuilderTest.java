 ...
 */
class LLMTotalTokenBuilderTest {

    private final LLMTotalTokenBuilder builder = new LLMTotalTokenBuilder();

    @Test
    void should_build_sum_aggregation() {
        var result = builder.buildSum("LLM_PROMPT_TOTAL_TOKEN#COUNT");

        assertThat(result).containsKey("LLM_PROMPT_TOTAL_TOKEN#COUNT");

        var agg = result.get("LLM_PROMPT_TOTAL_TOKEN#COUNT");
    ...
    void should_build_avg_aggregation() {
        var result = builder.buildAvg("LLM_PROMPT_TOTAL_TOKEN#AVG");

        assertThat(result).containsKey("LLM_PROMPT_TOTAL_TOKEN#AVG");

        var agg = result.get("LLM_PROMPT_TOTAL_TOKEN#AVG");
