 ...
 *
 * <p>{@code apiId} is required. {@code maxBuckets} controls how many distinct error
 ...
 * so the number of distinct values per API is expected to be small in practice.
 */
public record SearchConnectionLogErrorKeysQuery(String apiId, int maxBuckets) {
    public static final int DEFAULT_MAX_BUCKETS = 100;

    /**
     * Convenience factory using the default bucket cap.
     */
    public static SearchConnectionLogErrorKeysQuery of(String apiId) {
        return new SearchConnectionLogErrorKeysQuery(apiId, DEFAULT_MAX_BUCKETS);
    }

    public SearchConnectionLogErrorKeysQuery {
        if (apiId == null || apiId.isBlank()) {
            throw new IllegalArgumentException("apiId must not be null or blank");
        }
        if (maxBuckets <= 0) {
