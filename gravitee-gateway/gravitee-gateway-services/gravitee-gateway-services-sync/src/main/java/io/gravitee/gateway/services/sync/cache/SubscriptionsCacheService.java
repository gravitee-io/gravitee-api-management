...
public class SubscriptionsCacheService extends AbstractService implements EventListener<ReactorEvent, Reactable> {
    ...
    public void register(List<Api> apis) {
        ...
        final Map<String, Set<String>> plansByApi = apis.stream()
                .filter(Api::isEnabled)
                .map(api -> new AbstractMap.SimpleEntry<>(api.getId(), api.getPlans().stream()
                        ...
                        .whenComplete((result, throwable) -> {
                            if (throwable != null) {
                                ...
                                if (result.succeeded()) {
                                    ...
                                    LOGGER.error("An error occurs while doing a full subscriptions refresh for APIs [{}]",
                                            apisById.keySet(), result.cause());
