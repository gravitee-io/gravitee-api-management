...
export async function deleteIdentityProvidersForPortal(portalId: string): Promise<void> {
    const providers = await runTransaction<{ id: string; portalId: string }[]>(
    ...
    await Promise.all(
        providers.map(provider =>
            runTransaction(PORTAL_IDENTITY_PROVIDERS_STORE_NAME, 'readwrite', store => store.delete(provider.id)),
        ),
    );
}
