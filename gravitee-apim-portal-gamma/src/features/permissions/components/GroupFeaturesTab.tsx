...
import type { PortalGroup } from '../types/permissions.types';

interface GroupFeaturesTabProps {
    readonly group: PortalGroup;
...
export function GroupFeaturesTab({ group, onSave }: GroupFeaturesTabProps) {
    const [features, setFeatures] = useState<PortalTenantFeatures>(group.features);
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-muted-foreground">
                    Control which portal capabilities are available to members of this group.
                </p>
            </div>
                    ...
                    return (
                        <div key={key} className="flex items-start justify-between gap-4 px-4 py-4">
                            <div className="min-w-0">
                                <Label htmlFor={`group-feature-${key}`} className="font-medium">
                                    {meta.title}
                                </Label>
                                <p className="text-sm text-muted-foreground">{meta.description}</p>
                            </div>
                            <Switch
                                id={`group-feature-${key}`}
                                checked={enabled}
