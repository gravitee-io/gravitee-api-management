...
log_info "Waiting 15s for API deployment to sync to Primary Gateway and Redis..."
sleep 15
...
echo "---------------------------"
echo
if ! echo "$REDIS_KEYS_OUTPUT" | grep -q -E "distributed_event::API:${API_ID}|distributed_sync_state:"; then
  log_error "DSP keys related to API ID ${API_ID} not found in Redis."
fi
log_success "Found DSP keys in Redis (verify keys for API ID ${API_ID} are present above)."

log_info "Simulating DB and Primary Gateway failure: Stopping ${MONGO_SERVICE_NAME} and ${PRIMARY_GW_SERVICE_NAME}..."
echo
docker compose down $MONGO_SERVICE_NAME $PRIMARY_GW_SERVICE_NAME
log_info "Waiting 5s..."
sleep 5

log_info "Starting Secondary Gateway (${SECONDARY_GW_SERVICE_NAME})..."
export APIM_REGISTRY=${APIM_REGISTRY} && export APIM_VERSION=${APIM_VERSION} && docker compose up -d ${SECONDARY_GW_SERVICE_NAME}

log_info "Waiting ${WAIT_TIME_SECONDARY}s for Secondary Gateway to start and sync from Redis..."
echo
sleep $WAIT_TIME_SECONDARY

log_info "Verifying Secondary Gateway started..."
docker compose ps --filter "name=${SECONDARY_GW_CONTAINER_NAME}" --filter "status=running" | grep -q $SECONDARY_GW_CONTAINER_NAME || log_error "${SECONDARY_GW_CONTAINER_NAME} failed to start."
log_success "Secondary Gateway (${SECONDARY_GW_CONTAINER_NAME}) is running."
...
echo
log_success "DSP Sync Without DB Test Scenario Completed Successfully!"
echo
