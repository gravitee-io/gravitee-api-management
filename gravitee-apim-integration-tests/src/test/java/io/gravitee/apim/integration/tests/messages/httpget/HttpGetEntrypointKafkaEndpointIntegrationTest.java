...
class HttpGetEntrypointKafkaEndpointIntegrationTest extends AbstractKafkaEndpointIntegrationTest {
    ...
    void should_receive_all_messages(HttpClient client, Vertx vertx) {
        ...
        Single.fromCallable(() -> getKafkaProducer(vertx))
                .flatMapCompletable(producer -> publishMessage(producer, "message1")
        ...
        client.rxRequest(HttpMethod.GET, "/test").flatMap(request -> {
            request.putHeader(HttpHeaderNames.ACCEPT.toString(), MediaType.APPLICATION_JSON);
            return request.send();
        }).flatMap(response -> {
            assertThat(response.statusCode()).isEqualTo(200);
            return response.body();
        }).test().awaitDone(30, TimeUnit.SECONDS).assertValue(body -> {
    ...
    @DeployApi({"/apis/v4/messages/http-get-entrypoint-kafka-endpoint-at-least-once.json",
    ...
    void should_receive_empty_items_while_requesting_recent_cursor_with_specific_qos(Qos qos, HttpClient client, Vertx vertx) {
        ...
        client.rxRequest(HttpMethod.GET, "/test-"+qos.getLabel() + "?cursor=dGVzdC10b3BpY0AwIzI=").flatMap(request -> {
                    request.putHeader(HttpHeaderNames.ACCEPT.toString(), MediaType.APPLICATION_JSON);
                    return request.send();
                }).flatMap(response -> {
                    assertThat(response.statusCode()).isEqualTo(200);
                    return response.body();
                }).test().awaitDone(30, TimeUnit.SECONDS)
                .assertValue(body -> {
                    final JsonObject jsonResponse = new JsonObject(body.toString());
                    final JsonArray items = jsonResponse.getJsonArray("items");
                    assertThat(items).isEmpty();
