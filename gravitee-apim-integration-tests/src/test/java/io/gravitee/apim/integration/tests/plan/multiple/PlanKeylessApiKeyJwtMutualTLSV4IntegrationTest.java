...
import io.gravitee.apim.integration.tests.plan.jwt.PlanJwtV4IntegrationTest;
 ...
 */
public class PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest {

    public static void configureApi(ReactableApi<?> api, Class<?> definitionClass) {
        final Api apiDefinition = (Api) api.getDefinition();
        configurePlans(apiDefinition, Set.of("key-less", "api-key", "jwt", "mtls"));
    }
    ...
    public class SelectKeylessTest extends PlanKeylessV4IntegrationTest {
        ...
        public void configureApi(ReactableApi<?> api, Class<?> definitionClass) {
            if (isV4Api(definitionClass)) {
                PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configureApi(api, definitionClass);
            }
        }

        @Override
        public void configureEntrypoints(Map<String, EntrypointConnectorPlugin<?, ?>> entrypoints) {
            PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configureEntrypoints(entrypoints);
        }

        @Override
        public void configureEndpoints(Map<String, EndpointConnectorPlugin<?, ?>> endpoints) {
            PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configureEndpoints(endpoints);
        }

        @Override
        public void configureReactors(Set<ReactorPlugin<? extends ReactorFactory<?>>> reactors) {
            PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configureReactors(reactors);
        }

        @Override
        public void configurePolicies(Map<String, PolicyPlugin> policies) {
            PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configurePolicies(policies);
        }

        @Override
        protected Stream<Arguments> provideApis() {
            return PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.provideApis();
        }

        @SneakyThrows
        @Override
        protected void configureGateway(GatewayConfigurationBuilder config) {
            PlanKeylessApiKeyJwtMutualTLSV4IntegrationTest.configureGateway(config);
        }
    ...
    @GatewayTest
    @DeployApi(value = { "/apis/plan/v4-proxy-api.json", "/apis/plan/v4-message-api.json" })
    public class SelectJwtTest extends PlanJwtV4IntegrationTest {
