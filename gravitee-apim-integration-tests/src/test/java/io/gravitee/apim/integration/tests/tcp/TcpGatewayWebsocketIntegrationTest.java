...
class TcpGatewayWebsocketIntegrationTest extends AbstractWebsocketGatewayTest {
    ...
    protected void configureWebSocketClient(
        ...
        // we are doing a websocket anyway, so we use the TCP port
        options.setDefaultHost("localhost").setDefaultPort(gatewayConfig.tcpPort()).setSsl(true).setVerifyHost(false).setTrustAll(true);
    }

    @Test
    @DeployApi({ "/apis/v4/tcp/api.json" })
    void should_call_websocket_backend_via_TCP_port(VertxTestContext testContext, WebSocketClient webSocketClient) {
