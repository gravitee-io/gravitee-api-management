...
describe('AgentChatComponent', () => {
  ...
  const agentTurn = (text: string, complete: boolean): ChatTurn => ({ id: 't1', role: 'agent', text, complete });
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [{ provide: AgentChatStore, useValue: { turns, streaming, error, send, resetFor: jest.fn() } }],
    }).compileComponents();
  ...
  it('names the agent and the application whose key is used', async () => {
    const header = await harness.headerText();
  ...
  it('invites the viewer to start before anything has been said', async () => {
    expect(await harness.emptyStateText()).toContain('Incident Commander');
  });
  ...
  it('sends what was typed and clears the composer', async () => {
    ...
    expect(await harness.composerValue()).toBe('');
  });
  ...
  it('does not send on shift+enter, which is how a question gets a second line', async () => {
    ...
    expect(await harness.composerValue()).toBe('first line');
  });
  ...
  it('renders a streaming answer as plain text, so half-formed markdown does not flicker', async () => {
    ...
    expect(await harness.plainTurnTexts()).toEqual(['**bol']);
  });
  ...
  it('renders a question as plain text, never as markdown', async () => {
    turns.set([{ id: 'u1', role: 'user', text: '**not markdown**', complete: true }]);
    fixture.detectChanges();

    expect(await harness.hasMarkdownViewer()).toBe(false);
    expect(await harness.plainTurnTexts()).toEqual(['**not markdown**']);
  });
  ...
  it('shows a failure', async () => {
    ...
    expect(await harness.errorText()).toContain('401');
  });

  it('shows no failure when there is none', async () => {
    expect(await harness.errorText()).toBeNull();
  });
});
