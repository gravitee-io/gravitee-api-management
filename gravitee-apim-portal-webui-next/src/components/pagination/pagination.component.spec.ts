...
describe('PaginationComponent', () => {
  ...
  describe('Ellipsis page number display', () => {
    it('should show trailing ellipsis on first page', async () => {
      await init(1, 100);

      expect(component.pageNumbers()).toEqual([1, 2, 3, 4, 5, 'ellipsis', 10]);
    });

    it('should show both ellipses on middle page', async () => {
      await init(5, 100);

      expect(component.pageNumbers()).toEqual([1, 'ellipsis', 3, 4, 5, 6, 7, 'ellipsis', 10]);
    });

    it('should show leading ellipsis on last page', async () => {
      await init(10, 100);

      expect(component.pageNumbers()).toEqual([1, 'ellipsis', 6, 7, 8, 9, 10]);
    });
  });
  ...
  describe('goToPage bounds', () => {
    it('should bound page to max when exceeding total', async () => {
      await init(1, 30);
    ...
    it('should bound page to 1 when below minimum', async () => {
      await init(2, 30);
    ...
    it('should not emit when going to current page', async () => {
      await init(2, 30);
  ...
  describe('goToPreviousPage guard', () => {
    it('should not emit when already on first page', async () => {
      await init(1, 30);
