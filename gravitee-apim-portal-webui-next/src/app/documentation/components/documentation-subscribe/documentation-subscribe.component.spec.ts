...
describe('DocumentationSubscribeComponent', () => {
  ...
  it('should navigate to dashboard subscriptions when redirectFn is called', async () => {
    await init();
    component.redirectFn('sub-123');
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/dashboard', 'subscriptions', 'sub-123']);
  });
});
