    ...
    describe('Step 3 -- Checkout', () => {
      describe('When comment is required', () => {
        ...
        it('should allow subscribe without comment', async () => {
          ...
          expectPostCreateSubscription({ plan: API_KEY_PLAN_ID_COMMENT_REQUIRED, application: 'app-id' });
          expectPostCreateSubscription({
      ...
      describe('When a subscription form is enabled', () => {
        ...
        it('should send metadata from the form values', async () => {
          const store = fixture.debugElement.injector.get<GmdFormStateStore>(GMD_FORM_STATE_STORE);
          store.updateField({
            id: 'field-1',
            fieldKey: 'company',
          ...
          expectPostCreateSubscription({ plan: API_KEY_PLAN_ID, application: 'app-id', metadata: { company: 'Acme' } });
        });
      });
