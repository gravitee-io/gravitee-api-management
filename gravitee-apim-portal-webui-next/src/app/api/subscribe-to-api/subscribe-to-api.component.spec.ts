...
describe('SubscribeToApiComponent', () => {
  ...
  const init = async (sharedApiKeyModeEnabled: boolean, api: Api = API) => {
    ...
    httpTestingController.expectOne(`${TESTING_BASE_URL}/apis/${API_ID}/plans?size=-1`).flush({
      data: [
        ...
        fakePlan({ id: PUSH_PLAN_ID, security: null, comment_required: true, general_conditions: undefined, mode: 'PUSH' }),
      ],
    });
    fixture.detectChanges();
  };
  ...
  describe('User subscibes to Push plan', () => {
    beforeEach(async () => {});
    describe('xx', () => {
      it('should be able to go to form', async () => {
        await init(true);
