...
describe('TimeSeriesChartComponent', () => {
  ...
  const setChartData = (data: TimeSeriesResponse, type: TimeSeriesType = 'time-series-bar') => {
  ...
  describe('as bar chart', () => {
    ...
    it('should handle nested buckets structure', () => {
      const data = createTimeSeriesResponse([
        createNestedBucket('2025-10-07T06:00:00Z', { '100-199': 50, '200-299': 30 }),
        createNestedBucket('2025-10-08T06:00:00Z', { '100-199': 60, '200-299': 40 }),
      ]);
      setChartData(data, 'time-series-bar');
  ...
  describe('as line chart', () => {
    it('should create', () => {
      const data = createTimeSeriesResponse([createMeasureBucket('2025-10-07T06:00:00Z', 100)]);
      setChartData(data, 'time-series-line');
      expect(component).toBeTruthy();
    });
    ...
    it('should handle empty metrics list', () => {
      const emptyData: TimeSeriesResponse = { metrics: [], buckets: [] };
      setChartData(emptyData, 'time-series-line');
    ...
    it('should apply line-specific dataset properties', () => {
      const data = createTimeSeriesResponse([createMeasureBucket('2025-10-07T06:00:00Z', 100)]);
      setChartData(data, 'time-series-line');

      const result = component.dataFormatted();
      const lineDataset = result.datasets[0] as ChartDataset<'line', number[]>;
      expect(lineDataset.tension).toBe(0.4);
      expect(lineDataset.fill).toBe('start');
      expect(lineDataset.borderWidth).toBe(1);
    });

    it('should not apply line-specific properties for bar type', () => {
      const data = createTimeSeriesResponse([createMeasureBucket('2025-10-07T06:00:00Z', 100)]);
      setChartData(data, 'time-series-bar');

      const result = component.dataFormatted();
      const barDataset = result.datasets[0] as ChartDataset<'line', number[]>;
      expect(barDataset.tension).toBeUndefined();
      expect(barDataset.fill).toBeUndefined();
    });
  });
});
