...
import { PortalNavigationItem, PortalNavigationItemType } from '../../entities/portalNavigationItem';
import { EmptyStateComponent } from '../empty-state/empty-state.component';
import { GioPermissionModule } from '../gio-permission/gio-permission.module';
  ...
  onDrop(event: CdkDragDrop<SectionNode[]>) {
    ...
    if (currentIndex === 0) {
      ...
      if (currentIndex >= visibleNodes.length) {
        ...
        newOrder = (lastNode?.data?.order ?? 0) + 1;
      } else {
        const replacedItem = visibleNodes[currentIndex];
        ...
        newOrder = isMovingDown && isParentChange ? (replacedItem?.data?.order ?? 0) + 1 : (replacedItem?.data?.order ?? 0);
      }
