...
import { PortalPageContentType } from '../../entities/portalPageContent';
import { SnackBarService } from '../../services/snack-bar.service';
import { GioPermissionModule } from '../../components/gio-permission/gio-permission.module';
import { PortalNavigationItemService } from '../../services/portal-navigation-item.service';
...
import { HasUnsavedChanges } from '../../utils/has-unsaved-changes.guard';
import { confirmDiscardChanges, normalizeContent } from '../../utils/content.util';
import { PortalNavigationItemIconPipe } from '../../components/icon/portal-navigation-item-icon.pipe';
...
export class PortalNavigationItemsComponent implements HasUnsavedChanges {
  ...
  readonly publishDisabled: Signal<boolean> = computed(() => {
    ...
    if (!selectedNavigationItem?.data?.parentId) return false;
  ...
  hasUnsavedChanges() {
    if (this.isLoadingPageContent()) return false;
  ...
  onNodeMenuAction(event: NodeMenuActionEvent) {
    this.checkUnsavedChangesAndRun(() => {
      switch (event.action) {
        ...
        case 'publish':
        case 'unpublish':
          if (event.node.data) this.handlePublishToggle(event.node.data);
          break;
        default:
          ...
          if (event.node.data) {
            this.manageSection(
              ...
              event.action,
              'TOP_NAVBAR',
              this.mapSelectedNavItemToNode(event.node.data.parentId, this.menuLinks())?.data ?? undefined,
              event.node.data,
            );
          }
          break;
  ...
  private setupPageContentSubscription(): void {
    toObservable(this.selectedNavigationItem)
      ...
      .subscribe(result => {
        this.isLoadingPageContent.set(false);
        if (result.success) {
          this.currentPageContentType.set(result.type ?? null);
  ...
  private createApisInOrder(parentId: string | undefined, apiIds: string[]): Observable<string | null> {
    ...
    if (!Array.isArray(apiIds) || apiIds.length === 0) return of(null);
  ...
  protected onSave() {
    ...
    if (navItem && navItem.type === 'PAGE') {
      const pageId = (navItem as PortalNavigationPage).portalPageContentId;
  ...
  onEdit() {
    this.checkUnsavedChangesAndRun(() => {
      ...
      const parentItem = this.mapSelectedNavItemToNode(navItem.parentId, this.menuLinks())?.data ?? undefined;
      this.manageSection(navItem.type, 'edit', navItem.area, parentItem, navItem);
    });
  }

  onPublishToggle() {
    const data = this.selectedNavigationItem()?.data;
    if (data) this.handlePublishToggle(data);
  }
  ...
  onNodeMoved($event: NodeMovedEvent) {
    ...
    if (!node.data) return;
    const navData = node.data;
    if (!this.hasUnsavedChanges()) {
      this.updateItemOrderAndRefreshList(newParentId, newOrder, navData).pipe(takeUntilDestroyed(this.destroyRef)).subscribe();
    } else {
      confirmDiscardChanges(this.matDialog)
        .pipe(
          filter(confirmed => !!confirmed),
          switchMap(() => this.updateItemOrderAndRefreshList(newParentId, newOrder, navData)),
          takeUntilDestroyed(this.destroyRef),
        )
        .subscribe();
    }
...
export function findFirstAvailablePage(
  ...
  function search(currentFolder: PortalNavigationItem | null): PortalNavigationItem | null {
    ...
    for (const element of sortedChildren) {
      if (element.type === 'PAGE') return element;
