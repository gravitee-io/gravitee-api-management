...
import { SubscriptionForm } from '../../entities';
...
import { normalizeContent } from '../../utils/content.util';
...
export class SubscriptionFormComponent implements HasUnsavedChanges {
  ...
  private readonly store: any = inject(GMD_FORM_STATE_STORE);
  ...
  updateSubscriptionForm(): void {
    ...
    this.subscriptionFormService
      .updateSubscriptionForm(form.id, { gmdContent: this.contentControl.value })
      .pipe(
        tap(updatedForm => {
  ...
  private handleToggleChange(enabled: boolean): void {
    ...
    this.matDialog
      ...
      .afterClosed()
      .pipe(
        filter((confirmed): confirmed is true => {
