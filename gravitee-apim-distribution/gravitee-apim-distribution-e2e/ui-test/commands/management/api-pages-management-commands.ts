...
export function getPages(auth: BasicAuthentication, apiId: string, params = new ApiPagesQueryParams()) {
  return cy.request({
    method: 'GET',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/apis/${apiId}/pages`,
    auth,
...
export function getPage(auth: BasicAuthentication, apiId: string, pageId: string) {
  return cy.request({
    method: 'GET',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/apis/${apiId}/pages/${pageId}`,
    auth,
    failOnStatusCode: false,
  });
}
