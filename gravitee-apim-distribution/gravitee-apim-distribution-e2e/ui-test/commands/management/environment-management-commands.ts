...
export function createGroup(auth: BasicAuthentication, body: Group) {
  return cy.request({
    method: 'POST',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/configuration/groups`,
    auth,
...
export function getGroup(auth: BasicAuthentication, groupId: string) {
  return cy.request({
    method: 'GET',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/configuration/groups/${groupId}`,
    auth,
    failOnStatusCode: false,
  });
}
