...
export function getCurrentUser(auth: BasicAuthentication) {
  return cy.request({
    method: 'GET',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/user`,
    auth,
...
export function createUser(auth: BasicAuthentication, body: ApiUser) {
  return cy.request({
    method: 'POST',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/users`,
    body,
    auth,
    failOnStatusCode: false,
  });
...
export function deleteUser(auth: BasicAuthentication, userId: string) {
  return cy.request({
    method: 'DELETE',
    url: `${Cypress.expose('managementApi')}${Cypress.expose('defaultOrgEnv')}/users/${userId}`,
    auth,
    failOnStatusCode: false,
  });
}
