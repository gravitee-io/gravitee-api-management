...
import { ADMIN_USER, API_USER, BasicAuthentication } from '@gravitee/utils/configuration';
import { ADMIN_AUTH_FILE, API_PUBLISHER_AUTH_FILE, MANAGEMENT_BASE_URL } from '@utils/config';
 ...
 * then saves them for other projects to reuse via `storageState`.
 *
 ...
 */
async function saveSignedInState(page: Page, user: BasicAuthentication, authFile: string): Promise<void> {
  ...
  if (!response.ok()) {
    throw new Error(`Login for ${user.username} failed with status ${response.status()}`);
  }

  const cookies = await page.context().cookies();
  if (!cookies.some((cookie) => cookie.name === AUTH_COOKIE)) {
    throw new Error(
      `Login for ${user.username} returned ${response.status()} but no ${AUTH_COOKIE} cookie was set. ` +
...
setup('authenticate as admin', async ({ page }) => {
  await saveSignedInState(page, ADMIN_USER, ADMIN_AUTH_FILE);
});

setup('authenticate as API publisher', async ({ page }) => {
  await saveSignedInState(page, API_USER, API_PUBLISHER_AUTH_FILE);
});
