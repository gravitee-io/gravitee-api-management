...
describeIfClientGatewaySupportingApiProduct('API Product subscription-focused e2e', () => {
  ...
  describe('E3 - Two apps on same product plan (isolation)', () => {
    ...
    beforeAll(async () => {
      const createdApp1 = await applicationsResourceAsAdmin.createApplication({
        orgId,
        envId,
        newApplicationEntity: ApplicationsFaker.newApplication({
      ...
      const app1Id = createdApp1.id;

      const createdApp2 = await applicationsResourceAsAdmin.createApplication({
        orgId,
        envId,
        newApplicationEntity: ApplicationsFaker.newApplication({
