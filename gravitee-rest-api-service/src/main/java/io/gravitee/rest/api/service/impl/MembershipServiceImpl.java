...
public class MembershipServiceImpl extends AbstractService implements MembershipService {
    ...
    public Set<MemberEntity> getMembersByReferencesAndRole(MembershipReferenceType referenceType, List<String> referencesId, String role) {
        try {
            ...
            throw new TechnicalManagementException(
                "An error occurs while trying to get members for " + referenceType + " " + referencesId,
                ex
            );
        }
    }
        ...
        if (poRole.isPresent()) {
            try {
                Optional<io.gravitee.repository.management.model.Membership> poMember = membershipRepository
                    .findByReferenceAndRoleId(convert(referenceType), referenceId, poRole.get().getId())
                    .stream()
                    .findFirst();
    ...
    @Override
    public MemberEntity updateRoleToMemberOnReferenceBySource(
        MembershipReference reference,
        MembershipMember member,
        MembershipRole role,
        String source
