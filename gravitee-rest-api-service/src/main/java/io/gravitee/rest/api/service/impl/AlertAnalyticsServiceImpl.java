...
public class AlertAnalyticsServiceImpl implements AlertAnalyticsService {
    ...
    public AlertAnalyticsEntity findByReference(AlertReferenceType referenceType, String referenceId, AlertAnalyticsQuery analyticsQuery) {
        try {
            ...
            Map<AlertTrigger, HashSet<AlertEvent>> eventsByAlert = triggersById.values().stream()
                    .map(trigger -> alertEventRepository.search(
            ...
            // by severity
            Map<String, Integer> bySeverity = eventsByAlert.entrySet().stream().collect(groupingBy(
            ...
            List<AlertAnalyticsEntity.AlertTriggerAnalytics> alerts = eventsByAlert.entrySet()
                    .stream()
                    .sorted(Map.Entry.<AlertTrigger, HashSet<AlertEvent>>comparingByKey((a1, a2) -> compareSeverity().compare(a1.getSeverity(), a2.getSeverity()))
            ...
            final String message = "An error occurs while trying to list alerts analytics by reference " + referenceType
                    + '/' + referenceId;
