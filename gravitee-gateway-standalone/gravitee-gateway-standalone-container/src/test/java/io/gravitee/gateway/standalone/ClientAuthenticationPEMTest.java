...
public class ClientAuthenticationPEMTest extends AbstractGatewayTest {
    ...
    public void simple_request_client_auth() throws Exception {
        ...
        response = request.execute();
        returnResponse = response.returnResponse();
        assertEquals("trustAll = true, with truststore => 200", HttpStatus.SC_OK, returnResponse.getStatusLine().getStatusCode());

        // Fourth call is calling an endpoint where trustAll = true, without truststore => 200
        request = Request.Get("http://localhost:8082/test/my_team");
        response = request.execute();
        returnResponse = response.returnResponse();
        assertEquals("trustAll = true, without truststore => 200", HttpStatus.SC_OK, returnResponse.getStatusLine().getStatusCode());
