...
public class ClientAuthenticationPKCS12Test extends AbstractGatewayTest {
    ...
    public void simple_request_client_auth() throws Exception {
        ...
        assertEquals("trustAll = false, with keystore and without truststore => 200", HttpStatus.SC_BAD_GATEWAY, returnResponse.getStatusLine().getStatusCode());

        // Third call is calling an endpoint where trustAll = false, with truststore and keystore => 200
        request = Request.Get("http://localhost:8082/test/my_team");
        response = request.execute();
        returnResponse = response.returnResponse();
        assertEquals("trustAll = false, with truststore and keystore => 200", HttpStatus.SC_OK, returnResponse.getStatusLine().getStatusCode());

        // Fourth call is calling an endpoint where trustAll = true, with keystore and without truststore => 200
        request = Request.Get("http://localhost:8082/test/my_team");
        response = request.execute();
        returnResponse = response.returnResponse();
        assertEquals("trustAll = true,with keystore and  without truststore => 200", HttpStatus.SC_OK, returnResponse.getStatusLine().getStatusCode());
