...
rem uncomment the below line to set JAVA_HOME manually
...
for /f %%i in ('dir ..\lib\gravitee-gateway-standalone-bootstrap*.jar /s /b') do set runjar=%%i
...
%JAVA% %JAVA_OPTS% -javaagent:%GRAVITEE_HOME%/boot/jetty-alpn-agent-2.0.10.jar -cp %GRAVITEE_BOOT_CLASSPATH% -Dgravitee.home=%GRAVITEE_HOME% io.gravitee.gateway.standalone.boostrap.Bootstrap "%*"
