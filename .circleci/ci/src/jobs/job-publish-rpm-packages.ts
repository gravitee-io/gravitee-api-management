...
`;
    ...
    if (environment.isDryRun) {
      cmd += `echo "This is just a DRY RUN, no RPMs will be published"`;
    } else {
      cmd += `docker run --rm -v "\${GIT_GRAVITEE_PACKAGES_REPO}/apim/3.x:/packages" -e PACKAGECLOUD_TOKEN=\${GIO_PACKAGECLOUD_TOKEN} digitalocean/packagecloud push --yes --skip-errors --verbose graviteeio/rpms/el/7 /packages/*.rpm`;
    }
