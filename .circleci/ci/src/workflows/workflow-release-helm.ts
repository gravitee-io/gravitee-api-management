...
export class ReleaseHelmWorkflow {
  ...
  static create(dynamicConfig: Config, environment: CircleCIEnvironment) {
    const testApimChartsJob = TestApimChartsJob.create(dynamicConfig, environment);
    ...
    return new Workflow(ReleaseHelmWorkflow.workflowName, [
      new workflow.WorkflowJob(testApimChartsJob, { name: 'APIM - Lint & Test', context: config.jobContext }),
