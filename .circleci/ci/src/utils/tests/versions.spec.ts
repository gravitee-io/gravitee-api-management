...
describe('version', function () {
  ...
  describe('bridgeClientTags', () => {
    const on = (branch: string, pom: string, releasedTags: string[] | undefined): CircleCIEnvironment => ({
    ...
    // a line retires when the release after it ships — so this is what tells a `-latest` tag from a
    // public one.
    const RELEASED = ['4.5.0', '4.6.0', '4.7.0', '4.8.0', '4.9.0', '4.10.0', '4.11.0', '4.12.0', 'core_4.13.0', '4.12.17-hotfix.1'];
    const pom = (line: string) => `./src/utils/tests/resources/pinned/pom-${line.replace('.', '-')}.xml`;
    ...
    it('should reproduce the list master carries', () => {
      expect(bridgeClientTags(on('master', pom('4.13'), RELEASED))).toStrictEqual([
        'master-latest',
        '4.12.x-latest',
        'graviteeio@4.12.0',
        '4.11.x-latest',
    ...
    it('should reproduce the list 4.12.x carries, where every line is still supported', () => {
      expect(bridgeClientTags(on('4.12.x', pom('4.12'), RELEASED))).toStrictEqual([
        '4.12.x-latest',
        'graviteeio@4.12.0',
        '4.11.x-latest',
        'graviteeio@4.11.0',
    ...
    it('should reproduce the list 4.11.x carries, where 4.8 has retired', () => {
      expect(bridgeClientTags(on('4.11.x', pom('4.11'), RELEASED))).toStrictEqual([
        '4.11.x-latest',
        'graviteeio@4.11.0',
        '4.10.x-latest',
        'graviteeio@4.10.0',
    ...
    it('should reproduce the list 4.10.x carries, with two retired lines', () => {
      expect(bridgeClientTags(on('4.10.x', pom('4.10'), RELEASED))).toStrictEqual([
        '4.10.x-latest',
        'graviteeio@4.10.0',
        '4.9.x-latest',
        'graviteeio@4.9.0',
    ...
    // The day 4.13.0 ships, 4.9 retires and master's own list follows without anyone editing it.
    it('should retire a line as soon as the release that pushes it out has shipped', () => {
      const afterRelease = bridgeClientTags(on('4.12.x', pom('4.12'), [...RELEASED, '4.13.0']));
    ...
    it('should take the branch tip alone of a line that has not released yet', () => {
      const duringTheFreeze = bridgeClientTags(on('master', pom('4.14'), RELEASED));
    ...
    it('should refuse to guess across a major', () => {
      expect(() => bridgeClientTags(on('master', pom('5.0'), RELEASED))).toThrow(
    ...
    it('should refuse to run without the released tags', () => {
      expect(() => bridgeClientTags(on('master', pom('4.13'), undefined))).toThrow('bridgeClientTags - the released tags are missing');
    });
  });
