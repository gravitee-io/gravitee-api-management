...
import { WorkflowCoreRelease } from '../workflows';
import { validateGraviteeioVersion } from '../utils';
import { initDynamicConfig } from './config-factory';

export function generateCoreReleaseConfig(environment: CircleCIEnvironment): Config {
  if (environment.tag === undefined) {
    throw new Error('Core release is triggered by a tag, and no tag started this pipeline');
  }
  validateGraviteeioVersion(environment.graviteeioVersion);

  const dynamicConfig = initDynamicConfig();
  const workflow = WorkflowCoreRelease.create(dynamicConfig, environment);
