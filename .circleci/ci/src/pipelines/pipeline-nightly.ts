...
import { NightlyWorkflow } from '../workflows';
import { initDynamicConfig } from './config-factory';

export function generateNightlyConfig(environment: CircleCIEnvironment): Config {
  const dynamicConfig = initDynamicConfig();
  const workflow = NightlyWorkflow.create(dynamicConfig, environment);
