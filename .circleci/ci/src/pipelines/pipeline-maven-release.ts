...
import { WorkflowMavenRelease } from '../workflows';
import { validateGraviteeioVersion } from '../utils';
import { initDynamicConfig } from './config-factory';

export function generateMavenReleaseConfig(environment: CircleCIEnvironment): Config {
  if (environment.isDryRun) {
    throw new Error('Dry Run - Maven release is deactivated if dry run is true');
  }
  validateGraviteeioVersion(environment.graviteeioVersion);

  const dynamicConfig = initDynamicConfig();
  const workflow = WorkflowMavenRelease.create(dynamicConfig, environment);
