...
public class ApplicationSubscriptionsResourceTest extends AbstractResourceTest {
    ...
    public void shouldRenewApiKeyForApplicationSubscription() {
        ...
        assertEquals(CREATED_201, response.getStatus());
        assertEquals(envTarget().path(APPLICATION).path("subscriptions").path(SUBSCRIPTION).path("keys").path(NEW_APIKEY).getUri().toString(), response.getHeaders().getFirst(HttpHeaders.LOCATION));
    }
}
