...
public class GroupsResourceTest extends AbstractResourceTest {
    ...
    public void shouldCreate() {
        ...
        final Response response = target().request().post(Entity.json(newGroupEntity));
        assertEquals(HttpStatusCode.CREATED_201, response.getStatus());
        assertEquals(target().path("my-group-id").getUri().toString(), response.getHeaders().getFirst(HttpHeaders.LOCATION));
    }
}
