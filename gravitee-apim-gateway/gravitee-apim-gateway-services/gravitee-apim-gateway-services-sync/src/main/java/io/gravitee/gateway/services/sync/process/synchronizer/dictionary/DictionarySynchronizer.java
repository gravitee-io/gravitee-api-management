 ...
 */
package io.gravitee.gateway.services.sync.process.synchronizer.dictionary;
...
import io.gravitee.gateway.services.sync.process.deployer.DictionaryDeployer;
...
public class DictionarySynchronizer implements Synchronizer {
    ...
    private Flowable<DictionaryDeployable> prepareForUndeployment(final Flowable<Event> eventsByType) {
        return eventsByType
            ...
            .map(apiId -> DictionaryDeployable.builder().id(apiId).syncAction(SyncAction.UNDEPLOY).build());
    }
