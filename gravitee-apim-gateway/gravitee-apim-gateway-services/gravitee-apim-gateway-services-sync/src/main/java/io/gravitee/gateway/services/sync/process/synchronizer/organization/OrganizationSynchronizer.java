 ...
 */
package io.gravitee.gateway.services.sync.process.synchronizer.organization;
...
import io.gravitee.gateway.services.sync.process.deployer.OrganizationDeployer;
...
public class OrganizationSynchronizer implements Synchronizer {
    ...
    private Flowable<OrganizationDeployable> prepareForDeployment(final Flowable<Event> eventsByType) {
        return eventsByType
            ...
            .map(organization -> OrganizationDeployable.builder().id(organization.getId()).organization(organization).build())
