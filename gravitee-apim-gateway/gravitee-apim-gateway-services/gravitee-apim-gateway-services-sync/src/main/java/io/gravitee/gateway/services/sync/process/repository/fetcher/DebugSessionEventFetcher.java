...
/**
 * Fetches the debug session events published since the last sync.
 *
 * Unlike {@link DebugEventFetcher}, these are not addressed to one gateway: a
 * session applies to every node serving the API, so each one picks the events up
 * on its own.
 */
@RequiredArgsConstructor
public class DebugSessionEventFetcher {
