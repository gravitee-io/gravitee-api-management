...
public class RepositoryAuthzAppender implements AuthzAppender {
    ...
    public Single<List<ApiReactorDeployable>> appends(
        ...
        return stageResourcePolicies(allEntityIds, environments, stagedPolicies)
    ...
    private List<Event> lookupEvents(Set<String> environments) {
        ...
        if (snapshot != null && snapshot.matches(environments) && snapshot.isFresh(now)) {
        ...
        Set<String> envKey = environments == null ? Set.of() : Set.copyOf(environments);
        cachedEvents.set(new CachedEvents(envKey, safe, now));
        return safe;
    }
    ...
    private record CachedEvents(Set<String> environments, List<Event> events, Instant timestamp) {
        ...
        boolean isFresh(Instant now) {
            return Duration.between(timestamp, now).compareTo(EVENT_CACHE_TTL) < 0;
        }
    }
}
