...
public class AccessPointSynchronizerTest {
    ...
    class NoEventTest {
        ...
        void should_only_fetch_created_status_if_not_incremental() throws InterruptedException {
            ...
            cut.synchronize(-1L, to, List.of("env")).test().await().assertComplete();

            verify(accessPointFetcher).fetchLatest(-1L, to, List.of("env"), AccessPointStatus.CREATED);
        }
        ...
        void should_fetch_incremental_events() throws InterruptedException {
            ...
            Long to = Instant.now().toEpochMilli();
            cut.synchronize(from, to, List.of("env")).test().await().assertComplete();

            verify(accessPointFetcher).fetchLatest(from, to, List.of("env"), null);
        }
    ...
    class AccessPointSynchronizationTest {
        ...
        void should_deploy_created_access_points() throws InterruptedException {
            ...
            cut.synchronize(1L, 1L, List.of("env1")).test().await().assertComplete();
