...
class ApiDeployerTest {
    ...
    public void beforeEach() {
        ...
        authzRegistry = new AuthzRegistry();
    ...
    class AuthzRegistryHookTest {
        ...
        void should_unregister_authz_before_apiManager_unregister_on_undeploy() {
            ...
            AuthzRegistry spyRegistry = spy(new AuthzRegistry());
            spyRegistry.register(Set.of("api.api-1"));
            ...
            inOrder.verify(spyRegistry).unregister(Set.of("api.api-1"));
    ...
    class AutoDerivedEntityStagingTest {
        ...
        @Test
        void should_pass_empty_attributes_and_parents_to_engine_for_auto_derived_entities() {
            ReactableApi<?> reactableApi = httpProxyApi("api-1");
            ApiReactorDeployable deployable = ApiReactorDeployable.builder().apiId("api-1").reactableApi(reactableApi).build();

            cut.deploy(deployable).test().assertComplete();

            verify(authzEnginePort).addOrUpdateEntity(eq("Resource::\"api.api-1\""), eq(java.util.Map.of()), eq(java.util.List.of()));
        }
