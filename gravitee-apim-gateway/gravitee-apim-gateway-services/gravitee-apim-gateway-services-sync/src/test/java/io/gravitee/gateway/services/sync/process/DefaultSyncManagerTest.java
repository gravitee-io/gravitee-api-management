...
class DefaultSyncManagerTest {
    ...
    public void beforeEach() {
        ...
        cut = new DefaultSyncManager(router, node, synchronizers, 200, TimeUnit.MILLISECONDS, 1);
    }

    @Test
    void should_synchronize_at_startup() throws Exception {
        Synchronizer synchronizer2 = spy(new FakeSynchronizer(Completable.complete(), 2));
        Synchronizer synchronizer1 = spy(new FakeSynchronizer(Completable.complete(), 1));
        synchronizers.add(synchronizer2);
        synchronizers.add(synchronizer1);
        cut.start();
        InOrder inOrder = inOrder(route, synchronizer1, synchronizer2);
        inOrder.verify(route).handler(argThat(argument -> argument instanceof DefaultSyncHandler));
