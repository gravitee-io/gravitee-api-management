...
class AuthzAppenderTest {
    ...
    private final AuthzEntityMapper entityMapper = new AuthzEntityMapper(objectMapper);
    private final AuthzPolicyMapper policyMapper = new AuthzPolicyMapper(objectMapper);

    private AuthzAppender cut;

    @BeforeEach
    void setUp() {
        cut = new AuthzAppender(extractor, eventLatestRepository, entityMapper, policyMapper, enginePort);
    }
    ...
    @Test
    void null_event_latest_repository_disables_pre_loading_with_warn_and_passes_through() {
        AuthzAppender noRepo = new AuthzAppender(extractor, null, entityMapper, policyMapper, enginePort);
        ApiReactorDeployable deployable = deployableForApi("api-1");

        List<ApiReactorDeployable> result = noRepo.appends(true, List.of(deployable), ENVS).blockingGet();
    ...
    @Test
    void null_entity_mapper_disables_pre_loading_with_warn_and_passes_through() {
        AuthzAppender partialWiring = new AuthzAppender(extractor, eventLatestRepository, null, policyMapper, enginePort);
        ApiReactorDeployable deployable = deployableForApi("api-1");

        List<ApiReactorDeployable> result = partialWiring.appends(true, List.of(deployable), ENVS).blockingGet();
    ...
    @Test
    void null_policy_mapper_disables_pre_loading_with_warn_and_passes_through() {
        AuthzAppender partialWiring = new AuthzAppender(extractor, eventLatestRepository, entityMapper, null, enginePort);
        ApiReactorDeployable deployable = deployableForApi("api-1");
