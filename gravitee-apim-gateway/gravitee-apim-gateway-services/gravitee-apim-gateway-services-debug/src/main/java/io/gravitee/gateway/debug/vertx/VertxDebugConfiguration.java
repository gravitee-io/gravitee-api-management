...
public class VertxDebugConfiguration {
    ...
    public VertxHttpServerFactory debugHttpServerFactory(
        ...
        return new VertxHttpServerFactory(vertx, tlsOptionsRegistry, keyStoreLoaderFactoryRegistry, trustStoreLoaderFactoryRegistry);
    }
    ...
    public VertxHttpServer debugServer(
        ...
        if (environment.getProperty("servers[0].type") != null) {
            ...
            optionsBuilder =
                VertxDebugHttpServerOptions.builder().prefix("servers[0]").environment(environment).tlsOptionsRegistry(tlsOptionsRegistry);
        } else {
            // No server list configured, fallback to single http server configuration.
            optionsBuilder =
                VertxDebugHttpServerOptions.builder().prefix("http").environment(environment).tlsOptionsRegistry(tlsOptionsRegistry);
        }
