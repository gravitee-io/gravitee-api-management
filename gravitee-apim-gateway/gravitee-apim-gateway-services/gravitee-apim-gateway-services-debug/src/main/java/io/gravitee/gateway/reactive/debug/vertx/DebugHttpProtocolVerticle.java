...
/**
 * Debug Vertx Verticle in charge of starting the Vertx http server and to
 * listen and dispatch all incoming debug http requests.
 *
...
public class DebugHttpProtocolVerticle extends AbstractVerticle {
    ...
    public Completable rxStart() {
        ...
        // Note: In Vert.x 5, requestStream() was removed. Use requestHandler() instead.
        // Note: In Vert.x 5, channel() was removed; use
        // channelHandlerContext().channel() instead
        rxHttpServer.connectionHandler(connection -> {
        ...
        rxHttpServer.requestHandler(request -> requestDisposables.add(dispatchRequest(request).subscribe()));
    ...
    public Completable rxStop() {
        return Completable.fromRunnable(requestDisposables::dispose)
            .andThen(
                debugServer
                    .instances()
                    .stream()
