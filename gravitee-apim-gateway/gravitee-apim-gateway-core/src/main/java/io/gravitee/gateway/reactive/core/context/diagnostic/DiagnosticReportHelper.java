...
public class DiagnosticReportHelper {
    ...
    private static String prettifyThrowableName(Throwable t) {
        String displayName = t.getClass().getSimpleName();
        displayName = EXCEPTION_SUFFIX.matcher(displayName).replaceAll("");
        displayName = CAMEL_CASE_SPLIT.matcher(displayName).replaceAll("$1 $2").trim();
        String msg = t.getMessage();
        if (msg == null || msg.isBlank()) {
            return displayName.isBlank() ? "" : displayName;
        }
        return (displayName.isBlank() ? "" : displayName + ": ") + msg;
    }
}
