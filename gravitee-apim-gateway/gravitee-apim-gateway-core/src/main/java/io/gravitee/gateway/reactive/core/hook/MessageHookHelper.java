...
public class MessageHookHelper {
    ...
    public static <T extends HttpHook> Completable hook(
        ...
        if (hooks != null && !hooks.isEmpty()) {
            OnMessagesInterceptor onMessagesInterceptor = extractInterceptor(ctx, executionPhase);
            if (onMessagesInterceptor != null) {
                ...
                if (!messageHooks.isEmpty()) {
                    return Completable.defer(() -> {
                        Completable completable = completableSupplier.get();
                        ...
                        return completable
                            ...
                            .doFinally(() -> onMessagesInterceptor.unregisterMessagesInterceptor(interceptorId));
                    });
                }
            }
        }
    ...
    private static OnMessagesInterceptor extractInterceptor(final HttpExecutionContext ctx, final ExecutionPhase executionPhase) {
        OnMessagesInterceptor onMessagesInterceptor;
        if (executionPhase == ExecutionPhase.MESSAGE_REQUEST) {
            onMessagesInterceptor = (OnMessagesInterceptor) ctx.request();
        } else if (executionPhase == ExecutionPhase.MESSAGE_RESPONSE) {
            onMessagesInterceptor = (OnMessagesInterceptor) ctx.response();
        } else {
    ...
    public static <T extends MessageHook> FlowableTransformer<Message, Message> transformerInterceptor(
        ...
        return upstream ->
            upstream.flatMap(message ->
                executeHooks(componentId, hooks, HookPhase.PRE, ctx, executionPhase, message, null, null)
                    .andThen(Flowable.just(message))
                    .compose(interceptedTransformer)
                    .flatMap(m ->
                        executeHooks(componentId, hooks, HookPhase.POST, ctx, executionPhase, m, null, null).andThen(Flowable.just(m))
                    )
                    .onErrorResumeNext(throwable ->
                        executeHookOnError(componentId, hooks, ctx, executionPhase, message, throwable).andThen(Flowable.error(throwable))
                    )
            );
    }
