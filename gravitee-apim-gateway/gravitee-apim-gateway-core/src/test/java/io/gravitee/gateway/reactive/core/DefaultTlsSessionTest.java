...
class DefaultTlsSessionTest {
    ...
    class SslSessionDelegate {
        ...
        @BeforeEach
        void setUp() {
            cut = new DefaultTlsSession(sslSession, HttpHeaders.create(), null);
        }
    ...
    class ClientAuthCertificateExtraction {
        ...
        @BeforeEach
        void setUp() {
            cut = new DefaultTlsSession(sslSession, HttpHeaders.create().add(SSL_CERT_HEADER, VALID_PEM), SSL_CERT_HEADER);
        }
        ...
        void should_not_get_certificate_from_header_because_no_header_matching_name() {
            cut = new DefaultTlsSession(sslSession, HttpHeaders.create(), SSL_CERT_HEADER);
            when(sslSession.getPeerCertificates()).thenThrow(SSLPeerUnverifiedException.class);
        ...
        void should_not_get_certificate_from_header_because_invalid_pem() {
            cut = new DefaultTlsSession(sslSession, HttpHeaders.create().add(SSL_CERT_HEADER, "invalid"), SSL_CERT_HEADER);
            when(sslSession.getPeerCertificates()).thenThrow(SSLPeerUnverifiedException.class);
            assertThat(cut.getPeerCertificates()).isEmpty();
        }
