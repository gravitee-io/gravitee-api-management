...
import static io.gravitee.gateway.reactive.api.context.ExecutionContext.ATTR_INTERNAL_PREFIX;
...
class FlowChainTest {
    ...
    public void shouldExecuteOnRequest() {
        ...
        verify(ctx, times(1)).setInternalAttribute(eq(ATTR_INTERNAL_PREFIX + "flow." + FLOW_CHAIN_ID), any());
    }
    ...
    public void shouldExecuteAndReusePreviousFlowResolution() {
        ...
        final Flowable<Flow> resolvedFlows = Flowable.just(flow1, flow2);
        when(ctx.getInternalAttribute(eq(ATTR_INTERNAL_PREFIX + "flow." + FLOW_CHAIN_ID))).thenReturn(resolvedFlows);
        ...
        verify(ctx, times(0)).setInternalAttribute(eq(ATTR_INTERNAL_PREFIX + "flow." + FLOW_CHAIN_ID), any());
