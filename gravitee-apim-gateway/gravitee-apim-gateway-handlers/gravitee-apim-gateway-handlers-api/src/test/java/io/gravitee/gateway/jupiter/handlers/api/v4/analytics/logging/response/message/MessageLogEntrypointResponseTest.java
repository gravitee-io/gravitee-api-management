...
class MessageLogEntrypointResponseTest {
    ...
    class LogPayload {
        ...
        void should_log_payload_when_payload_logging_is_enabled() {
            when(loggingContext.entrypointResponseMessagePayload()).thenReturn(true);
        ...
        void should_not_log_payload_when_payload_logging_is_disabled() {
            when(loggingContext.entrypointResponseMessagePayload()).thenReturn(false);
            DefaultMessage message = new DefaultMessage("1");
            MessageLogEntrypointResponse cut = new MessageLogEntrypointResponse(loggingContext, message);
    ...
    class LogHeader {
        ...
        void should_log_payload_when_headers_logging_is_enabled() {
            when(loggingContext.entrypointResponseMessageHeaders()).thenReturn(true);
        ...
        void should_not_log_payload_when_headers_logging_is_disabled() {
            when(loggingContext.entrypointResponseMessageHeaders()).thenReturn(false);
            HttpHeaders headers = HttpHeaders.create();
            DefaultMessage message = DefaultMessage.builder().headers(headers).build();
            MessageLogEntrypointResponse cut = new MessageLogEntrypointResponse(loggingContext, message);
