 ...
 */
package io.gravitee.gateway.jupiter.handlers.api.v4.analytics.logging.request.message;
...
class MessageLogEntrypointRequestTest {
    ...
    class LogPayload {
        ...
        void should_log_payload_when_payload_logging_is_enabled() {
            when(loggingContext.entrypointRequestMessagePayload()).thenReturn(true);
        ...
        void should_not_log_payload_when_payload_logging_is_disabled() {
            when(loggingContext.entrypointRequestMessagePayload()).thenReturn(false);
            DefaultMessage message = new DefaultMessage("1");
            MessageLogEntrypointRequest cut = new MessageLogEntrypointRequest(loggingContext, message);
    ...
    class LogHeader {
        ...
        @Test
        void should_log_payload_when_headers_logging_is_enabled() {
            when(loggingContext.entrypointRequestMessageHeaders()).thenReturn(true);
        ...
        @Test
        void should_not_log_payload_when_headers_logging_is_disabled() {
            when(loggingContext.entrypointRequestMessageHeaders()).thenReturn(false);
            HttpHeaders headers = HttpHeaders.create();
            DefaultMessage message = DefaultMessage.builder().headers(headers).build();
            MessageLogEntrypointRequest cut = new MessageLogEntrypointRequest(loggingContext, message);
