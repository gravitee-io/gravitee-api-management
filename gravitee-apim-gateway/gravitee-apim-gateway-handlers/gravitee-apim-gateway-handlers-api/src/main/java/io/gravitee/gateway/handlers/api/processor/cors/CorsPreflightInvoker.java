...
/**
 * This specific preflight request {@link Invoker} is ran if the gateway has to process policies once preflight
 * processing has been done. This allows to run specific policies even if no upstream is invoked.
 *
...
public class CorsPreflightInvoker implements Invoker {
    ...
    public void invoke(ExecutionContext context, ReadStream<Buffer> stream, Handler<ProxyConnection> connectionHandler) {
        CorsPreflightProxyConnection connection = new CorsPreflightProxyConnection();
    ...
    private static class CorsPreflightProxyConnection implements ProxyConnection {
        ...
        @Override
        public void end() {
            responseHandler.handle(new CorsPreflightProxyResponse());
        }
