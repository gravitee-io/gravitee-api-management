package io.gravitee.gateway.jupiter.handlers.api.deployer;
...
public class ApiDeployer implements Deployer<Api> {
    ...
    public List<String> getPlans(Api api) {
        return api.getDefinition().getPlans().values().stream().map(Plan::getName).collect(Collectors.toList());
    }
