 ...
 */
public class PathParametersExtractor extends AbstractPathParametersExtractor<Api, FlowV2, Plan> {
    ...
    @Override
    protected List<FlowV2> getApiFlows(Api api) {
        return api.getFlows();
    }
    ...
    @Override
    protected List<FlowV2> getPlanFlows(Plan plan) {
        return plan.getFlows();
    }

    @Override
    protected boolean hasPathParam(FlowV2 flow) {
        return PARAM_PATTERN.asPredicate().test(flow.getPath());
    }

    @Override
    protected boolean isEnabled(FlowV2 flow) {
     ...
     */
    @Override
    protected Map<PathParameterHttpMethod, Set<PathParameters>> groupPatternsByMethod(final Stream<FlowV2> flows) {
