...
class TracingPolicyHookTest {
    ...
    void shouldExecutePreHookWithVerbose() {
        ...
        hook.pre(policyId, ctx, ExecutionPhase.REQUEST).test().assertResult();
        verify(tracer).startSpanFrom(any(InternalRequest.class));
        verify(span).addEvent(eq("policy.pre"), anyMap());
    }
    ...
    void shouldExecutePostHookWithTriggerAttributes() {
        ...
        when(ctx.getInternalAttribute("tracing-span-test-policy")).thenReturn(span);
        when(ctx.getInternalAttribute("policy.trigger.condition." + policyId)).thenReturn(condition);
        ...
        verify(span).withAttribute("policy.trigger.condition", condition);
        verify(span).withAttribute("policy.trigger.executed", "true");
    ...
    void shouldExecutePostHookWithVerbose() {
        String policyId = "test-policy";
        Map<String, String> preHeaders = Map.of("header1", "value1");
        Map<String, String> postHeaders = Map.of("header1", "value1", "header2", "value2");

        when(ctx.getInternalAttribute("tracing-span-test-policy")).thenReturn(span);
        when(ctx.getInternalAttribute(InternalContextAttributes.ATTR_INTERNAL_TRACING_VERBOSE_ENABLED)).thenReturn(true);
        when(ctx.getInternalAttribute(String.format("policy.pre.headers.%s", policyId))).thenReturn(preHeaders);
        lenient().when(ctx.getInternalAttribute("policy.trigger.condition." + policyId)).thenReturn(null);
        when(ctx.request()).thenReturn(request);
        when(request.headers()).thenReturn(httpHeaders);
        when(httpHeaders.toSingleValueMap()).thenReturn(postHeaders);
        when(ctx.getAttributeNames()).thenReturn(Set.of());
        when(ctx.getTracer()).thenReturn(tracer);

        hook.post(policyId, ctx, ExecutionPhase.REQUEST).test().assertResult();
        verify(span).addEvent(eq("policy.post"), anyMap());
        verify(ctx).removeInternalAttribute(String.format("policy.pre.headers.%s", policyId));
        verify(ctx).removeInternalAttribute(String.format("policy.pre.attributes.%s", policyId));
        verify(tracer).end(span);
    }
    ...
    void shouldNotAddTriggerConditionWhenBlank() {
        ...
        when(ctx.getInternalAttribute("tracing-span-test-policy")).thenReturn(span);
        when(ctx.getInternalAttribute("policy.trigger.condition." + policyId)).thenReturn("");
        lenient().when(ctx.getInternalAttribute(InternalContextAttributes.ATTR_INTERNAL_TRACING_VERBOSE_ENABLED)).thenReturn(false);
        when(ctx.getTracer()).thenReturn(tracer);
        hook.post(policyId, ctx, ExecutionPhase.REQUEST).test().assertResult();
        verify(span, never()).withAttribute(eq("policy.trigger.condition"), anyString());
        verify(span).withAttribute("policy.trigger.executed", "true");
    }
    ...
    void shouldHandleNullOrEmptyHeaders() {
        Map<String, String> captured = hook.captureHeaders(null);
        assertNotNull(captured);
        assertTrue(captured.isEmpty());
        hook.captureHeaders(new HashMap<>());
        assertNotNull(captured);
        assertTrue(captured.isEmpty());
    }
