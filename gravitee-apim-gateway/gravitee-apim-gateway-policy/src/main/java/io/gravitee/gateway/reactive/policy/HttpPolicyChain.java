...
public class HttpPolicyChain extends AbstractPolicyChain<HttpPolicy> implements Hookable<HttpHook> {
    ...
    public void addHooks(final List<HttpHook> hooks) {
        ...
        hooks.forEach(hook -> {
            if (hook instanceof PolicyHook policyHook) {
                this.policyHooks.add(policyHook);
            } else if (hook instanceof PolicyMessageHook policyMessageHook) {
                this.policyMessageHooks.add(policyMessageHook);
            }
        });
    }
