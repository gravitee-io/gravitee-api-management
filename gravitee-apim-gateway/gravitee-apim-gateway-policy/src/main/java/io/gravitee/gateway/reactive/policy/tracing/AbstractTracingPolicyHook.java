...
public abstract class AbstractTracingPolicyHook extends AbstractTracingHook {
    ...
    protected String getSpanName(final String id, final ExecutionPhase executionPhase) {
        String phase = getPhaseLabel(executionPhase);
        StringBuilder spanNameBuilder = new StringBuilder(phase).append(" ");
    ...
    protected String getPhaseLabel(final ExecutionPhase executionPhase) {
        String phase = null;
        switch (executionPhase) {
            case REQUEST:
            case ASYNC_REQUEST:
                phase = ExecutionPhase.REQUEST.name();
                break;
            case RESPONSE:
            case ASYNC_RESPONSE:
                phase = ExecutionPhase.RESPONSE.name();
                break;
        }
        return phase;
    }
