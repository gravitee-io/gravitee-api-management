...
* Java (JDK 25 — see `.sdkmanrc`)
* Maven
* Docker (the daemon must be running before the default build — see build notes below)
* Node.js 24.x (see `.nvmrc`)
* Yarn 4.1.1 (preferably with NVM)

TIP: The UI projects declare `node: ^24.0` and `packageManager: yarn@4.1.1` in their `package.json` files.
...
* Odd-numbered Node releases (23.x, 25.x) are not supported; use Node 24.x.
