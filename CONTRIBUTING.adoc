...
* Java (JDK >= 17)
* Maven
* Docker (the daemon must be running before the default build — see build notes below)
* Node.js 22.12.x
...
[IMPORTANT]
====
The default build runs repository tests that use https://java.testcontainers.org/[Testcontainers] to start Elasticsearch / MongoDB / etc.
**Docker must be running** before you invoke it; otherwise those tests fail with `Could not find a valid Docker environment`.

If you don't want to start Docker, skip tests with the command shown below.
====
...
1) (Optional) Build backend artifacts — two reactors since the distribution left the product one
   - Supported path: `task build-quick` (builds the engine, then the distribution against it)
   - By hand, both phases are needed:
     `mvn clean install -T 2C -DskipTests=true -Dskip.validation=true`
     `mvn -f gravitee-apim-distribution/pom.xml clean install -Pengine-snapshot -nsu -Dbundle=dev`
   - IMPORTANT: a root build alone no longer produces `target/distribution`. And building the
     distribution without `-Pengine-snapshot` assembles the pinned *released* engine instead of
     your working tree, with no error — run `task which-engine` to see which one you got.
...
[source, bash]
----
# The distribution is a reactor of its own, so this is two invocations: install the engine,
# then assemble against it. -pl paths are relative to the pom passed with -f.
DOCKER_MVN='docker run --rm -u "$(id -u):$(id -g)" -e HOME=/home/maven -e MAVEN_CONFIG=/home/maven/.m2 -v "$HOME/.m2":/home/maven/.m2 -v "$PWD":/workspace -w /workspace maven:3.9.9-eclipse-temurin-21 mvn'

eval $DOCKER_MVN install -DskipTests -Dskip.validation=true

eval $DOCKER_MVN -f gravitee-apim-distribution/pom.xml -Pengine-snapshot -nsu -Dbundle=dev \
  -pl gravitee-apim-distribution-standalone/gravitee-apim-distribution-standalone-rest-api -am \
  package -DskipTests -Dskip.validation=true
----

To refresh the local Management API image used by `docker/quick-setup/mongodb`:

* `docker build -t graviteeio/apim-management-api:latest -f gravitee-apim-distribution/gravitee-apim-distribution-standalone/gravitee-apim-distribution-standalone-rest-api/docker/Dockerfile gravitee-apim-distribution/gravitee-apim-distribution-standalone/gravitee-apim-distribution-standalone-rest-api/target`
...
==== Gamma Console (gravitee-gamma-control-plane-webui)

The Gamma Console is a micro-frontend host application that dynamically loads module plugins via Module Federation.

[[dev-guide-gamma-console]]
===== Prerequisites: Management API (Gamma enabled)

The Management API must expose the Gamma REST API at `/gamma` and load `gamma-module` plugins. By default, `gamma.enabled` is `false` in the standalone distribution config. Set it to `true` in the `gravitee.yml` used by your local API (for a built distribution, typically `${GRAVITEE_HOME}/config/gravitee.yml`; the template lives under `gravitee-apim-distribution/gravitee-apim-distribution-standalone/gravitee-apim-distribution-standalone-rest-api/src/main/resources/config/gravitee.yml`).
...
. Start your usual Management API dependencies (for example MongoDB) and run the Management API on port 8083 (see <<dev-guide-apim-management-api>>) with Gamma enabled as described above.
...
. Optional: when developing a module UI locally, run that module’s dev server and use `DEV_MODULE_ENTRIES` (see below).

===== Starting the Gamma Console

CLI Version::
Run `yarn gamma-console:serve` from the **repository root** to start the Gamma Console on port 4200.
...
. Start the Gamma Console with the `DEV_MODULE_ENTRIES` environment variable pointing to the local module:
+
[source,shell]
----
DEV_MODULE_ENTRIES="apim=http://localhost:3001/mf-manifest.json" yarn nx serve gamma-console
----
...
[source,shell]
----
DEV_MODULE_ENTRIES="apim=http://localhost:3001/mf-manifest.json,platform=http://localhost:3002/mf-manifest.json,billing=http://localhost:3003/mf-manifest.json" yarn nx serve gravitee-gamma-control-plane-webui
----

NOTE: The backend must still be running to serve the `/modules` listing endpoint and other APIs. Only the module **assets** are loaded from the local dev server.
