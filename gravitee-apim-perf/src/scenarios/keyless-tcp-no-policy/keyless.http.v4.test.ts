...
export function setup(): GatewayTestData {
  ...
  // This scenario is run with a secured gateway, meaning we want to call the target with HTTPS
  // For the self-signed keystore, we need to set K6 option insecureTlSVerify=true
  GatewayClient.waitForApiAvailability({ contextPath: contextPath, secured: true });
...
export default (data: GatewayTestData) => {
  // This scenario is run with a secured gateway, meaning we want to call the target with HTTPS
  const gatewayUrl = HttpHelper.changeToHttps(k6Options.apim.gatewayBaseUrl);
  const res = http.get(gatewayUrl + data.waitGateway.contextPath);
  check(res, {
