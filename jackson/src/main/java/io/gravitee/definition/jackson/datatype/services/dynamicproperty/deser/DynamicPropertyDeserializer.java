...
public class DynamicPropertyDeserializer extends ScheduledServiceDeserializer<DynamicPropertyService> {
    ...
    protected void deserialize(DynamicPropertyService service, JsonParser jsonParser, JsonNode node, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        if (service.getProvider() == DynamicPropertyProvider.HTTP) {
            HttpDynamicPropertyProviderConfiguration configuration =
                    configurationNode.traverse(jsonParser.getCodec()).readValueAs(HttpDynamicPropertyProviderConfiguration.class);
