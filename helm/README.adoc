...
|`+api.logging.debug+` |*Deprecated.* Whether to enable API debug logging or not. Use `+api.logback.override+` instead
|`+false+`

|`+api.logging.graviteeLevel+` |*Deprecated.* Logging level for Gravitee classes
|`+DEBUG+`

|`+api.logging.jettyLevel+` |*Deprecated.* Logging level for Jetty classes |`+INFO+`

|`+api.logging.stdout.encoderPattern+` |*Deprecated.* Logback standard output encoder
pattern |`+%d{HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n+`

|`+api.logging.file.enabled+` |*Deprecated.* Whether to enable file logging or not
|`+true+`

|`+api.logging.file.rollingPolicy+` |*Deprecated.* Logback file rolling policy
configuration |`+TimeBasedRollingPolicy+` for 30 days

|`+api.logging.file.encoderPattern+` |*Deprecated.* Logback file encoder pattern
|`+%d{HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n+`

|`+api.logging.additionalLoggers+` |*Deprecated.* List of additional logback loggers. Each logger is defined by a `name` and `level` (TRACE, DEBUG, INFO, WARN, or ERROR)
|`+empty+`

|`+api.logback.override+` |When enabled, uses the content of `+api.logback.content+` as the complete logback.xml. Takes precedence over `+api.logging.debug+`
|`+false+`

|`+api.logback.content+` |Complete logback.xml content to use when `+api.logback.override+` is `+true+`
|A default JSON-formatted logback configuration

|`+api.node.logging.mdc.format+` |MDC key-value format pattern |`+{key}: {value}+`

|`+api.node.logging.mdc.separator+` |MDC entries separator |`+ +` (space)

|`+api.node.logging.mdc.nullValue+` |Value to display when MDC entry is null |`+-+`

|`+api.node.logging.mdc.include+` |List of MDC keys to include in log output |`+[nodeId, envId, apiId, appId]+`

|`+api.node.logging.pattern.overrideLogbackXml+` |Whether to override logback.xml patterns at runtime |`+false+`

|`+api.node.logging.pattern.console+` |Console log pattern when overrideLogbackXml is enabled |`+%d{HH:mm:ss} %-5level %logger{36} [%mdcList] - %msg%n+`

|`+api.node.logging.pattern.file+` |File log pattern when overrideLogbackXml is enabled |`+%d %-5p [%t] %c [%mdcList] : %m%n+`
...
|`+api.user.password.policy.description+` |Your own explanation of the
password policy, carried verbatim by
`+GET /configuration/password-policy+` and empty when unset. Set one
whenever you change the pattern, otherwise the API logs a startup
warning and users see no guidance |`+none+`

|`+api.user.password.policy.pattern+` |Regular expression every local
account password must match. Unset means the API keeps its shipped
default (12 to 128 characters, one digit, one upper case, one lower
case, one special character, no more than 2 consecutive equal
characters) |`+none+`
...
|`+gateway.logging.debug+` |*Deprecated.* Whether to enable Gateway debug logging or
not. Use `+gateway.logback.override+` instead |`+false+`

|`+gateway.logging.additionalLoggers+` |*Deprecated.* List of additional logback loggers. Each logger is defined by a `name` and `level` (TRACE, DEBUG, INFO, WARN, or ERROR)
|`+empty+`
...
|`+gateway.logging.graviteeLevel+` |*Deprecated.* Logging level for Gravitee classes
|`+DEBUG+`

|`+gateway.logging.jettyLevel+` |*Deprecated.* Logging level for Jetty classes
|`+INFO+`

|`+gateway.logging.stdout.encoderPattern+` |*Deprecated.* Logback standard output
encoder pattern
|`+%d{HH:mm:ss.SSS} [%thread] [%X{api}] %-5level %logger{36} - %msg%n+`

|`+gateway.logging.file.enabled+` |*Deprecated.* Whether to enable file logging or not
|`+true+`

|`+gateway.logging.file.rollingPolicy+` |*Deprecated.* Logback file rolling policy
configuration |`+TimeBasedRollingPolicy+` for 30 days

|`+gateway.logging.file.encoderPattern+` |*Deprecated.* Logback file encoder pattern
|`+%d{HH:mm:ss.SSS} [%thread] [%X{api}] %-5level %logger{36} - %msg%n+`

|`+gateway.logback.override+` |When enabled, uses the content of `+gateway.logback.content+` as the complete logback.xml. Takes precedence over `+gateway.logging.debug+`
|`+false+`

|`+gateway.logback.content+` |Complete logback.xml content to use when `+gateway.logback.override+` is `+true+`
|A default JSON-formatted logback configuration with async appenders

|`+gateway.node.logging.mdc.format+` |MDC key-value format pattern |`+{key}: {value}+`

|`+gateway.node.logging.mdc.separator+` |MDC entries separator |`+ +` (space)

|`+gateway.node.logging.mdc.nullValue+` |Value to display when MDC entry is null |`+-+`

|`+gateway.node.logging.mdc.include+` |List of MDC keys to include in log output |`+[nodeId, apiId]+`

|`+gateway.node.logging.pattern.overrideLogbackXml+` |Whether to override logback.xml patterns at runtime |`+false+`

|`+gateway.node.logging.pattern.console+` |Console log pattern when overrideLogbackXml is enabled |`+%d{HH:mm:ss} %-5level %logger{36} [%mdcList] - %msg%n+`

|`+gateway.node.logging.pattern.file+` |File log pattern when overrideLogbackXml is enabled |`+%d %-5p [%t] %c [%mdcList] : %m%n+`
