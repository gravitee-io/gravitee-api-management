...
- Bump the Gravitee node Hazelcast cache and cluster plugins to 9.10.0, to stay consistent with `gravitee-node.version` in the APIM `pom.xml`.
- Render `otel-traces` and `otel-logs` in the Management API `gravitee.yml` with `type: none` when `api.otelTraces.type` / `api.otelLogs.type` are `none`, the default. The chart omitted both blocks in that case, and since the configmap replaces the whole `gravitee.yml` the scopes ended up with no type at all: the Management API then loads no repository for them, not even the no-op one, and the Gamma trace explorer answered every search and trace detail with a 500 (`No qualifying bean of type 'io.gravitee.repository.tracing.api.TracingRepository'`). It now returns empty results until a tracing backend is configured. The `elasticsearch` rendering is unchanged.
...
### 4.12.0
- Bump the Gravitee node Hazelcast cache and cluster plugins to 9.8.0, to stay consistent with `gravitee-node.version` in the APIM `pom.xml`.
