...
- Tokens only: no hex/RGB values and no arbitrary font-size px — use design tokens and CSS/SCSS variables.
...
- Card surfaces stay distinct from the app background, with proper spacing between card elements.
...
- Layout uses containers with centered content where a centered layout is intended.
