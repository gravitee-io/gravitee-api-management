...
Depth behind the "Automation API Sync" rule. Follow this when one of the rule's triggers fires — a Management API v2 change (or a lower-layer change in the v4 definition or repository) that the Automation API must mirror. This guide is the maintained copy of the checklist that also appears in `gravitee-apim-rest-api/AGENTS.md` (hand-written, replaced by a generated file in the next migration step); where they diverge, this guide wins.
