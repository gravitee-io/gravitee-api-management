...
public record AuthzPolicy(
    ...
    @NotNull AuthzPolicyStatus status,
    @NotBlank String environmentId,
    @NotNull Instant createdAt,
    @NotNull Instant updatedAt
) {
    public AuthzPolicy {
        AuthzValidators.validateCtor(AuthzPolicy.class, id, name, kind, entityId, policyText, status, environmentId, createdAt, updatedAt);
        switch (kind) {
            case GLOBAL -> {
                if (entityId != null) {
