 ...
 */
package io.gravitee.gamma.repository.mongodb.upgrade.upgrader.index;
...
public abstract class IndexUpgrader extends IndexMongoUpgrader {
    ...
    public boolean upgrade() {
        ...
        Mono<Boolean> create = template
            ...
            .doOnSubscribe(s -> log.info("Starting creation of index {} on {}", name, collection))
            .doOnSuccess(r -> log.info("Index {} has been created successfully on {}", name, collection))
            .thenReturn(true)
            .onErrorResume(e -> {
                log.error("Unexpected error while creating index {} on {}", name, collection, e);
                return Mono.just(false);
            })
            .cache();

        Flux.interval(Duration.ofSeconds(10))
            .doOnNext(t -> log.info("Index {} on {} is still being created...", name, collection))
