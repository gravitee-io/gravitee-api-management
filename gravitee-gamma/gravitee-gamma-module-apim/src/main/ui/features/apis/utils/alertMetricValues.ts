...
export interface AlertMetricLookups {
    ...
    applications?: AlertMetricValueOption[];
    plans?: AlertMetricValueOption[];
}
...
export function getMetricValueChoices(
    ...
    if (metric.valueSource === 'applications') {
        return lookups.applications ?? [];
    }
    if (metric.valueSource === 'plans') {
        return lookups.plans ?? [];
    }
