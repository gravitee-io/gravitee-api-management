...
export function findHttpStatusCode(value: string | number | undefined): HttpStatusCode | undefined {
    if (value === undefined || value === '') return undefined;
    const asString = String(value);
    return HTTP_STATUS_CODES.find(status => String(status.code) === asString);
}

export function isValidHttpStatusCode(value: string | number | undefined): boolean {
    if (value === undefined || value === '') return false;
    return findHttpStatusCode(value) !== undefined;
}
