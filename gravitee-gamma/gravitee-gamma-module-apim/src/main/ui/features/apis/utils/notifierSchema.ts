...
function adaptNode(node: unknown, depth: number, visited: WeakSet<object>): unknown {
    ...
    if (obj.properties && typeof obj.properties === 'object' && !Array.isArray(obj.properties)) {
        obj.properties = Object.fromEntries(
            Object.entries(obj.properties as Record<string, unknown>).map(([key, value]) => [
                key,
                adaptNode(value, depth + 1, visited),
            ]),
        );
    }
    if (obj.items !== undefined) {
        obj.items = Array.isArray(obj.items) ? obj.items.map(item => adaptNode(item, depth + 1, visited)) : adaptNode(obj.items, depth + 1, visited);
    }
...
function hasMissingRequired(schema: Record<string, unknown>, data: unknown): boolean {
    ...
    if (properties) {
        for (const [key, prop] of Object.entries(properties)) {
            if (prop?.type === 'object') {
                const nested = isPlainObject(obj[key]) ? obj[key] : {};
                if (hasMissingRequired(prop, nested)) {
                    return true;
                }
            }
        }
