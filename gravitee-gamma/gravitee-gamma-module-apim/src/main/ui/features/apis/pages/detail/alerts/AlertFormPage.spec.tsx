...
jest.mock('../../../context/ApiDetailContext', () => ({
    useApiDetailContext: jest.fn(() => ({ permissionsReady: true, api: null, isLoading: false })),
}));
...
it('renders create form with empty name and disabled Create until a rule is selected', () => {
    renderCreatePage();
...
it('enables Create after selecting a rule that needs no extra conditions', async () => {
    ...
    await user.type(screen.getByLabelText(/name/i), 'My runtime alert');
...
it('calls createAlertTrigger with correct payload when form is valid and submitted', async () => {
    ...
    expect(sentData.name).toBe('My runtime alert');
...
it('hides Create button for read-only users', () => {
    mockUseHasPermission.mockImplementation(({ anyOf }: { anyOf: string[] }) =>
        anyOf.includes('api-alert-c') || anyOf.includes('api-alert-u') || anyOf.includes('api-alert-d') ? false : true,
    );
    renderCreatePage();
