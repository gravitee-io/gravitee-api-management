...
function renderUserPermissionsPage(api: object) {
    ...
    return { invalidateQueries, unmount };
}
...
describe.each([
    ...
    it('shows the Manage groups control as soon as the page renders', () => {
    ...
    it('confirms the save, closes the sheet and keeps the added group selected once the page is loaded again', async () => {
        const user = userEvent.setup();
        const { unmount } = renderUserPermissionsPage(api);

        await openManageGroups(user);
        await user.click(screen.getByRole('checkbox', { name: /Internal/ }));
        await user.click(screen.getByRole('button', { name: 'Save' }));

        await waitFor(() => expect(mockNotifySuccess).toHaveBeenCalledWith('Groups updated'));
        await waitFor(() => expect(screen.queryByRole('checkbox', { name: /Partners/ })).not.toBeInTheDocument());

        unmount();
        renderUserPermissionsPage({ ...api, groups: ['group-a', 'group-b'] });
        await openManageGroups(user);
