...
describe('ApisPage', () => {
    ...
    it("renders the table with no API row and a permission empty state instead of 'No APIs found' when the search is refused with 403", () => {
        ...
        expect(screen.queryByText('Total APIs')).toBeNull();
    ...
    it.each<[string, unknown]>([
        ...
        expect(renderedApiRowCount()).toBe(0);
        expect(screen.queryByText('Why add an API proxy?')).toBeNull();
        expect(screen.queryByText('Total APIs')).not.toBeNull();
    });
