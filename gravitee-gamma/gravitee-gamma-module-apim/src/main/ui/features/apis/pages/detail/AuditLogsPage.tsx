...
import { AuditLogsLanding } from './audit/AuditLogsLanding';
...
export function AuditLogsPage() {
    ...
    const eventOptions = useMemo(
        () => [{ value: ALL_EVENTS, label: 'All events' }, ...(eventTypes ?? []).map(e => ({ value: e, label: e }))],
        [eventTypes],
    );

    const handleEventChange = useCallback((v: string) => {
        setSelectedEvent(v === ALL_EVENTS ? '' : v);
        setPage(1);
    }, []);
    ...
    // Show landing only on true zero-state (no filters, no data, not loading)
