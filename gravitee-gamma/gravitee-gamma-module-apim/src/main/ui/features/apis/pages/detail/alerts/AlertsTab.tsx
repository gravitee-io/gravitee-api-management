...
function infoOnlyMessage(): string {
...
export function AlertsTab({
    ...
    selectedRule,
    ruleLabel,
    ruleCategories = getAlertRuleCategoriesForApi(),
}: AlertsTabProps) {
    const lookups = useAlertLookupOptions();
    const conditionRuleId = selectedRule?.id ?? ruleId;
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {ruleCategories.map(category => {
                                            const categoryRules = ALERT_RULES.filter(rule => rule.category === category);
                ...
                <CardContent>
                    ...
                    {!selectedRule && !ruleId ? (
                        ...
                        <fieldset disabled={!canEdit} className="contents">
                            {conditionRuleId === 'REQUEST@METRICS_SIMPLE_CONDITION' && conditions[0] && (
                                <SimpleConditionForm
                            ...
                            {conditionRuleId === 'REQUEST@METRICS_AGGREGATION' && conditions[0] && (
                            ...
                            {conditionRuleId === 'REQUEST@METRICS_RATE' && conditions[0] && (
                                <RateConditionForm
                                    condition={conditions[0]}
                                    metrics={metricsForRule}
                                    projectionMetrics={projectionMetrics}
