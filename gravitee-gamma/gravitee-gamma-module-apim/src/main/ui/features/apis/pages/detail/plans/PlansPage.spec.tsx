...
jest.mock('@gravitee/graphene-core', () => {
    const { cloneElement, createContext, isValidElement, useContext } = jest.requireActual('react');
    ...
    return {
        ...
        DialogClose: function DialogClose({ children }: { children?: ReactNode }) {
            const close = useContext(DialogCloseContext);
            return isValidElement(children) ? cloneElement(children, { onClick: close }) : children;
        },
...
function renderPlansPage(ctx: PlanContext, overrides: { canCreate?: boolean; canRead?: boolean; canUpdate?: boolean } = {}) {
    return render(
...
describe('PlansPage while the plan counts are loading', () => {
    afterEach(() => {
        mockUsePlanStatusCounts.mockReturnValue({ staging: 0, published: 1, deprecated: 0, closed: 0, total: 1, isLoading: false });
    });

    it('shows loading skeletons instead of the plan list or the plans tutorial', () => {
        mockUsePlanStatusCounts.mockReturnValue({ staging: 0, published: 0, deprecated: 0, closed: 0, total: 0, isLoading: true });

        renderPlansPage(API_CTX);

        expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
        expect(screen.queryByTestId('plans-list')).toBeNull();
        expect(screen.queryByTestId('plans-learning')).toBeNull();
    });
});
...
describe('PlansPage allow multi JWT/OAuth2 subscriptions toggle', () => {
    ...
    it('leaves the setting unsaved when the confirmation is dismissed without enabling', async () => {
        givenMultiSubscriptionsAllowed(false);
        renderPlansPage(API_CTX);
    ...
    it('keeps the setting read-only for a user without the plan update permission', async () => {
        givenMultiSubscriptionsAllowed(false);
        renderPlansPage(API_CTX, { canUpdate: false });

        expect(multiSubscriptionsSwitch()).toBeDisabled();
    ...
    it('withholds the setting from an API Product plans page', () => {
        givenMultiSubscriptionsAllowed(false);
        renderPlansPage(API_PRODUCT_CTX);
