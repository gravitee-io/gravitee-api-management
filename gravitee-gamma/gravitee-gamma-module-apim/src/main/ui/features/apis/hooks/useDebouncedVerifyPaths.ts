...
import { verifyPaths } from '../../../services/apis/apis';
import type { ProxyConfig } from '../types/models';
...
function pathsToVerify(proxy: ProxyConfig): Array<{ path: string; host?: string }> {
    if (proxy.enableVirtualHosts) {
        ...
        return rows
            .filter(virtualHost => virtualHost.path?.trim() && virtualHost.host?.trim())
            .map(virtualHost => ({ path: virtualHost.path.trim(), host: virtualHost.host.trim() }));
    }
...
export function useDebouncedVerifyPaths(proxy: ProxyConfig, enabled: boolean) {
    ...
    const allVirtualHostsFilled =
        ...
        (proxy.virtualHosts?.every(virtualHost => Boolean(virtualHost.host?.trim()) && Boolean(virtualHost.path?.trim())) ?? false);
