...
describe('useApiStats', () => {
    ...
    it('warns with the error name when the failed count carries no HTTP status', async () => {
        const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
        ...
        await waitFor(() => expect(result.current.failed.published).toBe(true));
        expect(warn).toHaveBeenCalledWith(expect.stringContaining('published count query failed (TypeError)'), error);
    });

    it('does not count while the environment is not yet ready', () => {
        ...
        expect(result.current.private).toBeNull();
        expect(result.current.published).toBeNull();
    });

    it('reports the wait for the federation gate as loading, not as loaded empty counts', () => {
