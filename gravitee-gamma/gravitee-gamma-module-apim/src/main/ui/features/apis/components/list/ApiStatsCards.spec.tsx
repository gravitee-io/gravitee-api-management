...
const NEVER_FETCHED = { status: 'pending', fetchStatus: 'idle' };
...
/** A query that has started a fetch is 'fetching' and one that finished has left 'pending', so pending-and-idle means no count search was ever started. */
function countQueryStates(queryClient: QueryClient) {
    return queryClient
        .getQueryCache()
        .findAll({ queryKey: [...apiListKeys.all, 'count'] })
...
function renderCards() {
    ...
    return { queryClient, rerenderCards: () => rerender(tree()) };
}
...
describe('ApiStatsCards', () => {
    ...
    it('issues no search while the federation gate is unresolved, then counts proxies alone once it fails closed', async () => {
        ...
        expect(countQueryStates(queryClient)).toEqual([NEVER_FETCHED, NEVER_FETCHED, NEVER_FETCHED]);
