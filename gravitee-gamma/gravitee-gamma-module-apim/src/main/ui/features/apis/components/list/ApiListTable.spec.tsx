...
describe('ApiListTable', () => {
    ...
    it('omits pagination when no page handlers are supplied', () => {
    ...
    it('renders pagination when both page handlers are supplied', () => {
        renderTable({ apis: [makeApi()], totalCount: 1, onPageChange: jest.fn(), onPageSizeChange: jest.fn() });
    ...
    it('renders a permission empty state instead of the no-match message when the user may not list APIs', () => {
        renderTable({ apis: [], isLoading: false, forbidden: true } as Partial<Parameters<typeof ApiListTable>[0]>);
    ...
    describe('SyncStatusBadge', () => {
        ...
        it('shows the em dash when the deployment state is outside the known union', () => {
            ...
            expect(syncCell.textContent).toBe('—');
            expect(within(syncCell).queryByText('In sync')).toBeNull();
            expect(fallbackWarning).toHaveBeenCalledTimes(1);
            expect(fallbackWarning).toHaveBeenCalledWith(expect.any(String), 'ARCHIVED');
