...
import { validateStep } from '../../../utils/schema';
import { buildSteps, defaultSecurityConfig, getTemplateById } from '../../../utils/stepRegistry';
import type { StepConfig } from '../../../utils/stepRegistry';
import type { ApiCreationMode, ApiCreationState, SecurityConfig } from '../types/models';
...
const reducer = (state: ApiCreationStoreState, action: Action): ApiCreationStoreState => {
    switch (action.type) {
        ...
        case 'previousStep': {
            ...
            const idx = state.steps.findIndex(step => step.id === state.activeStepId);
        ...
        case 'updateField': {
            const { [action.path]: _cleared, ...remainingErrors } = state.validationErrors;
            ...
            if (action.path === 'security.type') {
                ...
                const activeStepId = nextSteps.some(step => step.id === state.activeStepId) ? state.activeStepId : nextSteps[0]?.id;
...
export function useApiCreationStore() {
    ...
    const activeStep = useMemo(
        () => (state.activeStepId ? state.steps.find(step => step.id === state.activeStepId) : undefined),
    ...
    const activeStepIndex = useMemo(
        () =>
            state.activeStepId
                ? Math.max(
                      0,
                      state.steps.findIndex(step => step.id === state.activeStepId),
                  )
                : 0,
        [state.activeStepId, state.steps],
    );
