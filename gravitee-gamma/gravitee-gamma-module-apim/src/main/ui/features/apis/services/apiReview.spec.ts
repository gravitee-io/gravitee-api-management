...
describe('apiReview service', () => {
    ...
    it('stops at the first failing rule check without writing the rest', async () => {
        const create = trackHandler('post', `${V1_ENV_BASE}/apis/:apiId/quality-rules`, { message: 'nope' }, 500);
        const update = trackHandler('put', `${V1_ENV_BASE}/apis/:apiId/quality-rules/:ruleId`, {});

        await expect(
            submitApiReview('DEFAULT', 'api-1', {
                decision: 'accept',
                checks: [
                    { qualityRuleId: 'r-1', checked: true, exists: false },
                    { qualityRuleId: 'r-2', checked: false, exists: true },
                ],
            }),
        ).rejects.toBeDefined();
