...
/**
 * Every search is filtered on the module's own set of API types, never a caller-supplied
 * `apiTypes` — so no count or list derived from this search can be widened from the client.
 * The set is the V4 proxy pair (HTTP and TCP), plus `FEDERATED` when the caller has passed
 * the resolved federation gate.
 */
