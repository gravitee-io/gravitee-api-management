...
export interface UseNavigationOptions {
    ...
    /** When set, OpenAPI/AsyncAPI pages use this API for default spec sources in non-portal trees. */
    readonly apiContextId?: string;
}
...
export function useNavigation(
    ...
    const { slug, mode = 'edit', getPagePath, onNavigate, portal, onPortalChange, apiContextId } = options;
    ...
    const addNavItem = useCallback(async (
        ...
        const resolvedApiId = apiAncestor?.apiId ?? apiContextId;
        ...
        if (type === 'API') {
            ...
            const specSource: OpenApiSpecSource = resolvedApiId
                ? { type: 'API', apiId: resolvedApiId }
            ...
            const specSource = resolvedApiId
                ? { type: 'API' as const, apiId: resolvedApiId }
        ...
        return item;
    }, [portalId, loadNavItems, navigateToPage, apiContextId]);
