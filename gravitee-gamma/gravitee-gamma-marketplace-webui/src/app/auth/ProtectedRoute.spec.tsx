...
import { ForceLoginRoute, ProtectedRoute, PublicOnlyRoute } from './ProtectedRoute';
...
function renderForceLoginRoute(initialPath = '/catalog') {
    return render(
        <MemoryRouter initialEntries={[initialPath]}>
            <Routes>
...
describe('ForceLoginRoute', () => {
    it('should render catalog without login when forceLogin is off', () => {
        renderForceLoginRoute();
        expect(screen.getByText('Catalog')).toBeTruthy();
    });

    it('should redirect to login when forceLogin is on and unauthenticated', () => {
        seedBootstrap({ forceLoginEnabled: true });
        renderForceLoginRoute();
        expect(screen.getByText('Login Page')).toBeTruthy();
    });

    it('should render catalog when forceLogin is on and authenticated', () => {
        seedBootstrap({ forceLoginEnabled: true });
        useAuthStore.setState({ user: buildUser(), initialized: true });
        renderForceLoginRoute();
        expect(screen.getByText('Catalog')).toBeTruthy();
