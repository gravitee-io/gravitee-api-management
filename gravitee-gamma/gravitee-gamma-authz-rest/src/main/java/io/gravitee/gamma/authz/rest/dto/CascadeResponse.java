...
public record CascadeResponse(List<String> deletedEntityIds, List<String> deletedPolicyIds, int totalAffected) {
    public static CascadeResponse from(CascadeResult result) {
        return new CascadeResponse(result.deletedEntityIds(), result.deletedPolicyIds(), result.totalAffected());
    }
}
