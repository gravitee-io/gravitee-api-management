...
describe('ClientRegistrationProviderPage', () => {
    ...
    it('shows Client ID and a revealable Client Secret for CLIENT_CREDENTIALS and Initial Access Token otherwise', async () => {
        ...
        const iat = document.getElementById('dcr-iat') as HTMLInputElement;
        expect(iat).not.toBeNull();
        expect(iat.type).toBe('password');
        expect(screen.queryByRole('textbox', { name: /Client ID/ })).toBeNull();
    });
    ...
    it('masks the Initial Access Token for a viewer', () => {
        mockPermissions.mockReturnValue({
            canRead: true,
            canCreate: false,
            canUpdate: false,
        ...
        mockProviderQuery.mockReturnValue({
            data: { ...PROVIDER, initial_access_token_type: 'INITIAL_ACCESS_TOKEN', initial_access_token: 'super-secret-token' },
            isLoading: false,
            isError: false,
            refetch: jest.fn(),
        } as unknown as ReturnType<typeof useClientRegistrationProvider>);
        ...
        const iat = document.getElementById('dcr-iat') as HTMLInputElement;
        expect(iat.type).toBe('password');
        expect(iat.value).toBe('********');
        expect(iat.disabled).toBe(true);
    });

    it('is read-only in edit mode for a create-only user, even though they could create a new provider', () => {
        mockPermissions.mockReturnValue({
            canRead: true,
            canCreate: true,
            canUpdate: false,
    ...
    it('stays editable when creating for a create-only user, even without update permission', () => {
        mockPermissions.mockReturnValue({
            canRead: true,
            canCreate: true,
            canUpdate: false,
        ...
        renderCreate();
        expect(screen.getByRole('button', { name: 'Create provider' })).not.toBeNull();
