...
import { ApiReviewSettingsPage, PORTAL_SETTINGS_CHANGED_EVENT } from './ApiReviewSettingsPage';
...
describe('ApiReviewSettingsPage', () => {
    ...
    it('broadcasts the save so the APIM module drops its own cached copy of the flags', () => {
        // APIM runs with a separate QueryClient, which this page cannot invalidate.
        const listener = jest.fn();
        window.addEventListener(PORTAL_SETTINGS_CHANGED_EVENT, listener);
        renderPage();
        fireEvent.click(screen.getByLabelText('Enable API Review'));
        fireEvent.click(screen.getByRole('button', { name: /save changes/i }));
        ...
        window.removeEventListener(PORTAL_SETTINGS_CHANGED_EVENT, listener);
    });
