...
export function ClientRegistrationPage() {
    ...
    const { canCreate, canUpdate, canDelete, canUpdateSettings } = useClientRegistrationPermissions();
    ...
    const hasSeededApplication = useRef(false);
    // A background refetch of settings (window refocus, cache invalidation from a save) must not
    // clobber local toggle state after the initial load, or an in-progress optimistic edit is lost.
    useEffect(() => {
        if (settings && !hasSeededApplication.current) {
            hasSeededApplication.current = true;
    ...
    // isSavingRef gates synchronously, unlike savingSettings (derived from mutation state, which
    // lags a render behind), so two toggles fired in the same tick cannot both start a mutation
    // from the same stale snapshot and race to overwrite each other.
    const isSavingRef = useRef(false);

    const patchApplication = useCallback(
        (updater: (previous: PortalSettingsApplication) => PortalSettingsApplication) => {
            if (!canUpdateSettings || isSavingRef.current || !settings) return;
            const previous = applicationRef.current ?? {};
            ...
            saveSettings.mutate(buildApplicationRegistrationSavePayload(settings, next), {
                ...
                onError: () => {
                    applicationRef.current = previous;
                    setApplication(previous);
                },
            });
        },
        [canUpdateSettings, saveSettings, settings],
    );
    ...
    function handleEdit(provider: ClientRegistrationProvider) {
        if (!hasDcrLicense) {
            setLicenseDialogOpen(true);
            return;
        }
                ...
                <CardContent className="pt-6">
                    ...
                    <TooltipProvider delayDuration={200}>
                        <ToggleRow
                            ...
                            onToggle={enabled =>
                                patchApplication(prev => ({
                                    ...prev,
                                    types: { ...prev.types, simple: { enabled } },
                                }))
                            }
                        />
                    </TooltipProvider>
                ...
                <CardContent className="space-y-6 pt-6">
                    ...
                    <TooltipProvider delayDuration={200}>
                        <ToggleRow
                            ...
                            onToggle={enabled => patchApplication(prev => ({ ...prev, registration: { enabled } }))}
                        />
                        ...
                        <div className="border-t">
                            ...
                            <div className="divide-y">
                                <ToggleRow
                                    ...
                                    onToggle={enabled =>
                                        patchApplication(prev => ({ ...prev, types: { ...prev.types, browser: { enabled } } }))
                                    }
                                />
                                <ToggleRow
                                    id="type-web"
                                    ...
                                    onToggle={enabled =>
                                        patchApplication(prev => ({ ...prev, types: { ...prev.types, web: { enabled } } }))
                                    }
                                />
                                <ToggleRow
                                    id="type-native"
                                    ...
                                    onToggle={enabled =>
                                        patchApplication(prev => ({ ...prev, types: { ...prev.types, native: { enabled } } }))
                                    }
                                />
                                <ToggleRow
                                    id="type-b2b"
                                    ...
                                    onToggle={enabled =>
                                        patchApplication(prev => ({
                                            ...prev,
                                            types: { ...prev.types, backend_to_backend: { enabled } },
                                        }))
                                    }
                                />
                            </div>
