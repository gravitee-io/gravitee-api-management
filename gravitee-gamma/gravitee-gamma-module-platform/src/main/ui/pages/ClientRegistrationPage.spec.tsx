...
describe('ClientRegistrationPage', () => {
    ...
    it('navigates to the provider when a row is edited and the org is licensed', async () => {
        mockProviders.mockReturnValue({
            data: [PROVIDER],
            isLoading: false,
            isError: false,
        ...
        expect(mockNavigate).toHaveBeenCalledWith('prov-1');
    });

    it('opens the license dialog instead of bouncing when a row is edited without apim-dcr-registration', async () => {
        mockUseHasFeature.mockReturnValue(false);
        mockProviders.mockReturnValue({
            data: [PROVIDER],
            isLoading: false,
    ...
    it('ignores a second toggle fired before the first save settles', () => {
        renderPage();
        fireEvent.click(screen.getByLabelText('Simple'));
    ...
    it('does not clobber an in-progress toggle when settings background-refetches', () => {
        const { rerender } = renderPage();
        fireEvent.click(screen.getByLabelText('Simple'));
        expect(screen.getByLabelText('Simple')).not.toBeChecked();
