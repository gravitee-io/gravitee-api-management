...
describe('PrimaryOwnerModePage', () => {
    ...
    it.each([
        ...
        ['USER', 'GROUP'],
        ['GROUP', 'HYBRID'],
    ] as const)('saves API mode %s and API Product mode %s', async (apiMode, apiProductMode) => {
        renderPage();

        await radioGroupHarness({ name: 'API Primary Owner mode' }).select(apiMode);
        await radioGroupHarness({ name: 'API Product Primary Owner mode' }).select(apiProductMode);
        await buttonHarness({ name: /Save changes/i }).click();

        expect(mutate).toHaveBeenCalledWith(
            expect.objectContaining({
                api: expect.objectContaining({ primaryOwnerMode: apiMode }),
                apiProduct: expect.objectContaining({ primaryOwnerMode: apiProductMode }),
            }),
            expect.any(Object),
        );
    });

    it('saves only the API Product mode when only that radio changes', async () => {
        renderPage();
        ...
        expect(mutate).toHaveBeenCalledWith(
            expect.objectContaining({
                api: expect.objectContaining({ labelsDictionary: ['internal'], primaryOwnerMode: 'USER' }),
    ...
    it('leaves an unrecognized API mode untouched when saving API Product', async () => {
        mockUsePortalSettings.mockReturnValue({
            data: {
                ...SETTINGS,
                api: { labelsDictionary: ['internal'], primaryOwnerMode: 'TEAM' },
                apiProduct: { primaryOwnerMode: 'HYBRID' },
            },
            isLoading: false,
            isError: false,
        ...
        expect(mutate).toHaveBeenCalledWith(
            expect.objectContaining({
                api: expect.objectContaining({ labelsDictionary: ['internal'], primaryOwnerMode: 'TEAM' }),
                apiProduct: expect.objectContaining({ primaryOwnerMode: 'USER' }),
            }),
            expect.any(Object),
        );
