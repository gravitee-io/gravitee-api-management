...
describe('DictionariesTable', () => {
    ...
    describe('search filtering', () => {
        it('filters rows by name', () => {
            renderTable();
            fireEvent.change(screen.getByPlaceholderText('Search dictionaries...'), { target: { value: 'remote' } });
            expect(screen.queryByText('Remote Codes')).not.toBeNull();
        ...
        it('filters rows by description', () => {
            ...
            fireEvent.change(screen.getByPlaceholderText('Search dictionaries...'), { target: { value: 'partner' } });
            expect(screen.queryByText('Remote Codes')).not.toBeNull();
            expect(screen.queryByText('Countries')).toBeNull();
        });
        ...
        it('shows the no-match message when search has no results', () => {
            renderTable();
            fireEvent.change(screen.getByPlaceholderText('Search dictionaries...'), { target: { value: 'zzznomatch' } });
            expect(screen.queryByText('No dictionaries match your search.')).not.toBeNull();
        });

        it('is case-insensitive', () => {
            renderTable();
            fireEvent.change(screen.getByPlaceholderText('Search dictionaries...'), { target: { value: 'COUNTRIES' } });
            expect(screen.queryByText('Countries')).not.toBeNull();
        });
    });
