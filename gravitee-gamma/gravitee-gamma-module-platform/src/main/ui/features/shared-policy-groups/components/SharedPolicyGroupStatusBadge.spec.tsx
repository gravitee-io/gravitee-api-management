...
describe('SharedPolicyGroupStatusBadge', () => {
    ...
    it('explains the pending state with a Graphene tooltip', async () => {
        const user = userEvent.setup();
        render(<SharedPolicyGroupStatusBadge lifecycleState="PENDING" />);

        await user.hover(screen.getByText('Pending'));

        expect((await screen.findByRole('tooltip')).textContent).toBe('Latest changes are not deployed');
    });
});
