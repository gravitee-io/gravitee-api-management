...
function useFederationGateInputs(): { readonly federationEnabled: boolean; readonly license: License | null } {
...
export function useFederationAvailable(): boolean {
    return isFederationAvailable(useFederationGateInputs());
}

/**
 * Federation is switched on but no license has been reported, so the gate denies without knowing
 * whether the organization is entitled — unlike an oss tier or an expired license, which are answers.
 */
export function useFederationEntitlementUnknown(): boolean {
    const { federationEnabled, license } = useFederationGateInputs();
