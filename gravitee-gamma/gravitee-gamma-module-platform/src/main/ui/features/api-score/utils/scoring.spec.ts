...
describe('SCORE_PILL_CLASS', () => {
    it('covers every scoreTone so overview and table pills cannot drift', () => {
        expect(SCORE_PILL_CLASS[scoreTone(0.8)]).toContain('success');
        expect(SCORE_PILL_CLASS[scoreTone(0.4)]).toContain('warning');
        expect(SCORE_PILL_CLASS[scoreTone(0.39)]).toContain('destructive');
    });
});
