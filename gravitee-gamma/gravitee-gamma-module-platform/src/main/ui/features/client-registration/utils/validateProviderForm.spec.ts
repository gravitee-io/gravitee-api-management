...
describe('validateProviderForm', () => {
    ...
    it('rejects duplicate and blank claim mapping rows', () => {
        ...
        expect(validateProviderForm(validForm({ claim_mappings: [{ id: '1', key: 'a', value: '' }] })).claim_mappings).toBe(
            'Every mapping needs both a claim name and a registration request field',
        );
        expect(
            validateProviderForm(validForm({ claim_mappings: [{ id: '1', key: 'org_id', value: 'metadata.organization' }] }))
                .claim_mappings,
        ).toBeUndefined();
    });
});
