...
describe('ClientRegistrationProvidersTable', () => {
    it('renders provider name and description', () => {
        render(<ClientRegistrationProvidersTable providers={[PROVIDER]} canDelete canUpdate onEdit={jest.fn()} onDelete={jest.fn()} />);
    ...
    it('hides Delete when the user cannot delete', async () => {
        ...
        render(
            <ClientRegistrationProvidersTable providers={[PROVIDER]} canDelete={false} canUpdate onEdit={jest.fn()} onDelete={jest.fn()} />,
        );
    ...
    it('calls onDelete from the kebab', async () => {
        ...
        const user = userEvent.setup();
        render(<ClientRegistrationProvidersTable providers={[PROVIDER]} canDelete canUpdate onEdit={jest.fn()} onDelete={onDelete} />);
        await user.click(screen.getByRole('button', { name: 'Provider actions' }));
        await user.click(screen.getByText('Delete'));
        expect(onDelete).toHaveBeenCalledWith(PROVIDER);
    });

    it('labels the row action View instead of Edit when the user cannot update', async () => {
        const user = userEvent.setup();
        render(
