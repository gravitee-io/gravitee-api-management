...
export function RegisterApplicationForm() {
    ...
    const { requireUserGroups, isLoading: isLoadingUserGroupSetting } = useUserGroupRequired();
    const createApplication = useCreateApplication();

    const [draft, setDraft] = useState<RegisterApplicationDraft>(EMPTY_DRAFT);

    const enabledTypes = applicationTypes;

    const selectedType = enabledTypes.find(type => type.id === draft.typeId) ?? enabledTypes[0];

    useEffect(() => {
        if (enabledTypes.length === 0) return;
        if (!enabledTypes.some(type => type.id === draft.typeId)) {
    ...
    const handleTypeChange = (typeId: string) => {
        const nextType = enabledTypes.find(type => type.id === typeId);
        if (!nextType) return;
        updateDraft({
            typeId,
    ...
    if (isLoadingTypes && enabledTypes.length === 0) {
        return (
                ...
                <CardContent className="space-y-5">
                    ...
                    {showGroupsField && (
                        <ApplicationGroupsField
                            ...
                            isLoading={isLoadingGroups || isLoadingUserGroupSetting}
                    ...
                    <div className="space-y-4">
                        ...
                        {enabledTypes.length > 0 && (
                            <ApplicationTypeSelector
                                ...
                                readOnly={enabledTypes.length === 1}
                            />
                        )}
