...
export function useApplicationSubscriptionApiKeys(applicationId: string | undefined, subscriptionId: string | undefined, enabled: boolean) {
    ...
    return useQuery({
        queryKey: applicationSubscriptionKeys.apiKeys(env?.id ?? '', applicationId ?? '', subscriptionId ?? ''),
        queryFn: async () => {
            const entities = await listSubscriptionApiKeys(env!.id, applicationId!, subscriptionId!);
            return mapApiKeysToRows(entities);
        },
        enabled: Boolean(enabled && env && applicationId && subscriptionId),
    });
}
