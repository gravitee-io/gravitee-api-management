...
export type PrimaryOwnerModeSelection = PrimaryOwnerMode | null;

export type PrimaryOwnerResource = 'API' | 'API Product';

export interface PrimaryOwnerModeFormState {
    api: PrimaryOwnerModeSelection;
    apiProduct: PrimaryOwnerModeSelection;
}
...
export function parsePrimaryOwnerMode(value: string | undefined): PrimaryOwnerModeSelection {
    const normalized = value?.trim().toUpperCase();
    if (!normalized) {
        return DEFAULT_PRIMARY_OWNER_MODE;
    }
    return isPrimaryOwnerMode(normalized) ? normalized : null;
}
