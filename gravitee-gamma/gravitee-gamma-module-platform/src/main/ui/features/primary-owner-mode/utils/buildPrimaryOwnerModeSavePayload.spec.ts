...
import { parsePrimaryOwnerMode } from './primaryOwnerMode';
...
describe('buildPrimaryOwnerModeSavePayload', () => {
    ...
    it('updates only apiProduct when api mode is not selected in form state', () => {
        const payload = buildPrimaryOwnerModeSavePayload({ cors: { maxAge: 60 } }, { api: null, apiProduct: 'USER' });
    ...
    it('preserves an unrecognized stored api mode when only apiProduct is saved', () => {
        const current: PortalSettings = {
            api: { labelsDictionary: ['internal'], primaryOwnerMode: 'TEAM' },
            apiProduct: { primaryOwnerMode: 'HYBRID' },
        };
        const formState = {
            api: parsePrimaryOwnerMode(current.api?.primaryOwnerMode),
            apiProduct: 'USER' as const,
        };

        expect(formState.api).toBeNull();
        expect(buildPrimaryOwnerModeSavePayload(current, formState)).toEqual({
            api: { labelsDictionary: ['internal'], primaryOwnerMode: 'TEAM' },
            apiProduct: { primaryOwnerMode: 'USER' },
        });
    });
});
