...
describe('primaryOwnerMode', () => {
    ...
    it('accepts only HYBRID, USER, and GROUP as live radio values', () => {
        expect(isPrimaryOwnerMode('HYBRID')).toBe(true);
        expect(isPrimaryOwnerMode('USER')).toBe(true);
        expect(isPrimaryOwnerMode('GROUP')).toBe(true);
        expect(isPrimaryOwnerMode('')).toBe(false);
    });

    it('does not coerce an unrecognized mode', () => {
        expect(isPrimaryOwnerMode('TEAM')).toBe(false);
        expect(parsePrimaryOwnerMode('TEAM')).toBeNull();
        expect(parsePrimaryOwnerMode('owner')).toBeNull();
    });

    it('defaults missing values to HYBRID', () => {
    ...
    it('keeps unrecognized stored modes as null in form state', () => {
        expect(
            buildPrimaryOwnerModeFormState({
                api: { primaryOwnerMode: 'TEAM' },
                apiProduct: { primaryOwnerMode: 'GROUP' },
            }),
        ).toEqual({ api: null, apiProduct: 'GROUP' });
    });
