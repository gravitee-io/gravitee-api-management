...
export function useResetPortalBrandedSenders() {
    ...
    return useMutation({
        mutationFn: () => {
            const envId = env?.id;
            if (!envId) {
                throw new Error('Environment is required to reset branded senders.');
            }
            return resetPortalBrandedSenders(envId);
        },
        onSuccess: (settings: PortalSettings) => {
            const envId = env?.id;
            if (envId) {
                queryClient.setQueryData(portalSettingsKeys.env(envId), settings);
            }
