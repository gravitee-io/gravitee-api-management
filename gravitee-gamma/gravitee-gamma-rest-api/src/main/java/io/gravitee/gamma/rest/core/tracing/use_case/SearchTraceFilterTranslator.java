...
public final class SearchTraceFilterTranslator {
     ...
     * {@code CommonTraceFilterContributor}; the {@code KAFKA_*} entries pair
     * with {@code EsmTraceFilterContributor} and use the attribute keys the native Kafka reactor
     * emits ({@code KafkaTracingHelper}). Each is written on EVERY per-request span: filters are ANDed
     * per span document downstream, so an attribute only some spans carry could never be combined.
     */
