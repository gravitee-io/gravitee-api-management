...
public class EventServiceTest {
    ...
    public void shouldFilterEvent() throws TechnicalException {
        ...
        Page<Map<String, String>> page = eventService.search(Arrays.asList(io.gravitee.management.model.EventType.GATEWAY_STARTED),
        ...
        // test with predicate
        page = eventService.search(Arrays.asList(io.gravitee.management.model.EventType.GATEWAY_STARTED),
                Collections.EMPTY_MAP, 0,0,1,10, (evt) -> {
                    Map<String, String> map = new HashMap<>();
                    map.put("id", evt.getId());
                    map.put("state", evt.getType().name());
                    return map;
                }, (map) -> !map.get("state").equals(io.gravitee.management.model.EventType.GATEWAY_STOPPED.name()));
        assertNotNull(page);
        assertNotNull(page.getContent());
