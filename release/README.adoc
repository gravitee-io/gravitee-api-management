...
Only the root pom moves. `gravitee-apim-distribution/pom.xml` carries its own triplet, so a core release leaves it alone — and so does the distribution's `apim.core.version`, the version of the core it assembles. Advancing that pin is a separate change, reviewed like any other: it is the moment someone decides a core is ready to ship.
