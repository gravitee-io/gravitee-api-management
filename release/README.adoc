...
| Derived from `--version`: `4.12.18` releases from `4.12.x`. Never from your current branch — no script reads your local git state. `--branch` overrides it; the branch that will be released is printed with the tag before the command asks to continue.
...
[source,shell]
----
yarn prepare_distribution_release --version=4.12.18
----

It publishes nothing itself. It announces whether the run publishes, shows the branch and the tag, and asks once. CI then commits `4.12.18`, tags it, reopens the branch on `4.12.19-SNAPSHOT`, and pushes the branch and, after it, the tag. *Pushing that tag is what publishes*: a second pipeline starts from it and produces the images, the bundle, the RPMs and the chart.
...
[source,shell]
----
yarn prepare_distribution_release --version=4.12.17-hotfix.1
----
