...
console.log(chalk.green(`Release started !`));
...
console.log(chalk.green(`Step 1/5 finished`));

const next = await question(
  chalk.blue(`Continue and run next step command 'npm run package_zip -- --version=${releasingVersion}'? (y/n)`),
).then((answer) => (answer === 'y' ? true : false));
if (!next) {
  process.exit(1);
}

await $`npm run package_zip -- --version=${releasingVersion}`;
