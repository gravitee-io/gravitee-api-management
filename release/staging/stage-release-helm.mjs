...
console.log(chalk.yellow(`This script will stage a new helm release in order to test it on ${branch}.`));
console.log(
  chalk.yellow(`This will be done by overwriting or creating chart apim ${graviteeio_version} at oci://graviteeio.azurecr.io/helm.`),
);
console.log(chalk.yellow(`If you are performing an actual release, then you're in the wrong place.`));

console.log();

const iKnowWhatImDoing = await question(chalk.blue(`Do you want to continue ? (Type 'yes' if you want to)\n`));
