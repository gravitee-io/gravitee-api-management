...
//        return createListResponse(GraviteeContext.getExecutionContext(), logs, paginationParam, metadata, false);
        var result = searchApplicationConnectionLogsUseCase.execute(GraviteeContext.getExecutionContext(), new SearchApplicationConnectionLogsUseCase.Input(
                applicationId,
                new SearchLogsFilters(logsParam.getFrom(), logsParam.getTo(), Set.of(applicationId), null, null, null, null),
                Optional.empty())
        );
        return createListResponse(GraviteeContext.getExecutionContext(), result.data().stream().map(log -> logMapper.convert(log)).toList(), paginationParam, result.metadata(), true);
    }
