...
public class ApplicationSubscribersResourceTest extends AbstractResourceTest {
    ...
    public void shouldFilterByAccessibleApisAndStatusesAndSortByHits() {
        ...
        api2.setId("api-2");
        doReturn(api1).when(apiSearchService).findGenericById(eq(GraviteeContext.getExecutionContext()), eq("api-1"), eq(false), eq(false));
        doReturn(api2).when(apiSearchService).findGenericById(eq(GraviteeContext.getExecutionContext()), eq("api-2"), eq(false), eq(false));
    ...
    public void shouldSortByNameWhenEqualHitsWithEmptyAnalytics() {
        ...
        apiB.setId("api-B");
        doReturn(apiA).when(apiSearchService).findGenericById(eq(GraviteeContext.getExecutionContext()), eq("api-A"), eq(false), eq(false));
        doReturn(apiB).when(apiSearchService).findGenericById(eq(GraviteeContext.getExecutionContext()), eq("api-B"), eq(false), eq(false));
