...
/**
 * In-memory implementation of ApiProductQueryService for testing purposes.
 */
public class ApiProductQueryServiceInMemory implements ApiProductQueryService, InMemoryAlternative<ApiProduct> {

    private final List<ApiProduct> storage;

    public ApiProductQueryServiceInMemory() {
    ...
    public Optional<ApiProduct> findById(String apiProductId) {
        return storage
            .stream()
            .filter(apiKey -> apiKey.getId().equals(apiProductId))
            .findFirst();
    }
    ...
    @Override
    public void initWith(List<ApiProduct> items) {
        storage.clear();
    ...
    @Override
    public List<ApiProduct> storage() {
