...
public class DeployModelV4CommandHandlerTest {
    ...
    public class Create {
        ...
        public void create_API(
            DeployModelCommandPayload.DeploymentMode deploymentMode,
            DeploymentMode expectedPermissionMode,
            boolean shouldHavePolicies
        ) {
            ...
            when(
                oaiToImportApiUseCase.execute(
                    argThat(input -> {
                        var baseConditions =
                            input.importSwaggerDescriptor().getPayload().equals(payload.swaggerDefinition()) &&
                            input.withDocumentation() &&
                            !input.withOASValidationPolicy() &&
                            input.auditInfo().organizationId().equals(ORGANIZATION_ID) &&
                            input.auditInfo().environmentId().equals(ENVIRONMENT_ID);

                        if (shouldHavePolicies) {
                            return (
                                baseConditions &&
                                input.importSwaggerDescriptor().getWithPolicies().size() == 1 &&
                                input.importSwaggerDescriptor().getWithPolicies().getFirst().equals("mock")
                            );
                        } else {
            ...
            ).thenAnswer(i -> new OAIToImportApiUseCase.Output(new ApiWithFlows()));
        ...
        static Stream<Arguments> create_API() {
            return Stream.of(
                Arguments.of(DeployModelCommandPayload.DeploymentMode.API_MOCKED, DeploymentMode.API_MOCKED, true),
                Arguments.of(DeployModelCommandPayload.DeploymentMode.API_PUBLISHED, DeploymentMode.API_PUBLISHED, true),
                Arguments.of(null, DeploymentMode.API_DOCUMENTED, false)
            );
        }
