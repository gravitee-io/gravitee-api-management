...
public class PlanSearchServiceImplTest {
    ...
    public void shouldFindByApi() throws TechnicalException {
        ...
        PlanEntity planEntity2 = new PlanEntity();
        planEntity2.setId(plan2.getId());
        when(genericPlanMapper.toGenericPlan(eq(api), eq(plan1), anyMap(), anyMap(), anyMap())).thenReturn(planEntity1);
        when(genericPlanMapper.toGenericPlan(eq(api), eq(plan2), anyMap(), anyMap(), anyMap())).thenReturn(planEntity2);
