...
public class AddOrgIdNotificationConfigsUpgraderTest {
    ...
    public void upgrade() throws TechnicalException {
        when(environmentRepository.findAll())
            .thenReturn(
                Set.of(
                    ...
                    Environment.builder().id("env2").organizationId("org1").build(),
                    Environment.builder().id("env3").organizationId("org2").build(),
                    Environment.builder().id("env4").organizationId("org2").build(),
                    Environment.builder().id("env5").organizationId("org3").build()
                )
            );
        when(apiRepository.findAll())
            .thenReturn(
                Set.of(
                    Api.builder().id("api1").environmentId("env1").build(),
                    Api.builder().id("api2").environmentId("env2").build(),
                    Api.builder().id("api3").environmentId("env3").build(),
                    Api.builder().id("api4").environmentId("env4").build(),
                    Api.builder().id("api5").environmentId("env5").build()
                )
            );
        when(applicationRepository.findAll())
            .thenReturn(
                Set.of(
                    Application.builder().id("app1").environmentId("env1").build(),
                    Application.builder().id("app2").environmentId("env2").build(),
                    Application.builder().id("app3").environmentId("env3").build(),
                    Application.builder().id("app4").environmentId("env4").build(),
                    Application.builder().id("app5").environmentId("env5").build()
                )
            );
        ...
        assertAll(
            () -> assertThat(genericNotifsConfigsById.get("gn1").getOrgId()).isEqualTo("org1"),
            () -> assertThat(genericNotifsConfigsById.get("gn1").getReferenceType()).isEqualTo(NotificationReferenceType.ENVIRONMENT),
            () -> assertThat(genericNotifsConfigsById.get("gn2").getOrgId()).isEqualTo("org2"),
            () -> assertThat(genericNotifsConfigsById.get("gn3").getOrgId()).isEqualTo("org3"),
            () -> assertThat(genericNotifsConfigsById.get("gn4").getOrgId()).isEqualTo("org1"),
            () -> assertThat(genericNotifsConfigsById.get("gn4").getReferenceType()).isEqualTo(NotificationReferenceType.ENVIRONMENT),
            () -> assertThat(genericNotifsConfigsById.get("gn5").getOrgId()).isEqualTo("org1"),
            () -> assertThat(portalNotifsConfigsByReferenceId.get("env4").getOrgId()).isEqualTo("org2"),
            () ->
                assertThat(portalNotifsConfigsByReferenceId.get("env4").getReferenceType())
                    .isEqualTo(NotificationReferenceType.ENVIRONMENT),
            () -> assertThat(portalNotifsConfigsByReferenceId.get("api1").getOrgId()).isEqualTo("org1"),
            () -> assertThat(portalNotifsConfigsByReferenceId.get("app2").getOrgId()).isEqualTo("org1"),
            () -> assertThat(portalNotifsConfigsByReferenceId.get("env2").getOrgId()).isEqualTo("org1"),
            () ->
                assertThat(portalNotifsConfigsByReferenceId.get("env2").getReferenceType())
                    .isEqualTo(NotificationReferenceType.ENVIRONMENT),
            () -> assertThat(portalNotifsConfigsByReferenceId.get("api5").getOrgId()).isEqualTo("org3")
        );
    }
}
