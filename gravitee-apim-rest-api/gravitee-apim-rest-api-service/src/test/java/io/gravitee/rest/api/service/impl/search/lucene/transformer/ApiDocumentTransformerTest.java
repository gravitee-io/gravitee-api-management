...
class ApiDocumentTransformerTest {
    ...
    private static final BytesRef SORT_KEY_OF_STARTED = expectedSortKeyOf("STARTED");

    private static final BytesRef SORT_KEY_OF_ALPHA_BETA_GAMMA_WITHOUT_SPECIAL_CHARS = expectedSortKeyOf("AlphaBeta Gamma");
    ...
    void should_transform_id_and_type_only_when_definition_version_and_name_are_null() {
        ...
        assertThat(doc.getFields()).extracting(IndexableField::name).containsExactlyInAnyOrder(FIELD_ID, FIELD_ID_SORTED, FIELD_TYPE);
    ...
    // Takes the already-cleaned value and builds its key with a test-local English collator, so the assertion still
    // catches a change of cleaning regex, collator locale or strength; computing the key instead of pinning its bytes
    // keeps it stable across JDK/CLDR collation data updates.
    private static BytesRef expectedSortKeyOf(String cleanedValue) {
        Collator collator = Collator.getInstance(Locale.ENGLISH);
        collator.setStrength(Collator.SECONDARY);
        return new BytesRef(collator.getCollationKey(cleanedValue).toByteArray());
    }
