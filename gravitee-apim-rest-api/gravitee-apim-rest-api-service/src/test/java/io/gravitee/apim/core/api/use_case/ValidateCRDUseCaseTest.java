...
class ValidateCRDUseCaseTest {

    private static final AuditInfo AUDIT_INFO = AuditInfo.builder().environmentId("TEST").organizationId("TEST").build();

    ValidateCRDDomainService validator = mock(ValidateCRDDomainService.class);

    ValidateCRDUseCase cut = new ValidateCRDUseCase(validator);

    @Test
    void should_return_status_and_no_errors() {
        var spec = ApiCRDFixtures.anApiCRD();

        var input = new ImportCRDUseCase.Input(AUDIT_INFO, spec);

        when(validator.validateAndSanitize(new ValidateCRDDomainService.Input(AUDIT_INFO, spec)))
            .thenReturn(Validator.Result.ofValue(new ValidateCRDDomainService.Input(AUDIT_INFO, spec)));
        ...
        when(validator.validateAndSanitize(new ValidateCRDDomainService.Input(AUDIT_INFO, spec)))
            .thenReturn(
                Validator.Result.ofBoth(
                    new ValidateCRDDomainService.Input(AUDIT_INFO, spec),
