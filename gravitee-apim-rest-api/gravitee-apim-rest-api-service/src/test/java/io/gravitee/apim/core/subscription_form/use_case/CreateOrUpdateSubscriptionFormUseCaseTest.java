...
class CreateOrUpdateSubscriptionFormUseCaseTest {
    ...
    void setUp() {
        ...
        useCase = new CreateOrUpdateSubscriptionFormUseCase(crudService, specDomainService);
        validateUseCase = new ValidateSubscriptionFormUseCase(specDomainService);
    }

    private static SubscriptionFormSpecDomainService.Spec aSpec(String name, boolean enabled, List<String> apiIds) {
        return new SubscriptionFormSpecDomainService.Spec(AUDIT_INFO, HRID, name, GMD, enabled, apiIds);
    }
    ...
    class Apply {
        ...
        void should_create_the_form_with_an_id_derived_from_the_hrid() {
            var output = useCase.execute(aSpec("Partners", true, null));

            assertThat(output.errors()).isEmpty();
            var created = output.subscriptionForm();
            assertThat(created.getId()).isEqualTo(aSpec("Partners", true, null).subscriptionFormId());
        ...
        void should_update_the_same_form_when_the_spec_is_applied_again() {
            var first = useCase.execute(aSpec("Partners", false, null)).subscriptionForm();
            ...
            var output = useCase.execute(aSpec("Partners v2", true, List.of("api-1")));
        ...
        void should_reject_a_blank_hrid() {
            var output = useCase.execute(new SubscriptionFormSpecDomainService.Spec(AUDIT_INFO, " ", "Partners", GMD, false, null));
    ...
    class DryRun {
        ...
        void should_report_the_findings_without_persisting() {
            var output = validateUseCase.execute(aSpec("Partners", true, List.of("ghost")));
        ...
        void should_preview_the_form_the_apply_would_write() {
            var existing = SubscriptionFormFixtures.aSubscriptionFormBuilder()
                .id(aSpec("Partners", false, null).subscriptionFormId())
                .name("Before")
                .enabled(false)
                .build();
            ...
            var output = validateUseCase.execute(aSpec("After", true, null));
        ...
        void should_return_no_error_for_a_valid_spec() {
            var spec = aSpec("Partners", true, null);
