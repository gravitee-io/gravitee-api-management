...
public class ConnectionLogsCrudServiceImplTest {
    ...
    class SearchApplicationConnectionLogs {
        ...
        @Test
        void should_search_application_connection_logs() throws AnalyticsException {
            when(logRepository.searchConnectionLogs(any(QueryContext.class), any(), any()))
                .thenReturn(new LogResponse<>(1, List.of()));

            logCrudService.searchApplicationConnectionLogs(GraviteeContext.getExecutionContext(), SearchLogsFilters.builder().build(), new PageableImpl(1, 20));

            var captorQueryContext = ArgumentCaptor.forClass(QueryContext.class);
            var captorConnectionLogDetailQuery = ArgumentCaptor.forClass(ConnectionLogQuery.class);
            var captorDefinitionVersions = ArgumentCaptor.forClass(List.class);
            verify(logRepository).searchConnectionLogs(captorQueryContext.capture(), captorConnectionLogDetailQuery.capture(), captorDefinitionVersions.capture());
