...
class SearchApiAnalyticUseCaseTest {
    ...
    SearchApiAnalyticUseCase useCase;

    @BeforeEach
    void setUp() {
        useCase =
            new SearchApiAnalyticUseCase(
    ...
    @AfterEach
    void tearDown() {
        fakeAnalyticsQueryService.apiAnalytic = null;
        GraviteeContext.cleanContext();
    }

    @Test
    void should_return_empty_api_analytic() {
        var result = useCase.execute(GraviteeContext.getExecutionContext(), new SearchApiAnalyticUseCase.Input(API_ID, REQUEST_ID));
        assertThat(result).isNotNull();
        assertThat(result.apiAnalytic()).isEmpty();
    }
    ...
    void should_return_api_analytic_with_unknow_app_and_plan() {
        ...
        fakeAnalyticsQueryService.apiAnalytic =
            io.gravitee.rest.api.model.v4.analytics.ApiAnalytic.builder().apiId(API_ID).applicationId(applicationId).planId(planId).build();

        var result = useCase.execute(GraviteeContext.getExecutionContext(), new SearchApiAnalyticUseCase.Input(API_ID, REQUEST_ID));

        assertThat(result).isNotNull();
        assertThat(result.apiAnalytic())
            .hasValueSatisfying(apiAnalytic -> {
                assertThat(apiAnalytic.getApplication())
                    .extracting(BaseApplicationEntity::getId, BaseApplicationEntity::getName)
                    .containsExactly(applicationId, "Unknown");

                assertThat(apiAnalytic.getPlan())
    ...
    void should_return_an_api_analytic() {
        ...
        fakeAnalyticsQueryService.apiAnalytic =
            io.gravitee.rest.api.model.v4.analytics.ApiAnalytic
                .builder()
                .apiId(API_ID)
        ...
        assertThat(result.apiAnalytic())
            .hasValueSatisfying(apiAnalytic -> {
                assertThat(apiAnalytic.getApiId()).isEqualTo(API_ID);
                assertThat(apiAnalytic.getRequestId()).isEqualTo(REQUEST_ID);
                assertThat(apiAnalytic.getTransactionId()).isEqualTo(transactionId);
                assertThat(apiAnalytic.getHost()).isEqualTo(host);
                assertThat(apiAnalytic.getUri()).isEqualTo(uri);
                assertThat(apiAnalytic.getStatus()).isEqualTo(status);
                assertThat(apiAnalytic.getRequestContentLength()).isEqualTo(requestContentLength);
                assertThat(apiAnalytic.getResponseContentLength()).isEqualTo(responseContentLength);
                assertThat(apiAnalytic.getGatewayLatency()).isEqualTo(gatewayLatency);
                assertThat(apiAnalytic.getGatewayResponseTime()).isEqualTo(gatewayResponseTime);
                assertThat(apiAnalytic.getRemoteAddress()).isEqualTo(remoteAddress);
                assertThat(apiAnalytic.getMethod()).isEqualTo(HttpMethod.GET);
                assertThat(apiAnalytic.getEndpointResponseTime()).isEqualTo(endpointResponseTime);
                assertThat(apiAnalytic.getEndpoint()).isEqualTo(endpoint);
                ...
                assertThat(apiAnalytic.getGateway())
