...
class MigrateApiUseCaseTest {
    ...
    private static final AuditInfo EXECUTION_CONTEXT = AuditInfo
        .builder()
    ...
    void should_return_fail_when_api_not_found() {
        // When
        var result = useCase.execute(new MigrateApiUseCase.Input(API_ID, null, EXECUTION_CONTEXT, USER_ID));

        // Then
        assertThat(result).isInstanceOf(MigrateApiUseCase.Output.Fail.class);
        var fail = (MigrateApiUseCase.Output.Fail) result;
        assertThat(fail.apiId()).isEqualTo(API_ID);
        assertThat(fail.errors()).containsExactly("API not found");
    }
    ...
    void should_return_fail_when_api_is_not_v2() {
        ...
        var fail = (MigrateApiUseCase.Output.Fail) result;
        assertThat(fail.apiId()).isEqualTo(API_ID);
        assertThat(fail.errors()).containsExactly("Cannot upgrade an API which is not a v2 definition");
    }
    ...
    void should_not_upgrade_api_in_dry_run_mode() {
        ...
        // When
        var result = useCase.execute(new MigrateApiUseCase.Input(API_ID, DRY_RUN, EXECUTION_CONTEXT, USER_ID));

        // Then
        assertThat(result).isInstanceOf(MigrateApiUseCase.Output.Success.class);
        var success = (MigrateApiUseCase.Output.Success) result;
        assertThat(success.apiId()).isEqualTo(API_ID);
    ...
    void should_upgrade_api_definition() {
        ...
        var upgradedApi = apiCrudService.findById(API_ID);
        assertThat(upgradedApi).map(Api::getDefinitionVersion).hasValue(DefinitionVersion.V4);

        var upgradedPlan = planCrudService.findByApiId(API_ID);
        assertThat(upgradedPlan).hasSize(1).map(Plan::getId).first().isEqualTo("plan-id");
    ...
    void should_upgrade_api_with_multiple_plans() {
        ...
        assertThat(upgradedPlans).hasSize(2);
        assertThat(upgradedPlans).extracting(Plan::getId).containsExactlyInAnyOrder("plan-1", "plan-2");
    }
