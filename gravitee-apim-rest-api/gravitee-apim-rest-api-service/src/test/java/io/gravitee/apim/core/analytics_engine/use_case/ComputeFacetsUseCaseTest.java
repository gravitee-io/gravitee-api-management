...
class ComputeFacetsUseCaseTest {
    ...
    void should_load_context_from_audit_info() {
        ...
        useCase.execute(new ComputeFacetsUseCase.Input(AUDIT_INFO, request));
    ...
    void should_apply_transformer_filters_to_query() {
        ...
        when(queryService.searchFacets(any(), any())).thenReturn(new FacetsResponse(List.of()));

        useCase.execute(new ComputeFacetsUseCase.Input(AUDIT_INFO, request));

        var requestCaptor = ArgumentCaptor.forClass(FacetsRequest.class);
        verify(queryService).searchFacets(eq(EXECUTION_CONTEXT), requestCaptor.capture());
    ...
    void should_call_post_processor_with_analytics_context() {
        ...
        verify(bucketNamesPostProcessor).mapBucketNames(eq(ANALYTICS_CONTEXT), eq(request.facets()), any(FacetsResponse.class));
    }
    ...
    void should_return_post_processed_response() {
        ...
        when(queryContextProvider.resolve(request)).thenReturn(Map.of(queryService, request));
        when(queryService.searchFacets(any(), any())).thenReturn(rawResponse);

        var output = useCase.execute(new ComputeFacetsUseCase.Input(AUDIT_INFO, request));
