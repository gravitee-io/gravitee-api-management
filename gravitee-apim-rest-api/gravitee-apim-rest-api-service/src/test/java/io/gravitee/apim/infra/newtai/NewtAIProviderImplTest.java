...
class NewtAIProviderImplTest {
    ...
    private final InstallationCrudServiceInMemory installationCrudService = new InstallationCrudServiceInMemory();

    @Mock
    private ApimMetadataImpl apimMetadata;
    ...
    void setUp() {
        ...
        cut = new NewtAIProviderImpl(cockpitConnector, apimMetadata, installationCrudService);
    }

    @AfterEach
    void tearDown() {
        installationCrudService.reset();
    }

    @Test
    @SneakyThrows
    void should_generate_el(VertxTestContext context) {
        when(installationCrudService.getInstallationId()).thenReturn(Optional.of("installationId"));
