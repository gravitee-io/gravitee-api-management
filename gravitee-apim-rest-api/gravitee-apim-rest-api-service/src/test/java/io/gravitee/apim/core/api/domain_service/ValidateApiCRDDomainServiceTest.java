...
class ValidateApiCRDDomainServiceTest {
    ...
    void should_return_input_with_id_cross_id_generated_from_hrid() {
        ...
        var crossIder = new UuidString.CrossId(input.auditInfo(), hrid);
        var expected = spec.toBuilder().id(crossIder.toID(input.auditInfo())).hrid(hrid).crossId(crossIder.toString()).build();
