...
Discover **${api.name}** through the Gravitee Developer Portal. Subscribe to a plan, then call this agent from your own applications and agents through a secure gateway.
...
<gmd-card class="overview-info">
    <gmd-card-title>Agent information</gmd-card-title>
    <gmd-md>
        - **Version:** `${api.version!""}`
...
<gmd-grid columns="3">
    <gmd-card class="overview-card">
        ...
        <gmd-md>
            Request access to a plan that fits your integration. Manage keys and credentials from your application dashboard.
        </gmd-md>
    </gmd-card>
    <gmd-card class="overview-card">
        <gmd-card-title>Explore documentation</gmd-card-title>
        <gmd-md>
            Browse the agent's skills, message schemas and example exchanges. Try them directly from the portal when available.
        </gmd-md>
    </gmd-card>
    <gmd-card class="overview-card">
        <gmd-card-title>Integrate</gmd-card-title>
        <gmd-md>
            Point your A2A client at the gateway endpoint. Check the subscription page for authentication details.
        </gmd-md>
    </gmd-card>
</gmd-grid>
...
Use this overview to introduce your agent, describe what it is good at, and guide consumers to the resources they need most.

- Add a **Quick start** section with an example conversation
