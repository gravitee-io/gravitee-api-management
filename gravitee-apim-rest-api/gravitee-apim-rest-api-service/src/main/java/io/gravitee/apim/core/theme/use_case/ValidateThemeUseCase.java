...
public class ValidateThemeUseCase {

    private final ValidateThemeDomainService validator;

    public CreateOrUpdateThemeUseCase.Output execute(ValidateThemeDomainService.Input input) {
        ...
        return new CreateOrUpdateThemeUseCase.Output(toTheme(sanitized), errors);
    }
    ...
    private static Theme toTheme(ValidateThemeDomainService.Input sanitized) {
        var id = sanitized.themeHrid() == null || sanitized.themeHrid().isBlank()
            ? null
            : HRIDToUUID.theme().context(sanitized.auditInfo()).hrid(sanitized.themeHrid()).id();
