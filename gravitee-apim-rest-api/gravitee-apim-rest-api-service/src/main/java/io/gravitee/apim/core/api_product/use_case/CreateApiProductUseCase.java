...
public class CreateApiProductUseCase {
    ...
    public record Input(CreateApiProduct createApiProduct, AuditInfo auditInfo) {
        public Input {
            if (createApiProduct == null) {
                throw new IllegalArgumentException("CreateApiProduct payload is required");
            }
            if (auditInfo == null) {
                throw new IllegalArgumentException("AuditInfo is required");
            }
            if (createApiProduct.getName() == null || createApiProduct.getName().trim().isEmpty()) {
                throw new TechnicalManagementException("Api Product name is required");
            }
            if (createApiProduct.getVersion() == null || createApiProduct.getVersion().isBlank()) {
                throw new TechnicalManagementException("Api Product version is required");
            }
        }
    }
    ...
    public Output execute(Input input) throws TechnicalException {
        var auditInfo = input.auditInfo;
        var payload = input.createApiProduct;
        String name = payload.getName().trim();

        apiProductQueryService
            .findByEnvironmentIdAndName(auditInfo.environmentId(), name)
            .ifPresent(existing -> {
                throw new ValidationDomainException(
                    "An API Product already exists in environment.",
                    Map.of("environmentId", auditInfo.environmentId(), "name", name)
                );
            });
