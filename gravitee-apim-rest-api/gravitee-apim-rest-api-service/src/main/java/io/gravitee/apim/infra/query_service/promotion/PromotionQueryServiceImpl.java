...
public class PromotionQueryServiceImpl implements PromotionQueryService {
    ...
    public Page<Promotion> search(PromotionQuery promotionQuery) throws TechnicalManagementException {
        ...
        if (promotionQuery.getApiId() != null && !promotionQuery.getApiId().isEmpty()) {
            criteria.apiId(promotionQuery.getApiId());
        }

        if (!CollectionUtils.isEmpty(promotionQuery.getStatuses())) {
            criteria.statuses(
                promotionQuery
                    .getStatuses()
                    .stream()
        ...
        if (!CollectionUtils.isEmpty(promotionQuery.getTargetEnvCockpitIds())) {
            criteria.targetEnvCockpitIds(promotionQuery.getTargetEnvCockpitIds().toArray(new String[0]));
        }

        if (promotionQuery.getTargetApiExists() != null) {
            criteria.targetApiExists(promotionQuery.getTargetApiExists());
        }
