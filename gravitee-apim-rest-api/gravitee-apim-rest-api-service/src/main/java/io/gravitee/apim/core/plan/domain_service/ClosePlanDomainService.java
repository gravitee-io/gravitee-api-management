...
public class ClosePlanDomainService {
    ...
    public void close(String planId, AuditInfo auditInfo) {
        ...
        var toUpdate = planCrudService.findById(planId);

        if (toUpdate.isClosed()) {
            throw new ValidationDomainException("Plan " + planId + " is already closed");
        }

        toUpdate.setPlanStatus(PlanStatus.CLOSED);
        toUpdate.setClosedAt(TimeProvider.now());
        toUpdate.setUpdatedAt(toUpdate.getClosedAt());
        toUpdate.setNeedRedeployAt(Date.from(toUpdate.getClosedAt().toInstant()));

        var planUpdated = planCrudService.update(toUpdate);
        createAuditLog(toUpdate, planUpdated, auditInfo);
    }
