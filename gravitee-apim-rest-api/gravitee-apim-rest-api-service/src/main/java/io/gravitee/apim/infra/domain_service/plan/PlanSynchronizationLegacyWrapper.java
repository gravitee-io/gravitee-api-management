...
public class PlanSynchronizationLegacyWrapper implements PlanSynchronizationService {
    ...
    @Override
    public boolean checkSynchronizedNative(Plan oldPlan, List<NativeFlow> oldFlows, Plan newPlan, List<NativeFlow> newFlows) {
        var oldPlanEntity = PlanAdapter.INSTANCE.toNativeEntityV4(oldPlan);
        oldPlanEntity.setFlows(oldFlows);
        var newPlanEntity = PlanAdapter.INSTANCE.toNativeEntityV4(newPlan);
        newPlanEntity.setFlows(newFlows);
