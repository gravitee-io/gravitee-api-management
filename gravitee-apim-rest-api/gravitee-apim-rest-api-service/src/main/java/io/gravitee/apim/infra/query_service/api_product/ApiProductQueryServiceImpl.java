...
/**
 * Implementation of ApiProductQueryService.
 * This service provides query operations for API Products.
 */
@Service
@Slf4j
public class ApiProductQueryServiceImpl implements ApiProductQueryService {
    ...
    private final ApiProductPrimaryOwnerDomainService primaryOwnerDomainService;

    public ApiProductQueryServiceImpl(
        @Lazy ApiProductsRepository apiProductRepository,
        @Lazy ApiProductPrimaryOwnerDomainService primaryOwnerDomainService
    ) {
    ...
    public Optional<ApiProduct> findByEnvironmentIdAndName(String environmentId, String name) {
        try {
            ...
            if (environmentId == null || name == null) {
                log.debug("environmentId or name is null, returning empty");
                return Optional.empty();
            }

            return apiProductRepository
                .findByEnvironmentIdAndName(environmentId, name)
                .map(ApiProductAdapter.INSTANCE::toModel)
                .map(this::addPrimaryOwnerInfo);
    ...
    public Optional<ApiProduct> findById(String apiProductId) {
        try {
            ...
            return repoApiProduct.map(ApiProductAdapter.INSTANCE::toModel).map(this::addPrimaryOwnerInfo);
        } catch (TechnicalException ex) {
    ...
    private ApiProduct addPrimaryOwnerInfo(ApiProduct apiProduct) {
        if (apiProduct == null) return null;
        try {
            String orgId = GraviteeContext.getCurrentOrganization();
            PrimaryOwnerEntity po = primaryOwnerDomainService.getApiProductPrimaryOwner(orgId, apiProduct.getId());
            apiProduct.setPrimaryOwner(po);
        } catch (Exception e) {
            log.warn("Could not fetch primary owner for API Product {}", apiProduct.getId(), e);
        }
        return apiProduct;
    }
}
