...
public class PortalNavigationListingDomainService {
    ...
    private static final class PathCollector implements PortalNavigationVisitor {
        ...
        @Override
        public void visitFolder(PortalNavigationFolder folder, String parentPath) {
            final var segment = folder.getEffectiveSegment();
            final var path = parentPath + "/" + segment;
            final var displayName = !segment.equals(folder.getTitle()) ? folder.getTitle() : null;
            paths.add(new NavigationPath(path, displayName));
        }
    }
}
