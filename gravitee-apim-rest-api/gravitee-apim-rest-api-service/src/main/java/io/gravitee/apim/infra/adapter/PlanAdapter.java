...
public interface PlanAdapter {
    ...
    PlanEntity toEntity(io.gravitee.repository.management.model.Plan plan);

    NewPlanEntity entityToNewPlanEntity(PlanEntity entity);

    @Named("computeCRDPlanSecurity")
    default io.gravitee.repository.management.model.Plan.PlanSecurityType computeCRDPlanSecurityType(CRDPlan crd) {
        return crd.getSecurity() == null
            ? null
            : io.gravitee.repository.management.model.Plan.PlanSecurityType.valueOf(crd.getSecurity().type());
    }
