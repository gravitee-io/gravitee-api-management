...
/**
 * Use case for disabling the subscription form for an environment.
 * When disabled, the form will not be displayed to API consumers during the subscription process.
...
@UseCase
@CustomLog
public class DisableSubscriptionFormUseCase {
    ...
    public Output execute(Input input) {
        ...
        existingForm.disable();
        var savedForm = subscriptionFormCrudService.update(existingForm);
        log.info("Disabled subscription form [{}] for environment [{}]", input.subscriptionFormId(), input.environmentId());
