...
public class CurrentThemeDomainService {
    ...
    public void disablePreviousEnabledTheme(Theme newCurrentTheme) {
        var enabledThemesPage =
            this.themeQueryService.searchByCriteria(
                    ThemeSearchCriteria.builder().type(newCurrentTheme.getType()).enabled(true).build(),
                    new PageableImpl(1, -1)
                );

        if (Objects.isNull(enabledThemesPage.getContent())) {
            return;
        }
        ...
        enabledThemesPage
            .getContent()
            .stream()
            .filter(theme -> !Objects.equals(theme.getId(), newCurrentTheme.getId()))
            .forEach(theme -> {
