...
@UseCase
public class GetMetricFacetSpecUseCase {

    private final AnalyticsDefinitionQueryService definition;

    public GetMetricFacetSpecUseCase(AnalyticsDefinitionQueryService definition) {
        this.definition = definition;
    }

    public record Input(String metric) {}

    public record Output(List<FacetSpec> specs) {}

    public Output execute(Input input) {
        var metric = definition.validateMetricName(input.metric());
        return new Output(definition.getFacets(metric));
    }
}
