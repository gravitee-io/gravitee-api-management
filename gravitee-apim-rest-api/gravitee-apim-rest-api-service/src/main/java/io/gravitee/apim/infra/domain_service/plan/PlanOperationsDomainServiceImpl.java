...
@Component
@CustomLog
public class PlanOperationsDomainServiceImpl implements PlanOperationsDomainService {

    private final PlanRepository planRepository;
    private final AuditService auditService;

    public PlanOperationsDomainServiceImpl(@Lazy PlanRepository planRepository, @Lazy AuditService auditService) {
        this.planRepository = planRepository;
        this.auditService = auditService;
    }

    @Override
    public Plan closePlanForApiProduct(final ExecutionContext executionContext, String planId) {
        try {
            ...
            // Update plan status
            plan.setStatus(io.gravitee.repository.management.model.Plan.Status.CLOSED);
    ...
    @Override
    public Plan deprecate(ExecutionContext executionContext, String planId) {
        try {
            ...
            io.gravitee.repository.management.model.Plan plan = planRepository
                ...
                .orElseThrow(() -> new PlanNotFoundException(planId));
            io.gravitee.repository.management.model.Plan previousPlan = new io.gravitee.repository.management.model.Plan(plan);
            if (plan.getStatus() == io.gravitee.repository.management.model.Plan.Status.DEPRECATED) {
                throw new PlanAlreadyDeprecatedException(planId);
            } else if (plan.getStatus() == io.gravitee.repository.management.model.Plan.Status.CLOSED) {
                throw new PlanAlreadyClosedException(planId);
            } else if (plan.getStatus() == io.gravitee.repository.management.model.Plan.Status.STAGING) {
                throw new PlanNotYetPublishedException(planId);
            }

            // Update plan status
            plan.setStatus(io.gravitee.repository.management.model.Plan.Status.DEPRECATED);
