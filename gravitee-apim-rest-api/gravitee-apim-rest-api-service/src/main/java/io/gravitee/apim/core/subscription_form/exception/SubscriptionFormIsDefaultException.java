...
/**
 * Thrown when deleting the environment default subscription form: it is the form every API without a dedicated
 * form falls back on, so it can be edited or hidden but never removed.
 *
...
public class SubscriptionFormIsDefaultException extends ConflictDomainException {
    ...
    public SubscriptionFormIsDefaultException(String subscriptionFormId) {
        super("The default subscription form cannot be deleted.", subscriptionFormId);
    }
}
