...
public class ApiProductPlanOperationsUseCase {
    ...
    private final PlanOperationsDomainService planOperationsDomainService;
    ...
    public Output execute(Input input) {
        ...
        if (input.operation.equals(DELETE)) {
            ...
            return new Output(planOperationsDomainService.closePlanForApiProduct(GraviteeContext.getExecutionContext(), input.planId));
        } else if (input.operation.equals(PUBLISH)) {
            log.debug("Plan {} published for API Product {}", input.planId, input.apiProductId);
            return new Output(planOperationsDomainService.publish(GraviteeContext.getExecutionContext(), input.planId));
        } else if (input.operation.equals(DEPRECATE)) {
            log.debug("Plan {} deprecated for API Product {}", input.planId, input.apiProductId);
            return new Output(planOperationsDomainService.deprecate(GraviteeContext.getExecutionContext(), input.planId));
        } else {
