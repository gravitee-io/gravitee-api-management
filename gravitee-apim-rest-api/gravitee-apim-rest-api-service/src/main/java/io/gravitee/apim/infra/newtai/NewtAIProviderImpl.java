...
public class NewtAIProviderImpl implements NewtAIProvider {
    ...
    private final ApimMetadataImpl apimMetadata;
    private final InstallationCrudService installationCrudService;

    public NewtAIProviderImpl(
        @Lazy CockpitConnector cockpitConnector,
        ApimMetadataImpl apimMetadata,
        InstallationCrudService installationCrudService
    ) {
    ...
    public Single<ELGenReply> generateEL(ELGenQuery query) {
        var installationId = installationCrudService.getInstallationId();
        if (installationId.isEmpty()) {
            return Single.error(new InstallationNotFoundException("Cockpit installation not found"));
        }
        var command = new ELCommand(
            new ELCommandPayload(query.message(), apimMetadata.getVersion(), query.properties(), installationId.get())
        );
