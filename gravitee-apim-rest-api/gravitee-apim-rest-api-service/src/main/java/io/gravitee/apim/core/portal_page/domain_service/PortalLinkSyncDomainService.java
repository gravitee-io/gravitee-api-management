...
public class PortalLinkSyncDomainService {
    ...
    public PortalNavigationLink materialize(
        ...
        if (existing instanceof PortalNavigationLink existingLink) {
            applyUpdate(existingLink, name, href, segment, order, parent);
    ...
    private static void applyUpdate(
        ...
        link.setOrder(order != null ? order : 0);
        link.setVisibility(PortalVisibility.PUBLIC);
        link.setPublished(true);
        if (parent == null) {
            link.markAsRoot();
        } else {
            link.updateParent(parent);
        }
    }
    ...
    private void rejectIfSegmentTakenByForeignItem(
        ...
        navigationItemsQueryService
            ...
            .ifPresent(squatter -> {
                throw PathConflictException.linkEntry(location);
            });
    }
}
