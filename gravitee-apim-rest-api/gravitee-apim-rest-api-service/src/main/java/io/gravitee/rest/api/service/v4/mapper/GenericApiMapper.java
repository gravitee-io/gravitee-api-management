    ...
    public GenericApiEntity toGenericApi(final Api api, final PrimaryOwnerEntity primaryOwner) {
        return switch (getVersionOfDefault(api)) {
            case V4 -> switch (api.getType()) {
                ...
                case A2A_PROXY, LLM_PROXY, MCP_PROXY, MCP_STUDIO, MESSAGE, PROXY -> apiMapper.toEntity(api, primaryOwner);
            };
    ...
    public GenericApiEntity toGenericApi(
        ...
        return switch (getVersionOfDefault(api)) {
            case V4 -> switch (api.getType()) {
                ...
                case A2A_PROXY, LLM_PROXY, MCP_PROXY, MCP_STUDIO, MESSAGE, PROXY -> apiMapper.toEntity(
                    executionContext,
                    api,
                    primaryOwner,
                    withApiFlows,
