...
public class EnvironmentsDefaultPortalNavigationItemsUpgrader implements Upgrader {
    ...
    public EnvironmentsDefaultPortalNavigationItemsUpgrader(@Lazy OrganizationRepository organizationRepository, @Lazy EnvironmentRepository environmentRepository, CreateDefaultPortalNavigationItemsUseCase createDefaultPortalNavigationItemsUseCase) {
    ...
    private boolean applyUpgrade() throws TechnicalException {
        for (var environment : environmentRepository.findAll()) {
            createDefaultPortalNavigationItemsUseCase.execute(environment.getId());
        }
        return true;
    }
}
