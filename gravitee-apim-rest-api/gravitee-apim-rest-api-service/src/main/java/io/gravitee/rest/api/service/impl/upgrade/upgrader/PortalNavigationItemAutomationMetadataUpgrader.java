...
public class PortalNavigationItemAutomationMetadataUpgrader implements Upgrader {
    ...
    private int migrateEnvironment(String organizationId, String environmentId, List<PortalPageContent> managedContents)
        ...
        // (rather than relying on the groupingBy to reject them) since a null key would throw instead
        // of being skipped. A single portalPageContentId can legitimately be shared by several nav
        // items (e.g. one API-doc page materialized into multiple nav-api rows), so every match must
        // be updated, not just one.
        Map<String, List<PortalNavigationItem>> pagesByContentId = portalNavigationItemRepository
        ...
        int updatedCount = 0;
        for (PortalPageContent content : managedContents) {
            List<PortalNavigationItem> pages = pagesByContentId.get(content.getId());
            if (pages == null) {
                continue; // no corresponding nav item
            }
            for (PortalNavigationItem page : pages) {
                if (page.getAutomationMetadata() != null) {
                    continue; // already migrated
                }
                page.setAutomationMetadata(content.getAutomationMetadata().trimmedForNavItem());
