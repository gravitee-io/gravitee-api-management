...
public class TaskServiceImpl extends AbstractService implements TaskService {
    ...
    private Set<String> getApiProductsForAPermission(
        ...
        for (MembershipAndPermissions membershipAndPermissions : membershipsAndPermissions) {
            if (membershipAndPermissions != null) {
                RoleEntity role = roleService.findById(membershipAndPermissions.membership.getRoleId());
                if (role.getScope() != RoleScope.API_PRODUCT) {
                    continue;
                }
                final char[] rights = membershipAndPermissions.permission.get(permission);
                if (rights != null) {
                    for (char c : rights) {
                        if (c == 'U') {
                            if (
                                membershipAndPermissions.membership.getReferenceType() ==
                                io.gravitee.rest.api.model.MembershipReferenceType.API_PRODUCT
                            ) {
                                apiProductIds.add(membershipAndPermissions.membership.getReferenceId());
                            }
                            break;
                        }
                    }
        ...
        if (isEnvironmentAdmin() && executionContext.hasEnvironmentId()) {
            try {
                Set<ApiProduct> envProducts = apiProductsRepository.findByEnvironmentId(executionContext.getEnvironmentId());
                if (envProducts != null) {
                    envProducts.stream().map(ApiProduct::getId).forEach(apiProductIds::add);
                }
            } catch (TechnicalException e) {
                log.error("Error retrieving API Products for environment admin tasks: {}", e.getMessage());
            }
        }
