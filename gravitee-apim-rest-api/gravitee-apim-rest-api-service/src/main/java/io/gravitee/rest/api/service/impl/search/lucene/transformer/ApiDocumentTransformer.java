...
public class ApiDocumentTransformer implements DocumentTransformer<GenericApiEntity> {
    ...
    public static final String FIELD_UPDATED_AT_SORTED = "updatedAt_sorted";
    ...
    public Document transform(GenericApiEntity api) {
        ...
        if (api.getUpdatedAt() != null) {
            ...
            doc.add(new NumericDocValuesField(FIELD_UPDATED_AT_SORTED, api.getUpdatedAt().getTime()));
        }
    ...
    private boolean isAllowedInApiProduct(GenericApiEntity api) {
        return switch (api) {
            case io.gravitee.rest.api.model.v4.api.ApiEntity v4 -> Boolean.TRUE.equals(v4.getAllowedInApiProducts());
            default -> false;
        };
    }
