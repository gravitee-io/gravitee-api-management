...
public class ApiLoggingConditionUpgrader implements Upgrader, Ordered {
    ...
    private void setExecutionStatus(InstallationEntity installation, UpgradeStatus status) {
        installation.getAdditionalInformation().put(InstallationService.API_LOGGING_CONDITION_UPGRADER_STATUS, status.toString());
    ...
    private boolean isStatus(InstallationEntity installation, UpgradeStatus status) {
        return status.name().equals(installation.getAdditionalInformation().get(InstallationService.API_LOGGING_CONDITION_UPGRADER_STATUS));
    }
}
