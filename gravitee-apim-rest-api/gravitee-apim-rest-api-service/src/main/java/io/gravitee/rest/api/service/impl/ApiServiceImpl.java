...
public class ApiServiceImpl extends AbstractService implements ApiService {
    ...
    private List<Api> findApisByUser(String userId, ApiQuery apiQuery, boolean portal) {
        ...
        if (userId != null) {
            ...
            // get APIs from user group memberships
            groupApis = findApisByUserGroups(userId, apiQuery, portal);
    ...
    private void createPageAndMedia(ApiEntity createdApiEntity, JsonNode jsonNode) {
        ...
        if (pages != null && pages.isArray()) {
            for (JsonNode page : pages) {
                PageEntity pageEntity = pageService.createWithDefinition(
    ...
    private void createOrUpdateChildrenPages(String apiId, String parentId, List<PageEntityTreeNode> children) {
        for (final PageEntityTreeNode child : children) {
            ...
            if (pageEntityToImport.getId() != null) {
                ...
                List<PageEntity> pages = pageService.search(query, GraviteeContext.getCurrentEnvironment());
                if (pages.size() == 1) {
