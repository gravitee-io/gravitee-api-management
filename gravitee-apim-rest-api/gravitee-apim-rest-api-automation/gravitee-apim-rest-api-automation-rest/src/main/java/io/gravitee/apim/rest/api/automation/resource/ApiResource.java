...
public class ApiResource extends AbstractResource {
    ...
    public Response getApiByHRID() {
        ...
        var input = new ExportApiCRDUseCase.Input(null, hrid, buildAuditInfo(executionContext, userDetails));
    ...
    public Response deleteApi() {
        var executionContext = GraviteeContext.getExecutionContext();
        var userDetails = getAuthenticatedUserDetails();
        AuditInfo auditInfo = buildAuditInfo(executionContext, userDetails);
        try {
            apiServiceV4.delete(GraviteeContext.getExecutionContext(), new UuidString.CrossId(auditInfo, hrid).toID(auditInfo), true);
        } catch (ApiNotFoundException e) {
            throw new HRIDNotFoundException(hrid);
        }
