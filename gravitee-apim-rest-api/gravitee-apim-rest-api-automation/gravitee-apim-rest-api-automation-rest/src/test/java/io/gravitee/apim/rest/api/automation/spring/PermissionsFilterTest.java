...
public class PermissionsFilterTest {
    ...
    class Api {
        ...
        @Test
        public void shouldBeAuthorizedWhenApiPermissionsWithLegacyHrid() {
    ...
    class Application {
        ...
        @Test
        public void shouldBeAuthorizedWhenApplicationPermissionsWithLegacyHrid() {
            when(permissionService.hasPermission(any(), any(), any(), any())).thenReturn(true);
    ...
    class Subscription {
        ...
        public void initApplicationMock() {
            Permission perm = mock(Permission.class);
            when(perm.value()).thenReturn(APPLICATION_SUBSCRIPTION);
            ...
            String jsonString = String.format("{\"%s\":\"%s\"}", HRID_PARAM, HRID);
            when(containerRequestContext.hasEntity()).thenReturn(true);
            when(containerRequestContext.getEntityStream())
                .thenReturn(new ByteArrayInputStream(jsonString.getBytes(StandardCharsets.UTF_8)));
        }
        ...
        public void shouldBeAuthorizedWhenApplicationPermissionsWithLegacyHrid() {
            ...
            verify(permissionService, times(1)).hasPermission(any(), eq(APPLICATION_SUBSCRIPTION), eq(HRID), eq(CREATE));
        }
