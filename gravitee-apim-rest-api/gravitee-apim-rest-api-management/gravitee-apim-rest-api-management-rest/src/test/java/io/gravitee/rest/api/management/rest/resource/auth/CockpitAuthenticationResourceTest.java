...
import static io.gravitee.rest.api.management.rest.resource.auth.CockpitAuthenticationResource.*;
 ...
 */
public class CockpitAuthenticationResourceTest extends AbstractResourceTest {
    ...
    @Before
    public void init() {
        System.setProperty("cockpit.enabled", "true");
        System.setProperty("cockpit.connector.ws.ssl.keystore.type", "PKCS12");
        System.setProperty(
            "cockpit.truststore.path",
            new File(getClass().getClassLoader().getResource("auth/ca-truststore.p12").getFile()).getAbsolutePath()
        );
        System.setProperty("cockpit.truststore.password", "truststore-secret");
        System.setProperty("cockpit.truststore.type", "PKCS12");
        System.setProperty(
            "cockpit.keystore.path",
            new File(getClass().getClassLoader().getResource("auth/keystore.p12").getFile()).getAbsolutePath()
        );
        System.setProperty("cockpit.keystore.password", "keystore-secret");
        System.setProperty("cockpit.keystore.type", "PKCS12");
        System.setProperty("cockpit.keystore.key.alias", "cockpit-ca");
        reset(userService);
    }

    @After
    public void teardown() {
        System.clearProperty("cockpit.enabled");
        System.clearProperty("cockpit.connector.ws.ssl.keystore.type");
        System.clearProperty("cockpit.truststore.path");
        System.clearProperty("cockpit.truststore.password");
        System.clearProperty("cockpit.truststore.type");
        System.clearProperty("cockpit.keystore.path");
        System.clearProperty("cockpit.keystore.password");
        System.clearProperty("cockpit.keystore.type");
        System.clearProperty("cockpit.keystore.key.alias");
    }
    ...
    private String generateJWT(
        ...
        try {
            KeyStore keystore =
                this.loadKeyStore(
                        System.getProperty("cockpit.keystore.type"),
                        System.getProperty("cockpit.keystore.path"),
                        System.getProperty("cockpit.keystore.password")
                    );
            PrivateKey privateKey = (PrivateKey) keystore.getKey(
                System.getProperty("cockpit.keystore.key.alias"),
                System.getProperty("cockpit.keystore.password").toCharArray()
            );
