...
public class OAuth2AuthenticationResourceTest extends AbstractResourceTest {
    ...
    public void shouldConnectExistingUser() throws Exception {
        ...
        when(userService.createOrUpdateUserFromSocialIdentityProvider(any(), eq(identityProvider), anyString(), anyString(), anyString()))
    ...
    public void should_exchange_token_by_query_param()
        ...
        UserEntity userEntity = mockUserEntity();
        //mock DB user connect
        when(userService.createOrUpdateUserFromSocialIdentityProvider(any(), eq(identityProvider), any(), anyString(), anyString()))
            .thenReturn(userEntity);
    ...
    public void shouldConnectNewUserWithGroupsMappingFromUserInfo() throws Exception {
        ...
        verify(userService, times(1))
            .createOrUpdateUserFromSocialIdentityProvider(any(), refEq(identityProvider), anyString(), anyString(), anyString());
