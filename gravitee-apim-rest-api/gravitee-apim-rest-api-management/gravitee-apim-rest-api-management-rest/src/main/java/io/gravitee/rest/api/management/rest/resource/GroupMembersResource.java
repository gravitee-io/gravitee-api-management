        ...
        if (!hasPermission) {
            if (groupEntity.getMaxInvitation() != null) {
                ...
                final long membershipsToAddSize = memberships
                    ...
                    .count();
                if (
                    (groupService.getNumberOfMembers(GraviteeContext.getExecutionContext(), group) + membershipsToAddSize) >
        ...
        for (GroupMembership membership : memberships) {
            ...
            if (membership.getRoles() != null && !membership.getRoles().isEmpty()) {
                ...
                if (previousApiRole == null && previousApplicationRole == null && previousGroupRole == null && updatedMembership != null) {
                    UserEntity userEntity = this.userService.findById(GraviteeContext.getExecutionContext(), updatedMembership.getId());
                    Map<String, Object> params = new HashMap<>();
                    params.put("group", groupEntity);
                    params.put("user", userEntity);
                    this.notifierService.trigger(GraviteeContext.getExecutionContext(), GROUP_INVITATION, params);
                }
            }
        }
