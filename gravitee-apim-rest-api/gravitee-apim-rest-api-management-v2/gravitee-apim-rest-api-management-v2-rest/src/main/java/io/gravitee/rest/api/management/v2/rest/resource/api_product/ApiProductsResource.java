...
/**
 * REST resource for API Product operations.
 * Handles API Product-related endpoints
 *
...
public class ApiProductsResource extends AbstractResource {
    ...
    public Response verifyApiProductName(@Valid @NotNull final VerifyApiProduct verifyApiProduct) {
        ...
        try {
            ...
            return Response.ok("API Product name [" + verifyApiProduct.getName() + "] is available").build();
        } catch (Exception e) {
            log.warn("API Product name verification failed: {}", e.getMessage());
    ...
    public Response createApiProduct(@Valid @NotNull final CreateApiProduct createApiProduct) throws TechnicalException {
        ...
        var apiProduct = createApiProductUseCase.execute(input);

        return Response.created(uriInfo.getAbsolutePathBuilder().path(apiProduct.apiProduct().getId()).build()).entity(apiProduct).build();
    }
    ...
    public Response getApiProducts(@BeanParam @Valid PaginationParam paginationParam) {
        var executionContext = GraviteeContext.getExecutionContext().getEnvironmentId();
        var output = getApiProductsUseCase.execute(GetApiProductsUseCase.Input.of(executionContext));
