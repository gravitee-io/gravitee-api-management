...
public class ApiPlansResource extends AbstractResource {
    ...
    public Response patchApiPlan(
        ...
        var builder = Response.ok(planV4);
        if (updatedAt != null) {
            var updatedAtInstant = updatedAt.toInstant();
            builder.tag(Long.toString(updatedAtInstant.toEpochMilli())).lastModified(Date.from(updatedAtInstant));
        }
