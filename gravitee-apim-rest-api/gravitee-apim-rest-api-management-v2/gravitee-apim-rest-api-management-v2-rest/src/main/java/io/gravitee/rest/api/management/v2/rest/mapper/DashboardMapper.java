...
public interface DashboardMapper {
    ...
    default Filter mapToRestFilter(DashboardWidget.Filter filter) {
        ...
        switch (operator) {
            case EQ -> restFilter.setActualInstance(
                new StringFilter().name(filterName).operator(operator).value((String) filter.getValue())
            );
            case LTE, GTE -> restFilter.setActualInstance(
                new NumberFilter().name(filterName).operator(operator).value(((Number) filter.getValue()).intValue())
            );
            case IN -> restFilter.setActualInstance(
                new ArrayFilter().name(filterName).operator(operator).value((List<String>) filter.getValue())
            );
        }
        return restFilter;
    }
