...
public interface FlowMapper {
    ...
    default List<io.gravitee.definition.model.v4.flow.selector.Selector> mapToSelectorEntityList(List<Selector> selectors) {
        return stream(selectors)
            .map(selector -> {
                ...
                return switch (selector.getActualInstance()) {
                    case HttpSelector ignored -> mapSelector(selector.getHttpSelector());
                    case ConditionSelector ignored -> mapSelector(selector.getConditionSelector());
                    case ChannelSelector ignored -> mapSelector(selector.getChannelSelector());
                    default -> null;
                };
            })
            .toList();
    ...
    default List<Selector> mapToSelectorApiModelList(List<io.gravitee.definition.model.v4.flow.selector.Selector> selectors) {
        return stream(selectors)
            .map(selector ->
                switch (selector) {
                    case io.gravitee.definition.model.v4.flow.selector.HttpSelector http -> new Selector(mapSelector(http));
                    case io.gravitee.definition.model.v4.flow.selector.ConditionSelector condition -> new Selector(mapSelector(condition));
                    case io.gravitee.definition.model.v4.flow.selector.ChannelSelector channel -> new Selector(mapSelector(channel));
