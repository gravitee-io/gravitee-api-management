...
public interface ImportExportApiMapper {
    ...
    /**
     * The {@code metadata} field on {@link io.gravitee.rest.api.management.v2.rest.model.GenericApi} is
     * an expand-only field populated only when {@code expands=metadata} is requested on GET /apis.
     * It must never appear in export/import payloads. Explicitly null it out here to override the
     * {@code new HashMap<>()} default generated by the OpenAPI codegen, which would otherwise
     * serialize as {@code "metadata": {}} in every exported API definition.
     */
    @AfterMapping
    default void clearMetadataExpand(@MappingTarget ApiV4 target) {
