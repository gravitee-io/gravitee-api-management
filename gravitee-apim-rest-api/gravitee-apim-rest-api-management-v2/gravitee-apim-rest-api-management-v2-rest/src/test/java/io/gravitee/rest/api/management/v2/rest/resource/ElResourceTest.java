 ...
 */
class ElResourceTest extends AbstractResourceTest {

    @Override
    protected String contextPath() {
        return "/el/schema.json";
    }

    @Test
    void should_get_schema_file() {
        var response = rootTarget().request().get();
        assertThat(response.getStatus()).isEqualTo(HttpStatusCode.OK_200);
        var schema = response.readEntity(JsonNode.class);
        assertThat(schema).isNotNull();
        assertThat(schema.get("$schema")).isNotNull();
        assertThat(schema.get("$defs")).isNotNull();
