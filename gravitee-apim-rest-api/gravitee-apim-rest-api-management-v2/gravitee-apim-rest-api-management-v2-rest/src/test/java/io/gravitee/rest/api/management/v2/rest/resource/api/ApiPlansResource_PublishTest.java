...
public class ApiPlansResource_PublishTest extends ApiPlansResourceTest {
    ...
    protected String contextPath() {
        return "/environments/" + ENVIRONMENT + "/apis/" + API + "/plans" + "/" + PLAN + "/_publish";
    }
    ...
    public void should_return_404_if_not_found() {
        ...
        verify(planServiceV4, never()).publish(any(), any());
        verify(planServiceV2, never()).publish(any(), any());
    }
    ...
    public void should_return_plan_when_v4_plan_published() {
        ...
        when(planSearchService.findById(GraviteeContext.getExecutionContext(), PLAN)).thenReturn(planEntity);
        when(planServiceV4.publish(GraviteeContext.getExecutionContext(), PLAN)).thenReturn(planEntity);
        ...
        verify(planServiceV4, times(1)).publish(GraviteeContext.getExecutionContext(), PLAN);
    }
    ...
    public void should_return_plan_when_v2_plan_published() {
        ...
        when(planSearchService.findById(GraviteeContext.getExecutionContext(), PLAN)).thenReturn(planEntity);
        when(planServiceV2.publish(GraviteeContext.getExecutionContext(), PLAN)).thenReturn(planEntity);
        ...
        verify(planServiceV2, times(1)).publish(GraviteeContext.getExecutionContext(), PLAN);
    }
}
