...
public class ApiPlansResourceTest extends AbstractResourceTest {
    ...
    class Update {
        ...
        public void should_update_v4_plan() {
            ...
            when(groupService.findAllByEnvironment(anyString())).thenReturn(mockGroups());
        ...
        private Set<Group> mockGroups() {
            Set<Group> mockGroups = new HashSet<>();

            // Mocking two groups as an example
            mockGroups.add(Group.builder().id("excludedGroup1").environmentId("environmentId").name("Group 1").build());

            mockGroups.add(Group.builder().id("excludedGroup2").environmentId("environmentId").name("Group 2").build());
