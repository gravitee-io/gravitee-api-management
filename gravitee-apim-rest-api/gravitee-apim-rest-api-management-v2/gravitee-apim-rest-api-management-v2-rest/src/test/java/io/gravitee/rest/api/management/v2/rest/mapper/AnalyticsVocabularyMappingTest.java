...
class AnalyticsVocabularyMappingTest {
    ...
    /**
     * Derived from the catalog rather than listed here: the invariant is "the analytics vocabulary refuses
     * exactly what the catalog withholds from the analytics signal". A literal would make every team adding
     * a logs-only filter edit this test.
     */
    private static Set<String> logsOnlyFilterNames() {
        var catalog = new AnalyticsDefinitionYAMLQueryService();
        var analytics = catalog
            .getFilters(Set.of(Signal.ANALYTICS))
            .stream()
        ...
        return catalog
            ...
            .stream()
            .map(spec -> spec.name().name())
            .filter(name -> !analytics.contains(name))
            .collect(Collectors.toSet());
    }
