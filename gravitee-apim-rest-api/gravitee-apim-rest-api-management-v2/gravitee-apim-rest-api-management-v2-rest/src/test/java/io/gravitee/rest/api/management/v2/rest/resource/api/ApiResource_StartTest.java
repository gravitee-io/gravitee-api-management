...
public class ApiResource_StartTest extends AbstractResourceTest {
    ...
    public void init() throws TechnicalException {
        reset(apiServiceV4, parameterService, apiSearchServiceV4, apiStateServiceV4, workflowService);
    ...
    public void should_not_start_api_if_already_started() {
        var startedEntity = apiEntity;
        startedEntity.setState(Lifecycle.State.STARTED);

        when(apiSearchServiceV4.findGenericById(eq(GraviteeContext.getExecutionContext()), eq(API))).thenReturn(startedEntity);
    ...
    public void should_start_api() {
        ...
        when(apiStateServiceV4.start(eq(GraviteeContext.getExecutionContext()), eq(API), eq(USER_NAME))).thenReturn(updatedEntity);
