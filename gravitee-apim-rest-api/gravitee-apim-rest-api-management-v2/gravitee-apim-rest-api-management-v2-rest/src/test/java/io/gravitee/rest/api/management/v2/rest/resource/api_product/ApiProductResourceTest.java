...
class ApiProductResourceTest extends AbstractResourceTest {
    ...
    class DeleteAllApisTest {
        ...
        void should_delete_all_apis_from_api_product() {
            when(updateApiProductUseCase.execute(any())).thenReturn(
                new UpdateApiProductUseCase.Output(
            ...
            SoftAssertions.assertSoftly(soft -> {
                var input = captor.getValue();
                soft.assertThat(input.apiProductId()).isEqualTo(API_PRODUCT_ID);
                soft.assertThat(input.updateApiProduct().getApiIds()).isEmpty();
            });
        }
    ...
    class DeleteSingleApiTest {
        ...
        void should_delete_single_api_from_api_product() {
            ApiProduct existingApiProduct = ApiProduct.builder()
                .id(API_PRODUCT_ID)
                .environmentId(ENV_ID)
                .name("My API Product")
                .apiIds(Set.of(API_ID, "api-2"))
                .build();

            when(getApiProductByIdUseCase.execute(any())).thenReturn(GetApiProductsUseCase.Output.single(Optional.of(existingApiProduct)));
            ...
            SoftAssertions.assertSoftly(soft -> {
                var input = captor.getValue();
                soft.assertThat(input.apiProductId()).isEqualTo(API_PRODUCT_ID);
                soft.assertThat(input.updateApiProduct().getApiIds()).containsExactly("api-2");
            });
        }
