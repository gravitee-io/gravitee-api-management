...
public class OidcLogoutService {
    ...
    public OidcLogoutService(
        ...
        this.cipher = new OidcIdTokenCookieCipher(environment.getProperty("jwt.secret", ""));
    }
    ...
    private String readIdentityProviderId(HttpServletRequest request) {
        Cookie cookie = WebUtils.getCookie(request, OidcCookieNames.IDENTITY_PROVIDER_COOKIE);
        return cookie != null ? cookie.getValue() : null;
    }

    private String readIdTokenHint(HttpServletRequest request) {
        Cookie cookie = WebUtils.getCookie(request, OidcCookieNames.ID_TOKEN_COOKIE);
        if (cookie == null || !StringUtils.hasText(cookie.getValue())) {
            return null;
        }
        return cipher.decrypt(cookie.getValue()).orElse(null);
    }
