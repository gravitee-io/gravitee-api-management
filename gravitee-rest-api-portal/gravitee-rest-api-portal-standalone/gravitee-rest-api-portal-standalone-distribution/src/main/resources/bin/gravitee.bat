...
for /f %%i in ('dir ..\lib\gravitee-rest-api-portal-standalone-bootstrap-*.jar /s /b') do set runjar=%%i
...
%JAVA% %JAVA_OPTS% -cp %GRAVITEE_BOOT_CLASSPATH% -Dgravitee.home=%GRAVITEE_HOME% -Dvertx.disableFileCaching=true -Dvertx.disableFileCPResolving=true -Dvertx.logger-delegate-factory-class-name=io.vertx.core.logging.SLF4JLogDelegateFactory io.gravitee.rest.api.portal.standalone.boostrap.Bootstrap "%*"
