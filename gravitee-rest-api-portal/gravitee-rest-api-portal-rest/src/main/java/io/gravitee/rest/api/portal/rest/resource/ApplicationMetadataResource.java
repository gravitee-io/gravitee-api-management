...
public class ApplicationMetadataResource extends AbstractResource {
    ...
    public Response getMetadataByApplicationId(@PathParam("applicationId") String applicationId, @BeanParam PaginationParam paginationParam) {
        List<ReferenceMetadata> applicationMetadata = metadataService.findAllByApplication(applicationId).stream()
                .map(this.referenceMetadataMapper::convert)
                .collect(Collectors.toList());
        return Response
                .ok(this.createDataResponse(applicationMetadata, paginationParam, null, true))
                .build();
    }
    ...
    public Response getApplicationMetadataByApplicationIdAndMetadataId(@PathParam("applicationId") String applicationId, @PathParam("metadataId") String metadataId) {
        return Response
                .ok(this.referenceMetadataMapper.convert(metadataService.findByIdAndApplication(metadataId, applicationId)))
                .build();
    }
    ...
    public Response updateApplicationMetadataByApplicationIdAndMetadataId(@PathParam("applicationId") String applicationId,
                                                                          @PathParam("metadataId") String metadataId,
                                                                          @Valid @NotNull final ReferenceMetadataInput metadata) {
        // prevent creation of a metadata on an another APPLICATION
        UpdateApplicationMetadataEntity updateApplicationMetadataEntity = this.referenceMetadataMapper.convert(metadata, applicationId, metadataId);

        return Response
                .ok(this.referenceMetadataMapper.convert(metadataService.update(updateApplicationMetadataEntity)))
                .build();
    }
    ...
    public Response deleteApplicationMetadata(@PathParam("applicationId") String applicationId, @PathParam("metadataId") String metadataId) {
