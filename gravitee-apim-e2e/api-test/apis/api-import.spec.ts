...
describe('Api import', () => {
  ...
  apiImportArray.forEach(({ name, payload, type, format, expectedFlowsLength }) => {
    describe(`API import via ${name}`, () => {
      let apiId: string;
      ...
      it('should import API without creating a documentation', async () => {
        const importSwaggerApiResponse = await apisResourceApiUser.importSwaggerApiRaw({
          orgId,
          envId,
          definitionVersion,
        ...
        expect(importSwaggerApiResponse.raw.status).toEqual(201);

        const api = await importSwaggerApiResponse.value();
        apiId = api.id;
        expect(api.id).not.toEqual('');
        expect(api.visibility).toEqual('PRIVATE');
        expect(api.state).toEqual('STOPPED');

        const pages = await pagesResourcesApiUser.getApiPages({ orgId, envId, api: api.id });
        expect(pages).toHaveLength(1);
      });

      it('should import API and create a swagger documentation', async () => {
        const importSwaggerApiResponse = await apisResourceApiUser.importSwaggerApiRaw({
          orgId,
          envId,
          definitionVersion,
      ...
      it('should fail to import the same Swagger API again', async () => {
        const importSwaggerApiResponse = await apisResourceApiUser.importSwaggerApiRaw({
          orgId,
          envId,
          definitionVersion,
      ...
      it('should import API and create a path (to add policies) for every declared Swagger path', async () => {
        const importSwaggerApiResponse = await apisResourceApiUser.importSwaggerApiRaw({
          orgId,
          envId,
          definitionVersion,
...
describe('API import with no provided import data', () => {
  it('should fail when trying to import an empty file/URL', async () => {
    await fail(
