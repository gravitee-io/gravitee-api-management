...
describe('API - V4 - Native Kafka - Import - Gravitee Definition - With metadata', () => {
  describe('Create v4 API from import with metadata', () => {
    ...
    const importedApiWithApimTeamFn = (): string => importedApiWithApimTeam.id;
    const importedSecondApiWithApimTeamFn = (): string => importedSecondApiWithApimTeam.id;
    const importedApiWithAmTeamFn = (): string => importedApiWithAmTeam.id;
    const importedApiWithCockpitTeamFn = (): string => importedApiWithCockpitTeam.id;
    ...
    afterAll(async () => {
      ...
      await noContent(
        v2ApisResourceAsApiPublisher.deleteApiRaw({
          envId,
          apiId: importedApiWithGlobalEmailMetadata.id,
        }),
      );
