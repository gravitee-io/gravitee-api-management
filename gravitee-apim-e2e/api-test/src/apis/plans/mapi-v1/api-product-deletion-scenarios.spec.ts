...
describeIfClientGatewaySupportingApiProduct(
  'C1 - Delete API Product with active plans/subscriptions: all product keys become invalid',
  () => {
    let api1: ApiV4;
    let api2: ApiV4;
    ...
    beforeAll(async () => {
      ...
      const subscriptionResponse = await fetch(`${managementV2BaseUrl}/environments/${envId}/api-products/${productId}/subscriptions`, {
        method: 'POST',
        headers: { Authorization: adminAuthHeader, 'Content-Type': 'application/json' },
        body: JSON.stringify({ applicationId: createdApp.id, planId: planBody.id }),
      });
...
describeIfClientGatewaySupportingApiProduct(
  'C2.1 - Stop and delete an underlying API: its path returns 404; sibling API remains 200',
  () => {
    let api1: ApiV4;
    let api2: ApiV4;
    let productId: string;
    ...
    beforeAll(async () => {
      ...
      const createdApp = await applicationsResourceAsAdmin.createApplication({
        ...
        envId,
        newApplicationEntity: ApplicationsFaker.newApplication({
          name: `e2e-c21-app-${Date.now()}`,
          description: 'C2.1 app',
        }),
      });
...
describeIfClientGatewaySupportingApiProduct(
  'C2.2 - Detach API from product scope while it remains deployed: detached path returns 401',
  () => {
    let api1: ApiV4;
    let api2: ApiV4;
    let productId: string;
    ...
    beforeAll(async () => {
      ...
      const createdApp = await applicationsResourceAsAdmin.createApplication({
        ...
        envId,
        newApplicationEntity: ApplicationsFaker.newApplication({
          name: `e2e-c22-app-${Date.now()}`,
          description: 'C2.2 app',
        }),
      });
...
describeIfClientGatewaySupportingApiProduct('C3 - Detach API from product mapping without explicit redeploy: access stops promptly', () => {
  let api1: ApiV4;
  let api2: ApiV4;
  let productId: string;
  let apiKey: string;
