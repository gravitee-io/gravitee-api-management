...
describe('API Product plans (management v2)', () => {
  ...
  test('should publish, deprecate then close API Product plan', async () => {
    ...
    // Verify deprecation by fetching the plan (response body may not reflect status immediately)
    const getAfterDeprecateResponse = await fetch(
      `${managementV2BaseUrl}/environments/${envId}/api-products/${apiProduct.id}/plans/${plan.id}`,
      {
        method: 'GET',
        headers: { Authorization: adminAuthHeader },
      },
    );
    expect(getAfterDeprecateResponse.status).toEqual(200);
    const deprecatedPlan = (await getAfterDeprecateResponse.json()) as ApiProductPlan;
    ...
    // Verify closure by fetching the plan (response body may not reflect status immediately)
    const getAfterCloseResponse = await fetch(
      `${managementV2BaseUrl}/environments/${envId}/api-products/${apiProduct.id}/plans/${plan.id}`,
      {
        method: 'GET',
        headers: { Authorization: adminAuthHeader },
    ...
    expect(getAfterCloseResponse.status).toEqual(200);
    const closedPlan = (await getAfterCloseResponse.json()) as ApiProductPlan;
