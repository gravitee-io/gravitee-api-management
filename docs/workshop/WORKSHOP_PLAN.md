...
5. **FE-1** — Angular service method + TypeScript response models + fixtures
6. **FE-2 / FE-3** — Enhanced stats cards + HTTP status pie chart (parallel)
7. **FE-4** — Dashboard layout, integration, removal of legacy response-status-ranges, and component tests
