...
public abstract class SingleValueFormatter<T extends Reportable> extends AbstractFormatter<T> {
    ...
    public void appendAdditional(WithAdditional withAdditional, Buffer buffer) {
        Map<String, Long> longAdditionalMetrics = withAdditional.longAdditionalMetrics();
        if (longAdditionalMetrics != null) {
            longAdditionalMetrics.values().forEach(value -> appendLong(buffer, value));
        }
        Map<String, Double> doubleAdditionalMetrics = withAdditional.doubleAdditionalMetrics();
        if (doubleAdditionalMetrics != null) {
            doubleAdditionalMetrics.values().forEach(value -> appendDouble(buffer, value));
        }
        ...
        if (keywordListAdditionalMetrics != null) {
            keywordListAdditionalMetrics.values().forEach(values -> appendString(buffer, String.join(LIST_SEPARATOR, values), true, false));
        }
        Map<String, Boolean> boolAdditionalMetrics = withAdditional.boolAdditionalMetrics();
        if (boolAdditionalMetrics != null) {
            boolAdditionalMetrics.values().forEach(value -> appendBoolean(buffer, value));
        }
        Map<String, Integer> intAdditionalMetrics = withAdditional.intAdditionalMetrics();
        if (intAdditionalMetrics != null) {
            intAdditionalMetrics.values().forEach(value -> appendInt(buffer, value));
        }
        Map<String, String> stringAdditionalMetrics = withAdditional.stringAdditionalMetrics();
        if (stringAdditionalMetrics != null) {
            stringAdditionalMetrics.values().forEach(value -> appendString(buffer, value, true, false));
        }
        Map<String, String> jsonAdditionalMetrics = withAdditional.jsonAdditionalMetrics();
        if (jsonAdditionalMetrics != null) {
            jsonAdditionalMetrics.values().forEach(value -> appendString(buffer, value, true, false));
        }
    }
}
