...
public class ElasticsearchFormatter<T extends Reportable> extends AbstractFormatter<T> {
    ...
    private final Map<Class<?>, ReportableFormatter<?>> formatters = new HashMap<>();
    ...
    public ElasticsearchFormatter(Node node, int elasticSearchVersion) {
        ...
        this.templatePath = String.format(TEMPLATES_PATH_PATTERN, elasticSearchVersion);
    ...
    public Buffer format0(T reportable, Map<String, Object> esOptions) {
        ReportableFormatter<T> formatter = (ReportableFormatter<T>) formatters.get(reportable.getClass());
     ...
     * @return ES bulk line
     */
    private Buffer getSource(final Metrics metrics, Map<String, Object> esOptions) {
     ...
     * @return ES bulk line
     */
    private Buffer getSource(final EndpointStatus endpointStatus, Map<String, Object> esOptions) {
        final Map<String, Object> data = new HashMap<>(5);
     ...
     * @return ES bulk line
     */
    private Buffer getSource(final Monitor monitor, Map<String, Object> esOptions) {
    ...
    private Buffer getSource(io.gravitee.reporter.api.v4.log.Log log, Map<String, Object> esOptions) {
        final Map<String, Object> data = new HashMap<>(5);
    ...
    private Buffer getSource(OperationEventMetrics metrics, Map<String, Object> esOptions) {
        return getSourceForEventMetrics(metrics, esOptions, "operation-event-metrics.ftl");
    }

    private Buffer getSource(TopicEventMetrics metrics, Map<String, Object> esOptions) {
        return getSourceForEventMetrics(metrics, esOptions, "topic-event-metrics.ftl");
    }

    private Buffer getSource(ApplicationEventMetrics metrics, Map<String, Object> esOptions) {
        return getSourceForEventMetrics(metrics, esOptions, "application-event-metrics.ftl");
    }

    private Buffer getSource(ApiEventMetrics metrics, Map<String, Object> esOptions) {
    ...
    private Buffer generateData(String template, Map<String, Object> data) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            freeMarkerComponent.generateFromTemplate(templatePath + template, data, new OutputStreamWriter(baos));
    ...
    private void initFormatters() {
        ...
        formatters.put(EndpointStatus.class, (r, o) -> getSource((EndpointStatus) r, o));
        ...
        formatters.put(MessageMetrics.class, (r, o) -> getSource((MessageMetrics) r, o));
        formatters.put(io.gravitee.reporter.api.v4.log.Log.class, (r, o) -> getSource((io.gravitee.reporter.api.v4.log.Log) r, o));
        formatters.put(MessageLog.class, (r, o) -> getSource((MessageLog) r, o));
        formatters.put(OperationEventMetrics.class, (r, o) -> getSource((OperationEventMetrics) r, o));
