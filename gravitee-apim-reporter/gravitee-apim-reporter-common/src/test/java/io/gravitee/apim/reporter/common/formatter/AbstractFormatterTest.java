 ...
 */
package io.gravitee.apim.reporter.common.formatter;

import static io.gravitee.apim.reporter.common.formatter.Mappers.JSON;
...
public abstract class AbstractFormatterTest {
    ...
    private static final String BASE_PATH = "io/gravitee/reporter/common/formatter";
    ...
    private static final FormatterFactory FACTORY = new FormatterFactory(
        ...
        FormatterFactoryConfiguration.builder()
            .rule(
                ...
                rules(Set.of("*"), Set.of("clientRequest.uri", "clientResponse.status", "timestamp"), Map.of("timestamp", "date"))
            )
            .rule(MetricsType.REQUEST, rules(Set.of("*"), Set.of("timestamp", "api", "proxyLatencyMs"), Map.of("timestamp", "date")))
            .rule(MetricsType.NODE_MONITOR, rules(Set.of("*"), Set.of("timestamp", "os"), Map.of("timestamp", "date")))
            .rule(MetricsType.HEALTH_CHECK, rules(Set.of("*"), Set.of("timestamp", "api", "success"), Map.of("timestamp", "date")))
            .build()
    );
    ...
    protected Reportable readGiven(String path, String className) {
        try {
            ...
            var clazz = Class.forName(fullyQualifiedClassName).asSubclass(Reportable.class);
    ...
    protected String readExpected(String path) {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to read expected bytes from file " + filePath, e);
        }
    }

    private InputStream getResourceAsStream(String filePath) {
        var resourceAsStream = getClass().getClassLoader().getResourceAsStream(filePath);

        if (resourceAsStream == null) {
            throw new IllegalArgumentException("Unable to find expected file " + filePath);
        }

        return resourceAsStream;
    }

    private static Rules rules(Set<String> excluded, Set<String> included, Map<String, String> renamed) {
        var rules = new Rules();
