...
class IndexPreparerIntegrationTest {
    ...
    void should_put_templates_against_opensearch_when_ism_property_names_set(String esDir) throws Exception {
        ...
        // Data-stream types always go through the composable index template API, on every tree. Both are
        // read back: they render from separate templates, so a regression can drop the lifecycle block
        // from one and leave the other intact.
        for (String type : DATA_STREAM_TYPES) {
            String stored = storedTemplate("_index_template", "gravitee-ism-override-" + type);
            assertThat(stored).as("%s template", type).contains("policy-" + type);
            assertThat(stored).as("%s template", type).doesNotContain("rollover_alias");
        }
