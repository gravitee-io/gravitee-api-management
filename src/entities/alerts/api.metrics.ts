...
export class ApiMetrics extends Metrics {
  ...
  static QUOTA_COUNTER: ApiMetrics = new ApiMetrics('quota.counter', 'Quota Counter',
  ...
  static API: ApiMetrics = new ApiMetrics('api', 'API',
    [StringCondition.TYPE], [Scope.APPLICATION, Scope.PLATFORM], (type: number, id: string, $injector: any) => {
      ...
      if (type === 2) {
        ...
        ($injector.get('ApiService') as ApiService).searchApis()
          .then(result => {
        ...
        ($injector.get('ApplicationService') as ApplicationService).getSubscribedAPI(id)
          .then(result => {
            result.data.forEach((api) => {
              apis.push(new Tuple(api.id, api.name));
            });
  ...
  static APPLICATION: ApiMetrics = new ApiMetrics('application', 'Application',
    [StringCondition.TYPE], [Scope.API, Scope.PLATFORM], (type: number, id: string, $injector: any) => {
      ...
      if (type === 0) {
        ...
        ($injector.get('ApiService') as ApiService).getSubscribers(id)
          .then(result => {
  ...
  static PLAN: ApiMetrics = new ApiMetrics('plan', 'Plan',
    [StringCondition.TYPE], undefined, (type: number, id: string, $injector: any) => {
      ...
      if (type === 0) {
        ...
        ($injector.get('ApiService') as ApiService).getPublishedApiPlans(id)
          .then(result => {
        ...
        ($injector.get('ApplicationService') as ApplicationService).listSubscriptions(id)
          .then(result => {
