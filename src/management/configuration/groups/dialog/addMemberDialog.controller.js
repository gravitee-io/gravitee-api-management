...
function DialogAddGroupMemberController($scope, $mdDialog, group, GroupService, UserService, NotificationService) {
    ...
    $scope.searchUser = function (query) {
        if (query) {
            return UserService.search(query).then(function (response) {
                ...
                return _.filter(membersFound, function (member) {
                    return _.findIndex($scope.groupItem.members, function (m) {
    ...
    $scope.addMembers = function () {
        ...
        for (var i = 0; i < $scope.usersSelected.length; i++) {
            ...
            GroupService.addOrUpdateMember($scope.groupItem.group.id, member).then(function () {
...
exports.default = DialogAddGroupMemberController;
