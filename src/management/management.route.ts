...
function managementRouterConfig($stateProvider) {
  ...
  $stateProvider
    ...
    .state('management.audit', {
      ...
      resolve: {
        resolvedApis: (ApiService: ApiService) => ApiService.list().then((response) => response.data),
        resolvedApplications: (ApplicationService: ApplicationService) => ApplicationService.list().then((response) => response.data),
