...
class DocumentationController {
    ...
    this.list().then( () => {
      ...
      this.$scope.$on('dragulardrop', (e, el, target, source, dragularList, index) => {
        ...
        this.DocumentationService.editPage(this.$state.params.apiId, movedPage.id, movedPage).then( () => {
          ...
          this.$state.go("management.apis.detail.portal.documentation.page", {apiId: this.$state.params.apiId, pageId: movedPage.id});
          this.list();
        });
      });
    });
  ...
  showNewPageDialog(pageType) {
	  this.$state.go('management.apis.detail.portal.documentation.new', {type: pageType, fallbackPageId: this.getPageId()});
  }
