...
class ApplicationAnalyticsController {
  constructor(ApplicationService, ApiService, resolvedApplication, $q, $scope, $state, $timeout) {
    ...
    $scope.$watch('filteredAPIs', function () {
      that.updateCharts();
    }, true);
  }
  ...
  updateDate(date) {
    if (date) {
      this.$state.transitionTo(this.$state.current, {applicationId: this.application.id, timestamp: date.getTime() / 1000, timeframe: ''}, { notify: false });
  ...
  updateTimeframe(timeframeId) {
    if (timeframeId) {
      this.$state.transitionTo(this.$state.current, {applicationId: this.application.id, timestamp: '', timeframe: timeframeId}, { notify: false });
  ...
  pushTopHitsData() {
    ...
    _.forEach(this.analyticsData.tops, function (top) {
      if (top.key === 'top-apis') {
        var request = top.request.call(_this.ApplicationService, _this.application.id,
  ...
  updateCharts() {
    ...
    var queryFilter = '';
    if (this.$scope.filteredAPIs.length) {
      queryFilter = ' AND(';
      for (var i = 0; i < this.$scope.filteredAPIs.length; i++) {
        queryFilter += 'api:' + this.$scope.filteredAPIs[i].topAPI + (this.$scope.filteredAPIs.length - 1 === i ? ')' : ' OR ');
      }
    }
  ...
  analytics() {
    ...
    return {
      tops: [
        {
          title: 'Top APIs',
          request: this.ApplicationService.applicationTopHits,
          key: "top-apis",
          query: "application:" + _this.application.id,
          field: "api",
          size: 20
        }
      ],
      indicators: [
        {
          title: 'Total calls',
          request: this.ApplicationService.applicationGlobalHits,
          key: "total",
          query: "application:" + _this.application.id + " AND status:[100 TO 599]",
          color: 'silver',
          isTotal: true
        },
        {
          ...
          request: this.ApplicationService.applicationGlobalHits,
          key: "1xx",
          query: "application:" + _this.application.id + " AND status:[100 TO 199]",
        ...
        },
        {
          ...
          request: this.ApplicationService.applicationGlobalHits,
          key: "2xx",
          query: "application:" + _this.application.id + " AND status:[200 TO 299]",
        ...
        },
        {
          ...
          request: this.ApplicationService.applicationGlobalHits,
          key: "3xx",
          query: "application:" + _this.application.id + " AND status:[300 TO 399]",
        ...
        },
        {
          ...
          request: this.ApplicationService.applicationGlobalHits,
          key: "4xx",
          query: "application:" + _this.application.id + " AND status:[400 TO 499]",
        ...
        },
        {
          ...
          request: this.ApplicationService.applicationGlobalHits,
          key: "5xx",
          query: "application:" + _this.application.id + " AND status:[500 TO 599]",
      ...
      reports: [
        {
          ...
          requests: [
            {
              "service": this.ApplicationService.applicationHitsBy,
              "key": "application-hits-by-status",
              "query": "application:" + _this.application.id,
              "field": "status",
              "aggType": "terms"
            }
          ...
          requests: [
            {
              ...
              "service": this.ApplicationService.applicationHitsBy,
              "key": "application-hits-by-latency",
              "query": "application:" + _this.application.id,
              "field": "response-time",
              "aggType": "avg"
            }
