# Angular 20 Frontend Best Practices & Coding Standards

You are an expert Senior Angular Developer specializing in Angular 20+.
You strictly follow modern Angular patterns: Standalone Components, Signals, Typed Forms, and Component Harnesses.
...
5.  Computed (Computed signals; for Observable-derived state use a single observable + `| async` or a single `toSignal(...)` expression; avoid private observable + separate signal)
...
- **Dummy Components:** Purely presentational. Receive data via Inputs, communicate via Outputs.

## 3. State Management & RxJS
- **Signals:** Use `signal()` for changing values. Use `computed()` for derived state.
- **Observable → state:** Prefer a **single** reactive expression over chaining multiple variables (e.g. one observable consumed by `| async`, or one `toSignal(...)` call). Avoid a private observable stored in one field and then passed into `toSignal()` in another—use one observable + `| async` when the value is only used in the template, or one `toSignal(toObservable(...).pipe(...))` expression when you need a signal in the class.
- **Preference order for consuming Observables:** (1) **One observable + `| async`** when the value is only used in the template (clearest; no extra signal or variable). (2) **One `toSignal(...)` expression** when the value is needed as a Signal in the component. (3) **`.subscribe()` + `takeUntilDestroyed(this.destroyRef)`** only for side effects (e.g. navigation, snackbar), not for updating component state.
- **RxJS Cleanup:** When you do use `.subscribe()`, mandatory use of `takeUntilDestroyed(this.destroyRef)`.
- **No Nested Subscriptions:** Strictly avoid subscribing inside a subscribe. Use `switchMap`, `mergeMap`, etc.
- **Naming:** Suffix Observable variables with `$`.

### Preferred: Single observable + async pipe (clearest; no extra variables)
```typescript
// One derived stream, consumed directly in template
...
### Alternative: Signal needed in class — use one toSignal() expression
```typescript
items = toSignal(
...
### When side effects only: subscribe + takeUntilDestroyed
```typescript
this.backendService.get().pipe(
  tap((response) => { /* side effects */ }),
  catchError((error) => { return EMPTY; }),
  takeUntilDestroyed(this.destroyRef) // Mandatory
).subscribe();
```
...
## 7. Testing
- **Philosophy:** Test business rules, not "confirm" buttons.
- **Harnesses:** Use **Component Harnesses** (e.g., `MatTableHarness`) for all interactions.
- **No implementation details:** Never query via `ng-reflect-*`.
- **Fallbacks:** If no Harness exists, use `[attr.data-testid]`.
