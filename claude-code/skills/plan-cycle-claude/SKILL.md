---
name: plan-cycle-claude
description: Stress-test implementation plans with Claude Code CLI
---

Use this skill to refine an `implementation_plan.md` through structured debate with Claude Code CLI before presenting to the developer. This sits **between planning and implementation** — the plan gets forged under pressure so the developer reviews something already battle-tested.
...
- `claude` CLI available in the terminal (authenticated via `claude login` or `ANTHROPIC_API_KEY`)
...
| ----------------------------------- | --------------- |
| `cat file.md \| claude -p "prompt"` | Initial review  |
| `claude -c -p "prompt"`             | Continue debate |
| `claude --print "prompt"`           | Same as `-p`    |

> [!IMPORTANT]
> Use `-p` (print) mode for these skill steps to get a single turn of output without entering the interactive REPL.

> [!TIP]
> Claude Code automatically manages context within a project. `-c` continues the most recent conversation.
...
Send the full `implementation_plan.md` to Claude with a clear review mandate:
...
Be tough but constructive. If the plan is solid, say so — but look hard before you do." 2>/dev/null
```
...
4. If satisfied, confirm the plan is ready for human review" 2>/dev/null
```
...
- Claude confirms the plan is ready, OR no new substantive feedback
...
| Claude agrees with everything          | Ask Claude to steelman the opposite approach                |
