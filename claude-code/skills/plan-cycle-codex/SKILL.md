---
name: plan-cycle-codex
description: Stress-test implementation plans with Codex CLI
---

Use this skill to refine an `implementation_plan.md` through structured debate with Codex before presenting to the developer. This sits **between planning and implementation** — the plan gets forged under pressure so the developer reviews something already battle-tested.
...
| ---------------------------------------------------------------------------------------- | --------------- |
| `codex exec --full-auto -c hide_agent_reasoning=true "prompt" 2>/dev/null`               | Initial review  |
| `codex exec resume --last --full-auto -c hide_agent_reasoning=true "prompt" 2>/dev/null` | Continue debate |
...
Send the full `implementation_plan.md` to Codex with a clear review mandate:
...
- Codex confirms the plan is ready, OR no new substantive feedback
...
- **Codex pushes hard**: The whole point is pressure — easy agreement defeats the purpose.
...
| Issue                        | Solution                                                    |
| ---------------------------- | ----------------------------------------------------------- |
| Codex agrees with everything | Ask Codex to steelman the opposite approach                 |
