...
import {
  AbstractControl,
  ControlContainer, FormArray, FormBuilder,
  FormGroup,
  FormGroupDirective,
  ReactiveFormsModule,
  UntypedFormControl
...
export class RuntimeAlertCreateNotificationsComponent implements OnInit {
    ...
    this.dampeningForm.controls.mode.valueChanges
      ...
      .subscribe({
        next: (mode: DampeningModesNames) => {
          this.isDampeningVisible = false;
          this.seedDampeningControls(mode);
          this.isDampeningVisible = true;
        }
      })
  }
  ...
  seedDampeningControls(mode: DampeningModesNames) {
    // this.dampeningForm.get('trueEvaluations')?.setValue(undefined);
    // this.dampeningForm.get('totalEvaluations')?.setValue(undefined);
    // this.dampeningForm.get('duration')?.setValue(undefined);
    // this.dampeningForm.get('timeUnit')?.setValue(undefined);
    ...
    switch (mode) {
      case DampeningModesNames.STRICT_COUNT: // N consecutive true evaluations
        this.dampeningForm.addControl('trueEvaluations', new UntypedFormControl(null));
        break;
