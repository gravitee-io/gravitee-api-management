...
export class ApiProductConfigurationComponent {
  ...
  private readonly apiProductId$ = merge(
  ...
  private readonly fetchedProduct$ = this.apiProductId$.pipe(
  ...
  readonly apiProduct = toSignal(merge(this.fetchedProduct$, this.updatedProduct$), {
  ...
  onSubmit(): void {
    ...
    if (!form.valid || !apiProductId) {
      form.markAllAsTouched();
      return;
    }

    const { name, version, description } = form.getRawValue();
    ...
    this.apiProductV2Service
      ...
      .pipe(
        tap(updatedApiProduct => {
          ...
          this.initialFormValue.set(form.getRawValue());
          form.markAsPristine();
  ...
  private nameUniquenessValidator(): AsyncValidatorFn {
    return (formControl: FormControl<string>): Observable<ValidationErrors | null> => {
      ...
      const current = this.currentApiProduct;
      if (current && trimmedName === current.name) return of(null);
