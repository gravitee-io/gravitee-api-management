...
import {
  KafkaClustersAddDialogComponent,
  KafkaClustersAddDialogData,
  KafkaClustersAddDialogResult,
} from '../add-dialog/kafka-clusters-add-dialog.component';
...
@Component({
  selector: 'kafka-cluster-list',
  templateUrl: './kafka-cluster-list.component.html',
  styleUrls: ['./kafka-cluster-list.component.scss'],
  imports: [
    CommonModule,
    MatCardModule,
    MatButtonModule,
...
export class KafkaClusterListComponent implements OnInit {
        ...
        switchMap(() =>
          this.clusterService
            .list(
              ...
              toKafkaClustersSortByParam(this.filters.sort),
  ...
  protected addKafkaCluster() {
    this.matDialog
      .open<KafkaClustersAddDialogComponent, KafkaClustersAddDialogData, KafkaClustersAddDialogResult>(KafkaClustersAddDialogComponent, {
        role: 'alertdialog',
        id: 'addKafkaCluster',
      ...
      .afterClosed()
      .pipe(
        switchMap((result: KafkaClustersAddDialogResult) => {
          if (!result) {
            return EMPTY;
          }
        ...
        tap(cluster => {
          this.snackBarService.success('Kafka Cluster created successfully');
          return this.router.navigate([cluster.id], {
            relativeTo: this.activatedRoute,
          });
        }),
        catchError(e => {
          this.snackBarService.error(e.error?.message ?? 'An error occurred while creating the Kafka Cluster!');
          return EMPTY;
        }),
      )
      .subscribe();
  ...
  protected remove(cluster: PageTableVM['items'][number]) {
    this.matDialog
      ...
      .pipe(
        ...
        map(() => this.snackBarService.success(`The Kafka Cluster has been deleted.`)),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe(() => {
        this.refreshPageTableVM$.next();
...
function toKafkaClustersSortByParam(sort?: Sort): ClustersSortByParam | undefined {
  if (!sort?.active) {
    return undefined;
  }
