...
export class EditMemberDialogComponent implements OnInit {
  ...
  private initializeEditMemberForm() {
  ...
  updateMemberships() {
    if (this.isUpgradeRole()) {
      ...
      let groupMembership = this.updatedMemberships.find((membership) => membership.id === this.user.id);

      if (!groupMembership) {
        groupMembership = this.mapGroupMembership(this.user.id, this.user.reference, this.editMemberForm.controls['defaultAPIRole'].value);
      }
      this.updatedMemberships = [groupMembership];
    }
  }
