...
export class EditMemberDialogComponent implements OnInit {
  ...
  private downgradedMember: Member = null;
  ...
  private getUserDetails() {
    this.usersService.search(this.member.displayName).subscribe({
      next: (users) => {
        this.user = users.length > 0 ? users.find((u) => u.id === this.member.id) : undefined;
      },
      error: () => {
        this.snackBarService.error(`Error occurred while searching details of the user.`);
      },
    });
  }
  ...
  disableDefaultAPIRole(): void {
    if (!this.isSuperAdmin() && this.group.lockApiRole) {
      this.editMemberForm.controls.defaultAPIRole.disable();
    }
  }

  disableDefaultApplicationRole(): void {
    if (!this.isSuperAdmin() && this.group.lockApplicationRole) {
      this.editMemberForm.controls.defaultApplicationRole.disable();
    }
  }

  disableDefaultIntegrationRole(): void {
    if (!this.isSuperAdmin()) {
      this.editMemberForm.controls.defaultIntegrationRole.disable();
    }
  }

  disableAPIRoleOptions() {
  ...
  submit() {
    if (this.isUpgradeRole() && this.ifPrimaryOwnerPresent()) {
      this.usersService.search(this.downgradedMember.displayName).subscribe({
        next: (response) => {
          const ownerId = this.downgradedMember.id;
          const reference = response.find((u) => u.id === ownerId).reference;
