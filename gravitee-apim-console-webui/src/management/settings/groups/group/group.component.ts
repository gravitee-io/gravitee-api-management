...
export class GroupComponent implements OnInit {
  ...
  private refreshGroup = new BehaviorSubject<number>(1);
  private groupMembers = new BehaviorSubject<Member[]>([]);
  private refreshGroupMembers = new BehaviorSubject<number>(1);
  private groupInvitations = new BehaviorSubject<Invitation[]>([]);
  private refreshInvitations = new BehaviorSubject<number>(1);
  private groupAPIs = new BehaviorSubject<any[]>([]);
  private refreshGroupAPIs = new BehaviorSubject<number>(1);
  private groupApplications = new BehaviorSubject<any[]>([]);
  private refreshGroupApplications = new BehaviorSubject<number>(1);
  ...
  private fetchGroup() {
    return this.route.params.pipe(
      ...
      switchMap((groupId: string) => (groupId && groupId !== 'new' ? this.groupService.get(groupId) : of({} as Group))),
      takeUntilDestroyed(this.destroyRef),
    );
  }
  ...
  private initializeGroupMembers() {
    this.groupMembers$ = this.refreshGroupMembers.pipe(
      switchMap(() => this.groupService.getMembers(this.groupId)),
      tap((members: Member[]) => {
  ...
  private initializeInvitations() {
    this.invitations$ = this.refreshInvitations.pipe(
      switchMap(() => this.groupService.getInvitations(this.groupId)),
  ...
  private initializeGroupAPIs() {
    this.groupAPIs$ = this.refreshGroupAPIs.pipe(
      switchMap(() => this.groupService.getMemberships(this.groupId, 'api')),
  ...
  private initializeGroupApplications() {
    this.groupApplications$ = this.refreshGroupApplications.pipe(
      switchMap(() => this.groupService.getMemberships(this.groupId, 'application')),
  ...
  openDeleteMemberDialog(member: Member) {
    this.matDialog
      ...
      .afterClosed()
      .pipe(
        filter((dialogResult: DeleteMemberDialogResult) => dialogResult.shouldDelete),
        mergeMap((dialogResult: DeleteMemberDialogResult) => {
          return this.groupService.deleteMember(this.groupId, member.id).pipe(
            tap(() => {
              ...
              this.refreshGroupMembers.next(1);
            }),
            catchError(() => {
        ...
        switchMap((dialogResult: DeleteMemberDialogResult) => {
          const shouldTransferOwnership = member.roles['API'] === RoleName.PRIMARY_OWNER;

          return shouldTransferOwnership
            ? this.groupService.addOrUpdateMemberships(this.groupId, [dialogResult.primaryOwnerMembership]).pipe(
                tap(() => {
    ...
    this.matDialog
      .open<GioConfirmDialogComponent, GioConfirmDialogData, boolean>(GioConfirmDialogComponent, {
        data: {
          ...
          content: `Are you sure you want to delete the invitation sent to ${email}?`,
