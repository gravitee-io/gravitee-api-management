...
describe('PortalSettingsComponent', () => {
  ...
  describe('Portal settings form', () => {
    ...
    it('does not display Portal Next settings moved to Portal Settings', async () => {
      portalSettingsMock = fakePortalSettings();
      expectPortalSettingsGetRequest(portalSettingsMock);
      ...
      expect(await loader.getAllHarnesses(MatSlideToggleHarness.with({ selector: '#enable-portal-next-member-mapping' }))).toHaveLength(0);
      expect(await loader.getAllHarnesses(MatSlideToggleHarness.with({ selector: '#enable-portal-next-transfer-ownership' }))).toHaveLength(
        0,
      );
      expect(await loader.getAllHarnesses(MatSlideToggleHarness.with({ selector: '#enable-portal-next-invitations' }))).toHaveLength(0);
    });
