...
export class ApiImportV4FormComponent {
  ...
  private resolveImportRequest$(): Observable<ApiV4> | null {
    ...
    if (source === 'remote') {
      ...
      if (format === 'gravitee') {
        // Update-from-URL is currently impossible from the UI (Remote source is disabled in update mode — see `sources`)
        // and the backend exposes create only. APIM-12142 will introduce the update path.
