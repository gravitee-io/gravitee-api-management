...
describe('WindowedCount', () => {
  ...
  describe('parse', () => {
    ...
    it('should throw an error for an invalid format', () => {
      expect(() => WindowedCount.parse('10')).toThrowError('Invalid format');
      expect(() => WindowedCount.parse('10-20')).toThrowError('Invalid format');
    });

    it('should throw an error if count is not a number', () => {
      expect(() => WindowedCount.parse('foo/PT1S')).toThrowError('Invalid format');
    });
    it('should throw an error if window is not a duration', () => {
      expect(() => WindowedCount.parse('1/bar')).toThrowError('Invalid format');
    });
  });
