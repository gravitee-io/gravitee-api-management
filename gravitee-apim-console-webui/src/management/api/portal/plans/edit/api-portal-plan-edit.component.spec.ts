  ...
  describe('With a V2 API', () => {
    describe('Create', () => {
      ...
      it('should create new plan', async () => {
        ...
        req.flush({});
        expect(fakeUiRouter.go).toHaveBeenCalledWith('management.apis.detail.portal.plans', { status: 'STAGING' });
      });
    });
    ...
    describe('Edit', () => {
      ...
      it.each(['STAGING', 'PUBLISHED', 'DEPRECATED'])('should edit plan', async (status: PlanStatus) => {
        ...
        req.flush({});
        expect(fakeUiRouter.go).toHaveBeenCalledWith('management.apis.detail.portal.plans', { status: PLAN.status });
      });
    });
