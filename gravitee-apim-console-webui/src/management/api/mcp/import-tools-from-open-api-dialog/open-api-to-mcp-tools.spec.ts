import {convertOpenApiToMcpTools} from './open-api-to-mcp-tools';
...
`;
      ...
      it.each(cases)('converts %s with proper prefixes and names', async (_label, input) => {
        ...
        expect(getTool.parameters.properties).toHaveProperty('h_X-Custom-Header'); // Check if header is serialized
        expect(getTool.parameters.properties['h_X-Custom-Header'].description).toBe('A custom header for the request'); // Check description
