...
export class McpComponent implements OnInit {
  ...
  canEnableMcp$: Observable<boolean> = this.connectorPluginsV2Service
    ...
    .pipe(
      map((plugins) =>
        plugins.some((plugin) => plugin.id === 'mcp-entrypoint' && this.gioPermissionService.hasAnyMatching(['api-definition-u'])),
      ),
    );
  ...
  ngOnInit() {
    this.api$ = this.apiV2Service.getLastApiFetch(this.apiId).pipe(
      ...
      map((api) => ({ ...api, hasMCPEntrypoint: api.listeners.some((l) => l.type === 'MCP') }) as ApiVM),
    );
  }
