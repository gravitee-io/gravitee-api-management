...
describe('ApiRuntimeLogsComponent', () => {
  ...
  describe('GIVEN there are logs', () => {
    ...
    describe('when there is more than one page', () => {
      ...
      it('should display the 1st page and then navigate according to paginator configuration', fakeAsync(async () => {
        await initComponentWithLogs({ hasLogs: true, total });
        tick();

        const logsListHarness = await componentHarness.listHarness();
        const paginator = await logsListHarness.getPaginator();
        expect(await logsListHarness.countRows()).toStrictEqual(perPage);
        ...
        expectApiWithLogs(total, { page: 2, perPage, ...DEFAULT_LOGS_TIME });
        ...
        expectApiWithLogs(total, { page: 1, perPage: 25, ...DEFAULT_LOGS_TIME });
  ...
  describe('GIVEN there are logs with filters', () => {
    ...
    describe('when there is more than one page and we apply a filter on applications', () => {
      ...
      it('should filter on application', fakeAsync(async () => {
        const appName = 'a';
    ...
    describe('when there is more than one page and we apply a filter on plans', () => {
      ...
      it('should filter on plans', fakeAsync(async () => {
  ...
  function expectApiWithLogs(total: number, param: ApiLogsParam = { perPage: 10, page: 1 }) {
    ...
    if (param.from != null) expectedURL = expectedURL.concat(`&from=${param.from}`);
    if (param.to != null) expectedURL = expectedURL.concat(`&to=${param.to}`);
    if (param.entrypointIds) expectedURL = expectedURL.concat(`&entrypointIds=${param.entrypointIds}`);
    ...
    if (param.mcpMethods) expectedURL = expectedURL.concat(`&mcpMethods=${param.mcpMethods}`);
