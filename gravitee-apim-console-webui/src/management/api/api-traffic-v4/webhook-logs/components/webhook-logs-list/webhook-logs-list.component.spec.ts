...
describe('WebhookLogsListComponent', () => {
  ...
  const defaultLogs: WebhookLogWithId[] = [
    createWebhookLog({
      ...
      additionalMetrics: createAdditionalMetrics({ 'boolean_webhook_dl-queue': true }),
    }),
    createWebhookLog({
      id: 'log-2',
  ...
  function baseAdditionalMetrics(): WebhookAdditionalMetrics {
    return {
      'string_webhook_request-method': 'GET',
      string_webhook_url: 'https://callback.test',
      'keyword_webhook_application-id': 'app-id',
      'keyword_webhook_subscription-id': 'sub-id',
      'int_webhook_retry-count': 0,
      'string_webhook_retry-timeline': '[]',
      'long_webhook_request-timestamp': Date.now(),
