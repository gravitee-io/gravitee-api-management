...
export class WebhookSettingsDialogComponent implements OnInit {
  ...
  private api: ApiV4;
  analytics?: Analytics;
  webhookEntrypoint: Entrypoint | null = null;
   ...
   */
  private findWebhookEntrypoint(api: ApiV4): Entrypoint | null {
    return api.listeners?.flatMap((listener) => listener.entrypoints ?? []).find((entrypoint) => entrypoint.type === 'webhook') ?? null;
  }
  ...
  private buildForm(webhookEntrypoint?: Entrypoint | null): void {
  ...
  get samplingCountPerWindowParts(): { count?: number; window?: number } | null {
    if (this.samplingDisplayType === 'COUNT_PER_TIME_WINDOW') {
      return this.getCountPerWindowParts(this.sampling?.value);
    }
    return null;
  }
  ...
  private get samplingDisplayType(): SamplingDisplayType {
    const sampling = this.sampling;
    return this.getSamplingDisplayType(sampling?.type, sampling?.value);
  }
