...
export class AnalyticsLayoutComponent implements OnDestroy {
  ...
  filterFields = input<string[]>(['plans']);
  ...
  private createQueryParamsFromFilters(filters: any): Record<string, any> {
    ...
    if (fields.includes('plans') && (filters as any).plans?.length) {
      params.plans = (filters as any).plans.join(',');
    }
    if (fields.includes('httpStatuses') && (filters as any).httpStatuses?.length) {
      params.httpStatuses = (filters as any).httpStatuses.join(',');
    }
    if (fields.includes('applications') && (filters as any).applications?.length) {
      params.applications = (filters as any).applications.join(',');
    }
  ...
  private getFilterFields(queryParams: BaseQueryParams) {
    ...
    if (fields.includes('plans')) {
      result.plans = this.processFilter(queryParams.plans);
    }
    if (fields.includes('httpStatuses')) {
      result.httpStatuses = this.processFilter(queryParams.httpStatuses);
    }
    if (fields.includes('applications')) {
      result.applications = this.processFilter(queryParams.applications);
    }
