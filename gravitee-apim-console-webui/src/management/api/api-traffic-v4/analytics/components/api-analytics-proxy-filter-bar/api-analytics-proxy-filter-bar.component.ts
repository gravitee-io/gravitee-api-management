...
import { ProxyFilters, ProxyFilterForm, ProxySpecificForm, FilterChip } from '../common/common-filters.types';
...
export class ApiAnalyticsProxyFilterBarComponent extends CommonFiltersComponent<ProxyFilters> implements OnInit {
  ...
  private applicationFilterChips: Signal<FilterChip[]> = toSignal(
    this.form.get('applications')?.valueChanges.pipe(switchMap((applications) => this.getApplicationChips(applications))) || of([]),
  );
  ...
  protected override removeSpecificFilter(key: string, value: string): void {
    if (key === 'httpStatuses') {
      const control = this.form.get('httpStatuses') as FormControl<string[] | null>;
      this.removeValueFromFilter(control.value, value, control);
    } else if (key === 'applications') {
      const control = this.form.get('applications') as FormControl<string[] | null>;
      this.removeValueFromFilter(control.value, value, control);
    }
  }
