...
describe('ApiGeneralInfoComponent', () => {
  ...
  const initComponent = (
    ...
    permissions = ['api-definition-u', 'api-definition-d', 'api-definition-c', 'api-definition-r'],
  ) => {
  ...
  describe('API V4 action strip permission gating', () => {
    ...
    it.each(actionButtons)('should not display the $button action button without $permission', async ({ permission, harnessFilter }) => {
      await renderProxyApiV4With(['api-definition-r', 'api-definition-c', 'api-definition-u'].filter(other => other !== permission));
