...
export class EnvApplicationListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = this.displayedColumnsByStatus.ACTIVE;
  ...
  ngOnInit(): void {
    const environmentId = this.$stateParams.environmentId;
      ...
      .pipe(
        ...
        switchMap(({ pagination, searchTerm, status, sort }) => {
          return this.applicationService
            .list(environmentId, status, searchTerm, this.toOrder(sort), pagination.index, pagination.size)
            .pipe(
  ...
  onStatusChange() {
    const filters = { ...this.defaultFilters, status: this.currentStatus };
  ...
  onRestoreActionClicked(application: TableData) {
    if (this.canRestore(application)) {
      this.matDialog
        ...
        .pipe(
          ...
          switchMap(() => this.applicationService.restore(this.$stateParams.environmentId, application.applicationId)),
  ...
  isArchived() {
    return 'ARCHIVED'.localeCompare(this.filters.status) === 0;
  }

  canListArchiveApplication() {
    return this.permissionService.hasRole({ scope: 'ORGANIZATION', name: 'ADMIN' });
  }

  canRestore(element) {
    return 'ARCHIVED'.localeCompare(element.status) === 0;
  }

  canEdit(element) {
    return !this.canRestore(element);
  }

  tableClassName() {
    return `applications-table-${this.currentStatus.toLowerCase()}`;
  }
  ...
  emptyMessage() {
    ...
    return 'There is no archived application';
  }
}
