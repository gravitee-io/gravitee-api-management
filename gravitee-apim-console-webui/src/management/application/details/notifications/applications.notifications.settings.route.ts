  ...
  $stateProvider
    .state('management.applications.application.notifications', {
      ...
      resolve: {
        ...
        resolvedHooks: ['NotificationSettingsService',(NotificationSettingsService: NotificationSettingsService) =>
          NotificationSettingsService.getHooks(Scope.APPLICATION).then((response) => response.data)],
        resolvedNotifiers: ['NotificationSettingsService', '$stateParams', (NotificationSettingsService: NotificationSettingsService, $stateParams) =>
          NotificationSettingsService.getNotifiers(Scope.APPLICATION, $stateParams.applicationId).then((response) => response.data)],
        notificationSettings: ['NotificationSettingsService', '$stateParams', (NotificationSettingsService: NotificationSettingsService, $stateParams) =>
          NotificationSettingsService.getNotificationSettings(Scope.APPLICATION, $stateParams.applicationId).then(
            (response) => response.data,
          )],
        alerts: ['AlertService', '$stateParams', (AlertService: AlertService, $stateParams) =>
