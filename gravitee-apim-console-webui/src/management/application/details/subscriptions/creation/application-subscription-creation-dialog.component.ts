...
type SubscriptionCreationForm = FormGroup<{
  ...
  customApiKey?: FormControl<string>;
...
export class ApplicationSubscriptionCreationDialogComponent {
  ...
  private canUseCustomApiKey = this.environmentSettingsService.getSnapshot().plan?.security?.customApiKey?.enabled;
  private canUseSharedApiKeys = this.environmentSettingsService.getSnapshot().plan?.security?.sharedApiKey?.enabled;
  private apiKeyModeSubscription: Subscription;
  ...
  protected apis$: Observable<Api[]> = this.form.controls.selectedApi.valueChanges.pipe(
    ...
    switchMap((term: string) => (term.length > 0 ? this.apiService.search({ query: term }) : of(null))),
  ...
  protected create() {
    this.dialogRef.close(
      toNewSubscriptionEntity(this.application, { ...this.form.getRawValue(), selectedApi: this.form.controls.selectedApi.value as Api }),
    );
  }
  ...
  private onPushPlanChange() {
    this.form.controls.selectedPlan.valueChanges
      .pipe(
        ...
        tap(() => {
          this.removeFormControls(['apiKeyMode', 'customApiKey']);
          this.apiKeyModeSubscription?.unsubscribe();
  ...
  private onJwtOauth2PlanChange() {
    this.form.controls.selectedPlan.valueChanges
      .pipe(
        ...
        tap(() => this.removeFormControls(['apiKeyMode', 'customApiKey', 'selectedEntrypoint', 'channel', 'entrypointConfiguration'])),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe();
  }
  ...
  private onApiKeyModeChange() {
    this.apiKeyModeSubscription = this.form.controls.apiKeyMode.valueChanges
      .pipe(
        distinctUntilChanged(),
        tap((value) => {
  ...
  private removeFormControls(names: ('apiKeyMode' | 'customApiKey' | 'selectedEntrypoint' | 'channel' | 'entrypointConfiguration')[]) {
