...
describe('ApplicationSubscriptionCreationDialogComponent', () => {
  ...
  const API_KEY_PLAN = fakePlanV2({ apiId: ANOTHER_API_ID, name: 'plan', security: { type: 'API_KEY' } });
  ...
  describe('subscription creation test', () => {
    ...
    describe('With api key plan', () => {
      const API_KEY_PLAN = fakePlanV4({ apiId: API_ID, security: { type: 'API_KEY' } });
      ...
      it('should create a subscription with a random key', fakeAsync(async () => {
        ...
        const subscription = fakeSubscription({ application: { apiKeyMode: APP.api_key_mode, ...APP }, plan: API_KEY_PLAN, api: API });
        expectApiSubscriptionsPostRequest(subscription, ApiKeyMode.EXCLUSIVE);
        expect(routerNavigateSpy).toHaveBeenCalledWith(['.', subscription.id], expect.anything());
      }));

      it('should create a subscription with a custom key', fakeAsync(async () => {
        await init(APP, {
          plan: {
            security: {
              ...DEFAULT_ENV_SETTINGS.plan.security,
        ...
        await dialogHarness.addCustomKey(customApiKey);
        expectApiKeyValidationPost(customApiKey);

        await dialogHarness.createSubscription();
        const subscription = fakeSubscription({ application: { apiKeyMode: APP.api_key_mode, ...APP }, plan: API_KEY_PLAN, api: API });
        expectApiSubscriptionsPostRequest(subscription, ApiKeyMode.EXCLUSIVE, customApiKey);
        expect(routerNavigateSpy).toHaveBeenCalledWith(['.', subscription.id], expect.anything());
      }));
      ...
      it('should create a subscription with shared api key', fakeAsync(async () => {
        ...
        const subscription = fakeSubscription({ application: { apiKeyMode: ApiKeyMode.SHARED, ...APP }, plan: API_KEY_PLAN, api: API });
    ...
    it.each(['JWT', 'OAUTH2'])(
      'should not be able to create subscription for %s plan when application client_id is null',
      fakeAsync(async (planType: PlanSecurityType) => {
        ...
        const plan = fakePlanV4({ apiId: API_ID, security: { type: planType } });
        expectSubscribableApiPlansGet([plan]);

        tick(100);
        await dialogHarness.selectPlan(plan.name);

        expect(await dialogHarness.isCreateSubscriptionDisabled()).toStrictEqual(true);
      }),
    );

    it.each(['JWT', 'OAUTH2'])(
      'should create subscription for %s plan',
      fakeAsync(async (planType: PlanSecurityType) => {
        await init();
        ...
        await dialogHarness.createSubscription();
        const subscription = fakeSubscription({ application: { apiKeyMode: APP.api_key_mode, ...APP }, api: API, plan });
        expectApiSubscriptionsPostRequest(subscription);
        expect(routerNavigateSpy).toHaveBeenCalledWith(['.', subscription.id], expect.anything());
      }),
    );
    ...
    it('should create subscription for push plan', fakeAsync(async () => {
      ...
      const pushPlan = fakePlanV4({ apiId: API_ID, security: null, mode: 'PUSH' });
      ...
      const subscription = fakeSubscription({
        application: { apiKeyMode: APP.api_key_mode, ...APP },
        api: API,
        plan: pushPlan,
        consumerConfiguration: { entrypointId: WEBHOOK_ENTRYPOINT.id },
      });
      expectApiSubscriptionsPostRequest(subscription, null, null, 'PUSH');
      expect(routerNavigateSpy).toHaveBeenCalledWith(['.', subscription.id], expect.anything());
    }));
  });
  ...
  const expectApiSubscriptionsPostRequest = (
    ...
    const req = httpTestingController.expectOne({
      url: `${CONSTANTS_TESTING.env.v2BaseURL}/apis/${subscription.api.id}/subscriptions`,
      method: 'POST',
    });
    expect(req.request.body).toEqual({
      ...
      ...(planMode === 'PUSH'
        ? {
            consumerConfiguration: expect.objectContaining({
              entrypointId: subscription.consumerConfiguration.entrypointId,
            }),
          }
        : {}),
  ...
  const expectApiKeyValidationPost = (apiKey: string) => {
    httpTestingController
      .match({
      ...
      .map((req) => {
        if (!req.cancelled) {
          expect(req.request.body).toEqual({ applicationId: APPLICATION_ID, apiKey });
          req.flush({ ok: true });
        }
      });
  };
