...
export class HasApplicationPermissionGuard implements CanActivate, CanActivateChild, CanDeactivate<ManagementComponent> {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    return this.gioPermissionService
      .loadApplicationPermissions(route.params.applicationId)
  ...
  canActivateChild(route: ActivatedRouteSnapshot, _state: RouterStateSnapshot): Observable<boolean> {
    const permissions = route.data.applicationPermissions?.only;
    if (!permissions) {
      return of(true);
    }
  ...
  canDeactivate(_component: ManagementComponent, _currentRoute: ActivatedRouteSnapshot, _currentState: RouterStateSnapshot): boolean {
    this.gioPermissionService.clearApplicationPermissions();
    return true;
  }
}
