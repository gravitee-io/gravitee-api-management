...
const ApiPortalHeaderComponentAjs: ng.IComponentOptions = {
  bindings: {
    apiPortalHeaders: '<',
    settings: '<',
  },
  ...
  controller: [
    ...
    function (
      ...
      this.$onInit = () => {
        Promise.all([ApiHeaderService.list(), PortalSettingsService.get()]).then(([apiHeaderResponse, portalSettingsResponse]) => {
          this.apiPortalHeaders = apiHeaderResponse.data;
...
export default ApiPortalHeaderComponentAjs;
