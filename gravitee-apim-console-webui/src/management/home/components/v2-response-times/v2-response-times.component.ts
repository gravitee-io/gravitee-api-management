...
@Component({
  selector: 'v2-response-times',
  standalone: true,
  imports: [GioChartLineModule, GioLoaderModule, MatCardModule],
  templateUrl: './v2-response-times.component.html',
  styleUrl: './v2-response-times.component.scss',
})
export class V2ResponseTimesComponent implements OnInit {
    ...
    this.homeService
      ...
      .pipe(
        switchMap(({ interval, from, to }) => {
          this.isLoading = true;
          return this.analyticsService.getAverageResponseTimes({
            interval,
            from,
            to,
          });
