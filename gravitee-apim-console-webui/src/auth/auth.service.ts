...
export class AuthService {
  ...
  checkAuth(): Observable<void> {
    ...
    this.oidcCallbackInFlight = this.completeOidcCallback(providerIdSelected, authorizationCode, state).pipe(
  ...
  private completeOidcCallback(providerId: string, authorizationCode: string, state: string | null): Observable<void> {
    const identityProvider = this.constants.org.identityProviders?.find(idp => idp.id === providerId);
    if (!identityProvider) {
