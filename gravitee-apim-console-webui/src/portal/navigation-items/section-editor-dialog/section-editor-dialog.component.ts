...
interface SectionFormControls {
  ...
  settings?: FormGroup<{ // Optional for 'LINK' type
...
@Component({
  ...
  imports: [MatDialogModule, ReactiveFormsModule, MatButtonModule, MatFormFieldModule, MatSlideToggleModule, MatInputModule],
...
export class SectionEditorDialogComponent implements OnInit {
  ...
  public mode: 'create';
  public title: string;
  public authenticationLabel: string;
  ...
  constructor() {
    ...
    this.authenticationLabel = `Authentication is required to view this ${this.type.toLowerCase()}.`
  }
  ...
  onSubmit(): void {
    if (this.form.valid) {
      this.dialogRef.close({ value: this.form.getRawValue() });
    }
  }
