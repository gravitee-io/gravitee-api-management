...
export class AgentSectionEditorDialogHarness extends ComponentHarness {
  static readonly hostSelector = 'agent-section-editor-dialog';
  ...
  private readonly locateAlreadyAddedLabels = this.locatorForAll(SpanHarness.with({ selector: '[data-testid^="agent-picker-already-added-"]' }));
  private readonly locateLoadError = this.locatorForOptional(SpanHarness.with({ selector: '[data-testid="agent-picker-load-error"]' }));
  private readonly locateLoading = this.locatorForOptional(SpanHarness.with({ selector: '[data-testid="agent-picker-loading"]' }));
  private readonly locateEmptyState = this.locatorForOptional(SpanHarness.with({ selector: '[data-testid="agent-picker-empty"]' }));
  ...
  async getAlreadyAddedLabel(agentId: string): Promise<SpanHarness | null> {
    const expectedDataTestId = `agent-picker-already-added-${agentId}`;
