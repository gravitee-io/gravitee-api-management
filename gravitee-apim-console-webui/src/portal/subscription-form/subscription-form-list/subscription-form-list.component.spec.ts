...
describe('SubscriptionFormListComponent', () => {
  ...
  it('should render a row per form and highlight the selected one', async () => {
    await init([globalForm, partnerForm], true, 'form-partner');
  ...
  it('should keep the toggle on the form visibility until the forms input changes', async () => {
    await init([partnerForm]);

    const getToggle = () => harnessLoader.getHarness(MatSlideToggleHarness.with({ selector: '[data-testid=enable-toggle-form-partner]' }));
    await (await getToggle()).toggle();
    expect(await (await getToggle()).isChecked()).toBe(false);

    fixture.componentRef.setInput('forms', [{ ...partnerForm, enabled: true }]);
    expect(await (await getToggle()).isChecked()).toBe(true);
  });

  it('should disable the toggles without the update permission', async () => {
    await init([globalForm], false);

    const toggle = await harnessLoader.getHarness(MatSlideToggleHarness.with({ selector: '[data-testid=enable-toggle-form-global]' }));
