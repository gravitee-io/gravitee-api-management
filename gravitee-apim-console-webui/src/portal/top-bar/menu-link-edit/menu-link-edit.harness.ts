...
export class MenuLinkEditHarness extends ComponentHarness {
  static readonly hostSelector = 'menu-link-edit';

  private getNameInput = this.locatorFor(MatInputHarness.with({ selector: '[formControlName=name]' }));
  private getTargetInput = this.locatorFor(MatInputHarness.with({ selector: '[formControlName=target]' }));
