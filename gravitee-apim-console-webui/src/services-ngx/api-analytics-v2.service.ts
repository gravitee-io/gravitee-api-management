...
export class ApiAnalyticsV2Service {
  ...
  getGroupByAnalytics(apiId: string, ranges: string, { from, to, interval }: TimeRangeParams) {
    const url = `${this.constants.env.v2BaseURL}/apis/${apiId}/analytics?type=GROUP_BY&field=status&ranges=${ranges}&interval=${interval}&from=${from}&to=${to}`;
    return this.http.get<GroupByAnalyticsResponse>(url);
  }
}
