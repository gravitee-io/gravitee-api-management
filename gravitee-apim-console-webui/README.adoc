...
This project uses the `gravitee-markdown` library which is built from the portal-webui-next project. Before running any development, linting, or build commands, you need to build this library first.

**Important**: The following commands automatically build the library before execution:
- `serve` - Builds library then starts dev server
- `build` - Builds library then builds the app
- `build:prod` - Builds library then builds the app in production mode
- `lint` - Builds library then runs linting
- `lint:fix` - Builds library then runs linting with auto-fix
...
The library is built to `node_modules/gravitee-markdown` and is automatically linked as a local dependency.
